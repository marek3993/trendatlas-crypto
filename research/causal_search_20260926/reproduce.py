...
def main():
    ...
    parser.add_argument('--cache',type=Path,default=engine.ROOT/'scratch/causal_search_entry_delay_cache')
    ...
    assert original==repeated,'Core research results differ on replay'
    for name,sha in original['files'].items():
        assert digest((reference/name).read_bytes())==sha
        assert digest((args.out/name).read_bytes())==sha
    ...
    write_json(reference/'reproducibility_verification.json',dict(
        all_core_files_identical=True,core_file_count=len(original['files']),
    ...
    print(f"PASS: {len(original['files'])} bitwise-identical core files; 54 fresh grid scenario replays")
