...
OUT_DIR = OUTPUTS / "phase51_btc_fallback"
...
ALL_SYMBOLS = [
    ...
    "DOGEUSDT", "LINKUSDT", "AVAXUSDT", "LTCUSDT", "TRXUSDT", "DOTUSDT",
]
...
MODEL_LABELS = {
    "phase51_main_current": "Current main",
    "phase51_btc_top_override": "BTC top-score override",
    "phase51_btc_trend_override": "BTC trend override",
    "phase51_btc_gap_override": "BTC score-gap override",
}
...
def btc_allowed_by_trend(btc_row: dict) -> bool:
    return (
        btc_row["lt_score"] >= BTC_TREND_RULE["lt_min"]
        and btc_row["st_score"] >= BTC_TREND_RULE["st_min"]
        and btc_row["xs20"] >= BTC_TREND_RULE["xs20_min"]
    )


def btc_allowed_by_gap(btc_row: dict, next_non_btc: dict | None) -> bool:
    if not (
        btc_row["lt_score"] >= BTC_GAP_RULE["lt_min"]
        and btc_row["st_score"] >= BTC_GAP_RULE["st_min"]
        and btc_row["xs20"] >= BTC_GAP_RULE["xs20_min"]
    ):
        return False

    if next_non_btc is None:
        return True

    return (btc_row["score"] - next_non_btc["score"]) >= BTC_GAP_RULE["gap_min"]


def select_daily_top1_variant(asset_tables: dict[str, pd.DataFrame], variant: str) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        if market_score >= MARKET_THRESHOLD and candidates:
            ...
            if variant == "main_current":
                top = candidates[0]
                selected = top["symbol"]

            elif variant == "btc_top_override":
                top = candidates[0]
                selected = top["symbol"]

            elif variant == "btc_trend_override":
                top = candidates[0]
                btc_candidate = next((c for c in candidates if c["symbol"] == "BTCUSDT"), None)
                if btc_candidate is not None and btc_allowed_by_trend(btc_candidate):
                    selected = "BTCUSDT"
                else:
                    selected = top["symbol"]

            elif variant == "btc_gap_override":
                top = candidates[0]
                btc_candidate = next((c for c in candidates if c["symbol"] == "BTCUSDT"), None)
                next_non_btc = next((c for c in candidates if c["symbol"] != "BTCUSDT"), None)
                if btc_candidate is not None and btc_allowed_by_gap(btc_candidate, next_non_btc):
                    selected = "BTCUSDT"
                else:
                    selected = top["symbol"]
...
def compute_recent_summary(model_df: pd.DataFrame) -> dict:
    recent = model_df.loc[model_df.index >= RECENT_START].copy()
    if recent.empty:
        return {
            "recent_total_return_pct": np.nan,
            "recent_cagr_pct": np.nan,
            "recent_max_drawdown_pct": np.nan,
            "recent_trade_count": np.nan,
            "recent_cash_days_pct": np.nan,
        }

    s = compute_summary(recent)
    return {
        "recent_total_return_pct": s["total_return_pct"],
        "recent_cagr_pct": s["cagr_pct"],
        "recent_max_drawdown_pct": s["max_drawdown_pct"],
        "recent_trade_count": s["trade_count"],
        "recent_cash_days_pct": s["cash_days_pct"],
    }
...
def main() -> None:
    ...
    variants = [
        ("phase51_main_current", "main_current"),
        ("phase51_btc_top_override", "btc_top_override"),
        ("phase51_btc_trend_override", "btc_trend_override"),
        ("phase51_btc_gap_override", "btc_gap_override"),
    ]

    for model_key, variant_key in variants:
        print(f"Running {model_key}...", flush=True)
        selection = select_daily_top1_variant(daily_full, variant_key)
    ...
    for model_key, df in model_runs.items():
        row = {
            ...
            **compute_summary(df),
            **compute_recent_summary(df),
        }
        summary_rows.append(row)

    summary_df = pd.DataFrame(summary_rows).sort_values(
        ["cagr_pct", "recent_cagr_pct", "max_drawdown_pct"],
        ascending=[False, False, False],
    )
    summary_df.to_csv(OUT_DIR / "phase51_btc_fallback_summary.csv", index=False)
    ...
    eq = eq.reset_index()
    eq.to_csv(OUT_DIR / "phase51_btc_fallback_equity_curves.csv", index=False)

    print("\n=== PHASE51 BTC FALLBACK SUMMARY ===\n")
    print(summary_df.to_string(index=False))
    print(f"\nSaved summary: {OUT_DIR / 'phase51_btc_fallback_summary.csv'}")
    print(f"Saved equity curves: {OUT_DIR / 'phase51_btc_fallback_equity_curves.csv'}")
