...
OUT_DIR = OUTPUTS / "phase57_profit_lock_robustness"
...
RULES = {
    "phase57_core": None,
    "phase57_lock_a": {"ret10_min": 0.10, "st_max": 20.0, "accel_max": -4.0},
    "phase57_lock_b": {"ret10_min": 0.10, "st_max": 24.0, "accel_max": 0.0},
    "phase57_lock_c": {"ret10_min": 0.12, "st_max": 24.0, "accel_max": 0.0},   # current winner zone
    "phase57_lock_d": {"ret10_min": 0.14, "st_max": 24.0, "accel_max": 0.0},
    "phase57_lock_e": {"ret10_min": 0.12, "st_max": 28.0, "accel_max": 4.0},
    "phase57_lock_f": {"ret10_min": 0.14, "st_max": 28.0, "accel_max": 4.0},
}

MODEL_LABELS = {
    "phase57_core": "Core",
    "phase57_lock_a": "Lock A",
    "phase57_lock_b": "Lock B",
    "phase57_lock_c": "Lock C",
    "phase57_lock_d": "Lock D",
    "phase57_lock_e": "Lock E",
    "phase57_lock_f": "Lock F",
}
...
def build_daily_tables(assets: dict[str, pd.DataFrame], macro_df: pd.DataFrame, symbols: list[str]) -> dict[str, pd.DataFrame]:
    ...
    for symbol, tbl in out.items():
        ...
        tbl["raw_ret10"] = ret10[symbol].reindex(tbl.index).fillna(0.0)
...
def profit_lock_trigger(row: pd.Series, rule: dict | None) -> bool:
    if rule is None:
        return False

    return (
        float(row["raw_ret10"]) >= rule["ret10_min"]
        and float(row["st_score"]) <= rule["st_max"]
        and float(row["xs_accel"]) <= rule["accel_max"]
    )
...
def select_daily_top1_variant(asset_tables: dict[str, pd.DataFrame], model_key: str) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        if (
            model_key != "phase57_core"
            and prev_selected != "CASH"
            and selected == prev_selected
            and prev_selected in candidate_rows
        ):
            held_row = candidate_rows[prev_selected]
            if profit_lock_trigger(held_row, rule):
                selected = "CASH"
...
def main() -> None:
    ...
    for model_key, df in model_runs.items():
        ...
        if RULES[model_key] is None:
            row["ret10_min"] = np.nan
            row["st_max"] = np.nan
            row["accel_max"] = np.nan
        else:
            row["ret10_min"] = RULES[model_key]["ret10_min"]
            row["st_max"] = RULES[model_key]["st_max"]
            row["accel_max"] = RULES[model_key]["accel_max"]
    ...
    summary_df.to_csv(OUT_DIR / "phase57_profit_lock_robustness_summary.csv", index=False)
    ...
    eq = eq.reset_index()
    eq.to_csv(OUT_DIR / "phase57_profit_lock_robustness_equity_curves.csv", index=False)

    print("\n=== PHASE57 PROFIT LOCK ROBUSTNESS ===\n")
    print(summary_df.to_string(index=False))
    print(f"\nSaved summary: {OUT_DIR / 'phase57_profit_lock_robustness_summary.csv'}")
    print(f"Saved equity curves: {OUT_DIR / 'phase57_profit_lock_robustness_equity_curves.csv'}")
