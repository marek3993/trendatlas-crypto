    ...
    if out.empty:
        return pd.DataFrame(
            columns=[
                ...
                "mr_score", "confidence", "yz_vol_20", "atr_pct", "general_score", "base_rank"
            ]
        ).rename_axis("ts")
    return out.set_index("ts")


def enrich_breakout(base_tables: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    rolling_high_20 = close_df.shift(1).rolling(20).max()
    rolling_high_30 = close_df.shift(1).rolling(30).max()
    ...
    # jednoduchý squeeze / release proxy
    vol20 = close_df.pct_change().rolling(20).std()
    vol5 = close_df.pct_change().rolling(5).std()
    squeeze_release = ((vol5 / (vol20 + 1e-9)) - 1.0).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    ...
    for symbol, tbl in base_tables.items():
        ...
        tmp["accel_score"] = accel[symbol].reindex(tmp.index).fillna(0.0)
        ...
        tmp["squeeze_release"] = squeeze_release[symbol].reindex(tmp.index).fillna(0.0)
...
def model_rank(row: pd.Series, model: str) -> float:
    ...
    if model == "rs_accel":
        return (
            ...
            + 0.35 * float(row["accel_score"])
        )

    if model == "rs_breakout":
        breakout_score = 180.0 * float(row["breakout20"]) + 120.0 * float(row["breakout30"])
        return (
            base_rs
            + 0.35 * float(row["xs_rank_7"])
            + 0.25 * breakout_score
            + 0.20 * float(row["accel_score"])
        )

    if model == "rs_breakout_volume":
        breakout_score = 180.0 * float(row["breakout20"]) + 120.0 * float(row["breakout30"])
        vol_score = 25.0 * clamp(float(row["vol_burst"]) - 1.0, 0.0, 3.0)
        squeeze_score = 20.0 * clamp(float(row["squeeze_release"]), -1.0, 2.0)
        return (
            base_rs
            + 0.35 * float(row["xs_rank_7"])
            + 0.25 * breakout_score
...
def result_row(model: str, paper: pd.DataFrame) -> dict:
    ...
    robust_score = (
        ...
        + 0.05 * median_ret
        - 0.60 * abs(worst_dd)
        + 12.0 * spikes["spike_capture_ratio"]
    )
...
def main() -> None:
    ...
    asset_tables = enrich_breakout(base_tables)
    ...
    for model in models:
        ...
        paper.to_csv(OUTPUT_DIR / f"phase13_breakout_{model}.csv")
    ...
    out.to_csv(OUTPUT_DIR / "phase13_breakout_results.csv", index=False)

    best = out.iloc[0].to_dict()
    with open(OUTPUT_DIR / "phase13_breakout_best.json", "w", encoding="utf-8") as f:
        json.dump(best, f, indent=2)

    print("\nPHASE 13 BREAKOUT")
    print(out.to_string(index=False))
    print("\nuložené:")
    print("outputs\\phase13_breakout_results.csv")
    print("outputs\\phase13_breakout_best.json")
    print("outputs\\phase13_breakout_baseline_rs.csv")
    print("outputs\\phase13_breakout_rs_accel.csv")
    print("outputs\\phase13_breakout_rs_breakout.csv")
    print("outputs\\phase13_breakout_rs_breakout_volume.csv")
