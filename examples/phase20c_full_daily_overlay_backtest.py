...
OUT_RESULTS_CSV = OUTPUTS_DIR / "phase20c_full_daily_overlay_results.csv"
OUT_EQUITY_CSV = OUTPUTS_DIR / "phase20c_full_daily_overlay_equity_curves.csv"
OUT_JSON = OUTPUTS_DIR / "phase20c_full_daily_overlay_best.json"
...
CANDIDATES = [
    {
        "model": "phase20_btc_3d_neg",
        "mode": "btc_3d_neg",
        "breadth_threshold": 0.35,
        "kill_multiplier": 0.0,
    },
    {
        "model": "phase20_combo_all_2d",
        "mode": "combo_all_2d",
        "breadth_threshold": 0.35,
        "kill_multiplier": 0.0,
    },
...
def run_overlay_backtest(
    ...
    # signal z daily close je použiteľný až ďalší deň
    exec_signal_by_day = daily_signal.shift(1).fillna(False)

    df["overlay_kill_day"] = df["date"].map(exec_signal_by_day).fillna(False).astype(bool)
    ...
    # cost/turnover nechávame rovnaké ako v base stream-e
    # je to konzervatívnejšie než nulovať ich ručne bez plného engine rerunu
    df["overlay_equity"] = (1.0 + df["overlay_strategy_ret"]).cumprod()
    df["overlay_equity"] = df["overlay_equity"] / df["overlay_equity"].iloc[0]

    df["overlay_selected"] = np.where(df["overlay_multiplier"] < 0.999999, "CASH_OVERLAY", df["selected"])
...
def main() -> None:
    ...
    for cfg in CANDIDATES:
        ...
        kill_day_pct = float(signal.shift(1).reindex(overlay_df["date"]).fillna(False).astype(bool).mean() * 100.0)

        results.append(
            {
                "model": cfg["model"],
                "overlay_mode": cfg["mode"],
                "breadth_threshold": cfg["breadth_threshold"],
                "kill_multiplier": cfg["kill_multiplier"],
        ...
        equity_curves[cfg["model"]] = overlay_df["overlay_equity"].values

        detail_path = OUTPUTS_DIR / f"{cfg['model']}_detail.csv"
        overlay_df[
            [
                "ts",
                "date",
    ...
    print("\n=== PHASE 20C FULL DAILY OVERLAY RESULTS ===")
