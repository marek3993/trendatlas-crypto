...
def make_param_grid() -> list[tuple[str, StrategyParams]]:
    out: list[tuple[str, StrategyParams]] = []
    ...
    for mode_name, allow_long, allow_short in modes:
        for enter_conf, hold_conf, long_mr_floor, short_mr_ceiling, exit_long_bias, exit_short_bias in product(
            [40.0, 50.0],
            [30.0, 35.0],
            [-80.0],
            [80.0],
            ...
            params = StrategyParams(
                ...
                short_mr_ceiling=short_mr_ceiling,
                exit_long_bias=exit_long_bias,
                exit_short_bias=exit_short_bias,
...
def main() -> None:
    ...
    if not files:
        raise RuntimeError("Nemáš žiadne CSV v data/ohlcv")
    ...
    for idx, (mode_name, params) in enumerate(grid, start=1):
        fold_rows = []
        asset_rows = []
        ...
        for symbol, ohlcv in assets.items():
            ...
            folds = split_folds(paper, n_folds=3)
        ...
        score = robust_score(fold_df)

        result_rows.append(
            {
                "mode": mode_name,
                "enter_conf": params.enter_conf,
                "hold_conf": params.hold_conf,
                "long_mr_floor": params.long_mr_floor,
                "short_mr_ceiling": params.short_mr_ceiling,
                "exit_long_bias": params.exit_long_bias,
                "exit_short_bias": params.exit_short_bias,
                "robust_score": score,
    ...
    res = pd.DataFrame(result_rows).sort_values("robust_score", ascending=False).reset_index(drop=True)
    res.to_csv(OUTPUT_DIR / "optimization_results.csv", index=False)
    ...
    print("\nUložené:", flush=True)
    print("outputs\\optimization_results.csv", flush=True)
    print("outputs\\best_params.json", flush=True)
