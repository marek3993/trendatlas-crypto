...
# fixed gates from current winner
...
# scan only ranking mix
BASE_RANK_WEIGHTS = [0.80, 1.00]
XS20_WEIGHTS = [0.35, 0.45]
XS90_WEIGHTS = [0.05, 0.10, 0.20]
XS_PERSIST_WEIGHTS = [0.10, 0.15]
ACCEL_WEIGHTS = [0.20, 0.25, 0.30]
RET5_WEIGHTS = [0.10, 0.15, 0.20]

OUT_RESULTS_CSV = OUTPUT_DIR / "phase42_ranking_weight_scan_results.csv"
OUT_JSON = OUTPUT_DIR / "phase42_ranking_weight_scan_best.json"
...
def main() -> None:
    ...
    for (
        ...
        variant = (
            f"br{base_rank_weight:.2f}"
            f"_x20{xs20_weight:.2f}"
            f"_x90{xs90_weight:.2f}"
            f"_xp{xs_persist_weight:.2f}"
            f"_ac{accel_weight:.2f}"
            f"_r5{ret5_weight:.2f}"
        ).replace(".", "p")
        ...
        daily_selection = select_daily_top1(
            ...
            xs20_weight=xs20_weight,
            xs90_weight=xs90_weight,
            xs_persist_weight=xs_persist_weight,
            accel_weight=accel_weight,
            ret5_weight=ret5_weight,
        )
        daily_selection.to_csv(OUTPUT_DIR / f"phase42_{variant}_selection.csv")

        paper = run_daily_model(daily_selection, daily_tables)
        paper.to_csv(OUTPUT_DIR / f"phase42_{variant}_paper.csv")
