...
UNIVERSE_VARIANTS = {
    "full12": ALL_SYMBOLS,
    "without_BNBUSDT": [s for s in ALL_SYMBOLS if s != "BNBUSDT"],
    "without_DOGEUSDT": [s for s in ALL_SYMBOLS if s != "DOGEUSDT"],
    "without_BNBUSDT_and_DOGEUSDT": [s for s in ALL_SYMBOLS if s not in {"BNBUSDT", "DOGEUSDT"}],
}
...
OUT_RESULTS_CSV = OUTPUT_DIR / "phase45_pruned_universe_confirm_results.csv"
OUT_EQUITY_CSV = OUTPUT_DIR / "phase45_pruned_universe_confirm_equity.csv"
OUT_JSON = OUTPUT_DIR / "phase45_pruned_universe_confirm_best.json"
...
def build_daily_tables(
    ...
    btc20 = ret20["BTCUSDT"] if "BTCUSDT" in ret20.columns else ret20.sub(ret20.mean(axis=1), axis=0).mean(axis=1) * 0.0
    btc90 = ret90["BTCUSDT"] if "BTCUSDT" in ret90.columns else ret90.sub(ret90.mean(axis=1), axis=0).mean(axis=1) * 0.0

    if "BTCUSDT" not in ret20.columns:
...
def evaluate_variant(all_assets: dict[str, pd.DataFrame], macro: pd.DataFrame, label: str, symbols: list[str]) -> tuple[dict, pd.DataFrame]:
...
def build_equity_compare(papers: dict[str, pd.DataFrame]) -> pd.DataFrame:
    cols = {}
    for label, paper in papers.items():
        eq = paper["equity"].copy()
        eq.name = label
        cols[label] = eq
...
def main() -> None:
    ...
    for label, symbols in UNIVERSE_VARIANTS.items():
        print(f"testujem {label} ...", flush=True)
        row, paper = evaluate_variant(all_assets, macro, label, symbols)
        rows.append(row)
        papers[label] = paper
    ...
    equity_df = build_equity_compare(papers)
    ...
    print("\n=== PHASE 45 PRUNED UNIVERSE CONFIRM ===")
