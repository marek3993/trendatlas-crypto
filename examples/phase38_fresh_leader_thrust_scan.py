...
# current winner base config from phase34/37
ACCEL_WEIGHT = 0.25
...
THRUST_WEIGHT_VALUES = [0.00, 0.10, 0.15, 0.20]
THRUST_MODE_VALUES = ["ret5", "xs_accel", "combo"]

OUT_RESULTS_CSV = OUTPUT_DIR / "phase38_fresh_leader_thrust_scan_results.csv"
OUT_JSON = OUTPUT_DIR / "phase38_fresh_leader_thrust_scan_best.json"
...
def select_daily_top1(asset_tables: dict[str, pd.DataFrame], thrust_weight: float, thrust_mode: str) -> pd.DataFrame:
...
def main() -> None:
    ...
    rows = []
    for thrust_mode, thrust_weight in product(THRUST_MODE_VALUES, THRUST_WEIGHT_VALUES):
        variant = f"{thrust_mode}_w{thrust_weight:.2f}".replace(".", "p")
        ...
        daily_selection.to_csv(OUTPUT_DIR / f"phase38_{variant}_selection.csv")

        paper = run_daily_model(daily_selection, daily_tables)
        paper.to_csv(OUTPUT_DIR / f"phase38_{variant}_paper.csv")
    ...
    print("\n=== PHASE 38 FRESH LEADER THRUST SCAN ===")
