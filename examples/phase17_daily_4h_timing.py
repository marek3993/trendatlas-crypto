...
def timing_ok(row: pd.Series, model: str) -> bool:
    ...
    if model == "fast_exit_4h":
        return (
            float(row["close"]) > float(row["ema42"])
            and float(row["xs18"]) > -5.0
            and float(row["accel"]) > -15.0
            and float(row["dv_rank"]) >= 0.20
            and float(row["breadth42"]) >= 0.30
            and float(row["atr_pct"]) <= 12.0
        )

    if model == "confirm_breakout_4h":
        return (
            float(row["close"]) > float(row["ema42"])
            and float(row["ema42"]) > float(row["ema126"])
            and float(row["xs18"]) > 0.0
            ...
            and float(row["breadth42"]) >= 0.35
            and float(row["atr_pct"]) <= 10.0
            and float(row["breakout30"]) >= -0.005
            and float(row["vol_burst"]) >= 1.00
        )
...
def get_daily_symbol_for_4h_ts(daily_sel: pd.Series, ts: pd.Timestamp) -> str:
    ...
    if signal_day not in daily_sel.index:
        subset = daily_sel.loc[daily_sel.index <= signal_day]
        if len(subset) == 0:
            return "CASH"
        return str(subset.iloc[-1])
    return str(daily_sel.loc[signal_day])
...
def main() -> None:
    ...
    daily_missing = [s for s in TARGET_SYMBOLS if not (DAILY_DIR / f"{s}_1d.csv").exists()]
    h4_missing = [s for s in TARGET_SYMBOLS if not (H4_DIR / f"{s}_4h.csv").exists()]

    if daily_missing:
        print("CHÝBAJÚ DAILY CSV:")
        for s in daily_missing:
            print(f"- data\\ohlcv\\{s}_1d.csv")
        return
    ...
    daily_selection.to_csv(OUTPUT_DIR / "phase17_daily_selection.csv")
    ...
    for model in models:
        ...
        paper = run_timing_model(daily_selection, h4_tables, model=model, total_cost_bps=15.0)
        rows.append(result_row(model, paper))
        paper.to_csv(OUTPUT_DIR / f"phase17_{model}.csv")
    ...
    out.to_csv(OUTPUT_DIR / "phase17_daily_4h_timing_results.csv", index=False)

    best = out.iloc[0].to_dict()
    with open(OUTPUT_DIR / "phase17_daily_4h_timing_best.json", "w", encoding="utf-8") as f:
        json.dump(best, f, indent=2)

    print("\nPHASE 17 DAILY + 4H TIMING")
    print(out.to_string(index=False))
    print("\nuložené:")
    print("outputs\\phase17_daily_4h_timing_results.csv")
    print("outputs\\phase17_daily_4h_timing_best.json")
    print("outputs\\phase17_daily_selection.csv")
    print("outputs\\phase17_always_daily.csv")
    print("outputs\\phase17_confirm_4h.csv")
    print("outputs\\phase17_fast_exit_4h.csv")
    print("outputs\\phase17_confirm_breakout_4h.csv")
