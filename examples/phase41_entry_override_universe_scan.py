...
UNIVERSE_VARIANTS = {
    "full12": ALL_SYMBOLS,
    "no_btc": [s for s in ALL_SYMBOLS if s != "BTCUSDT"],
    "no_trx": [s for s in ALL_SYMBOLS if s != "TRXUSDT"],
    "liquid10": ["BTCUSDT", "ETHUSDT", "BNBUSDT", "XRPUSDT", "SOLUSDT", "ADAUSDT", "DOGEUSDT", "LINKUSDT", "AVAXUSDT", "LTCUSDT"],
}
...
ENTRY_OVERRIDE_VALUES = [False, True]
ENTRY_RET5_THRESHOLDS = [40.0, 60.0, 80.0]
ENTRY_XS20_THRESHOLDS = [0.0, 20.0]
ENTRY_CONF_THRESHOLDS = [25.0, 30.0]

OUT_RESULTS_CSV = OUTPUT_DIR / "phase41_entry_override_universe_scan_results.csv"
OUT_JSON = OUTPUT_DIR / "phase41_entry_override_universe_scan_best.json"
...
def select_daily_top1(
    ...
    entry_override: bool,
    entry_ret5_threshold: float,
    entry_xs20_threshold: float,
    entry_conf_threshold: float,
) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        for symbol, tbl in asset_tables.items():
            ...
            override_ok = (
                entry_override
                and float(row["confidence"]) >= entry_conf_threshold
                and float(row["yz_vol_20"]) <= YZ_CAP
                and float(row["atr_pct"]) <= 70.0
                and float(row["thrust_ret5"]) >= entry_ret5_threshold
                and float(row["xs20"]) >= entry_xs20_threshold
            )

            if normal_ok or override_ok:
                candidates.append((symbol, candidate_score(row), int(normal_ok), int(override_ok)))
        ...
        if candidates:
            normal_candidate_count = sum(x[2] for x in candidates)
            override_candidate_count = sum(x[3] for x in candidates)
...
def main() -> None:
    ...
    for universe_name, symbols in UNIVERSE_VARIANTS.items():
        print(f"počítam tables pre universe={universe_name} ...", flush=True)
        daily_tables = build_daily_tables(all_assets, macro, symbols)

        for entry_override, entry_ret5_threshold, entry_xs20_threshold, entry_conf_threshold in product(
            ENTRY_OVERRIDE_VALUES,
            ENTRY_RET5_THRESHOLDS,
            ENTRY_XS20_THRESHOLDS,
            ENTRY_CONF_THRESHOLDS,
        ):
            if not entry_override:
                # stačí jeden baseline bez override
                if not (
                    entry_ret5_threshold == ENTRY_RET5_THRESHOLDS[0]
                    and entry_xs20_threshold == ENTRY_XS20_THRESHOLDS[0]
                    and entry_conf_threshold == ENTRY_CONF_THRESHOLDS[0]
                ):
                    continue

            variant = (
                f"{universe_name}"
                f"__ovr{int(entry_override)}"
                f"__r5{int(entry_ret5_threshold)}"
                f"__xs{int(entry_xs20_threshold)}"
                f"__cf{int(entry_conf_threshold)}"
            )
            print(f"testujem {variant} ...", flush=True)

            daily_selection = select_daily_top1(
                asset_tables=daily_tables,
                entry_override=entry_override,
                entry_ret5_threshold=entry_ret5_threshold,
                entry_xs20_threshold=entry_xs20_threshold,
                entry_conf_threshold=entry_conf_threshold,
            )
            daily_selection.to_csv(OUTPUT_DIR / f"phase41_{variant}_selection.csv")

            paper = run_daily_model(daily_selection, daily_tables)
            paper.to_csv(OUTPUT_DIR / f"phase41_{variant}_paper.csv")
    ...
    print("\n=== PHASE 41 ENTRY OVERRIDE + UNIVERSE SCAN ===")
