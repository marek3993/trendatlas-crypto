...
OUT_MAIN_RESULTS = OUTPUT_DIR / "phase44a_coin_pruning_false_start_results.csv"
OUT_LEADER_ATTR = OUTPUT_DIR / "phase44a_leader_attribution.csv"
OUT_FALSE_STARTS = OUTPUT_DIR / "phase44a_false_starts.csv"
OUT_JSON = OUTPUT_DIR / "phase44a_coin_pruning_false_start_best.json"
...
def build_daily_tables(
    ...
    btc20 = ret20["BTCUSDT"] if "BTCUSDT" in ret20.columns else ret20.mean(axis=1)
    btc90 = ret90["BTCUSDT"] if "BTCUSDT" in ret90.columns else ret90.mean(axis=1)
...
def build_false_starts(paper: pd.DataFrame) -> pd.DataFrame:
    entries = []
    prev_sel = "CASH"
    idx = list(paper.index)

    for i, ts in enumerate(idx):
        sel = str(paper.loc[ts, "selected"])
        if prev_sel == "CASH" and sel != "CASH":
            future = paper.iloc[i:min(i + 10, len(paper))].copy()
            entries.append(
                {
                    "entry_ts": ts,
                    "selected": sel,
                    "ret_3d_pct": round(float((1.0 + future["strategy_ret"].iloc[:3]).prod() - 1.0) * 100.0, 2) if len(future) >= 3 else np.nan,
                    "ret_5d_pct": round(float((1.0 + future["strategy_ret"].iloc[:5]).prod() - 1.0) * 100.0, 2) if len(future) >= 5 else np.nan,
                    "ret_10d_pct": round(float((1.0 + future["strategy_ret"]).prod() - 1.0) * 100.0, 2),
                    "false_start_5d": int(len(future) >= 5 and ((1.0 + future["strategy_ret"].iloc[:5]).prod() - 1.0) < 0),
                    "false_start_10d": int(((1.0 + future["strategy_ret"]).prod() - 1.0) < 0),
                }
            )
        prev_sel = sel
...
def build_leader_attribution(paper: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for symbol, g in paper[paper["selected"] != "CASH"].groupby("selected"):
        rows.append(
            {
                "symbol": symbol,
                "times_selected": int(len(g)),
                "avg_selected_ret_next_pct": round(float(g["selected_ret_next"].mean()) * 100.0, 4),
                "sum_selected_ret_next": round(float(g["selected_ret_next"].sum()), 4),
                "sum_leader_gap_ret": round(float(g["leader_gap_ret"].sum()), 4),
                "pain_bars": int((g["leader_gap_ret"] > 0.02).sum()),
            }
        )
    out = pd.DataFrame(rows)
    if not out.empty:
        out = out.sort_values(["sum_leader_gap_ret", "pain_bars"], ascending=[False, False]).reset_index(drop=True)
    return out


def evaluate_universe(all_assets: dict[str, pd.DataFrame], macro: pd.DataFrame, symbols: list[str], label: str) -> dict:
    ...
    score = (
        12.0 * summary["sharpe"]
        + 0.20 * summary["cagr_pct"]
        - 0.60 * abs(summary["max_drawdown_pct"])
        - 0.03 * summary["cash_days_pct"]
        - 0.04 * summary["missed_leader_pct_of_in_market"]
        - 0.03 * summary["pain_bar_pct_of_in_market"]
        - 0.50 * summary["sum_leader_gap_ret"]
    )
    return {
        "label": label,
...
def main() -> None:
    ...
    all_assets = {s: load_ohlcv_csv(DAILY_DIR / f"{s}_1d.csv") for s in TARGET_SYMBOLS}
    ...
    print("testujem full universe...", flush=True)
    base = evaluate_universe(all_assets, macro, TARGET_SYMBOLS, "full12")
    base_paper = base.pop("paper")
    rows.append(base)

    for symbol in TARGET_SYMBOLS:
        symbols = [s for s in TARGET_SYMBOLS if s != symbol]
        print(f"testujem without_{symbol} ...", flush=True)
        res = evaluate_universe(all_assets, macro, symbols, f"without_{symbol}")
        res.pop("paper")
        rows.append(res)

    out = pd.DataFrame(rows).sort_values(["score", "sharpe", "cagr_pct"], ascending=False).reset_index(drop=True)
    out.to_csv(OUT_MAIN_RESULTS, index=False)

    false_starts = build_false_starts(base_paper)
    false_starts.to_csv(OUT_FALSE_STARTS, index=False)

    leader_attr = build_leader_attribution(base_paper)
    leader_attr.to_csv(OUT_LEADER_ATTR, index=False)

    best = out.iloc[0].to_dict()
    with open(OUT_JSON, "w", encoding="utf-8") as f:
        json.dump(best, f, indent=2)

    print("\n=== PHASE 44A COIN PRUNING + FALSE START AUDIT ===")
    print(out.to_string(index=False))

    print("\n=== LEADER ATTRIBUTION ===")
    if leader_attr.empty:
        print("No leader attribution rows.")
    else:
        print(leader_attr.to_string(index=False))

    print("\n=== FALSE STARTS SUMMARY ===")
    if false_starts.empty:
        print("No entry events.")
    else:
        print(false_starts.groupby("selected").agg(
            entries=("selected", "count"),
            false_start_5d=("false_start_5d", "sum"),
            false_start_10d=("false_start_10d", "sum"),
            avg_ret_5d_pct=("ret_5d_pct", "mean"),
            avg_ret_10d_pct=("ret_10d_pct", "mean"),
        ).reset_index().to_string(index=False))

    print("\nuložené:")
    print(OUT_MAIN_RESULTS)
    print(OUT_LEADER_ATTR)
    print(OUT_FALSE_STARTS)
    print(OUT_JSON)
