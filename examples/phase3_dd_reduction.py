...
def main() -> None:
    ...
    grid = list(product(
        [0.0, 10.0, 20.0],     # min_lt_score_long
        [-10.0, 0.0],          # min_st_score_long
        [0.50, 0.70, 0.90],    # max_yz_vol_entry
        [70.0, 85.0],          # max_atr_pct_entry
        [False, True],         # exit_on_transition
    ))

    rows = []

    for idx, (min_lt_score_long, min_st_score_long, max_yz_vol_entry, max_atr_pct_entry, exit_on_transition) in enumerate(grid, start=1):
        params = StrategyParams(
            ...
            slippage_bps=5.0,
            exit_on_chaos=True,
            min_lt_score_long=min_lt_score_long,
            min_st_score_long=min_st_score_long,
            max_yz_vol_entry=max_yz_vol_entry,
            max_atr_pct_entry=max_atr_pct_entry,
    ...
    out.to_csv(OUTPUT_DIR / "phase3_dd_reduction.csv", index=False)

    best = out.iloc[0].to_dict()
    with open(OUTPUT_DIR / "best_dd_reduction.json", "w", encoding="utf-8") as f:
        json.dump(best, f, indent=2)

    print("\nPHASE 3 DD REDUCTION")
    print(out.head(15).to_string(index=False))
    print("\nuložené:")
    print("outputs\\phase3_dd_reduction.csv")
    print("outputs\\best_dd_reduction.json")
