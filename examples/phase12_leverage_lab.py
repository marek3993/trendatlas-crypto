...
def run_top1_model(
    ...
    for dt in all_dates:
        ...
        if market_score >= market_score_floor and candidates:
            ...
            if leverage_mode == "1.0x":
                lev = 1.0
            elif leverage_mode == "1.25x":
                lev = 1.25
            elif leverage_mode == "1.5x":
                lev = 1.5
            elif leverage_mode == "dynamic":
                conf_scale = clamp(selected[0]["confidence"] / 70.0, 0.35, 1.0)
                vol_scale = clamp(1.0 - max(0.0, selected[0]["yz_vol_20"] - 0.4), 0.35, 1.0)
                regime_scale = 1.0 if selected[0]["general_score"] >= 20.0 else 0.75
                lev = clamp(1.0 + 0.6 * conf_scale * vol_scale * regime_scale, 1.0, 1.5)
            else:
                raise ValueError(f"Unknown leverage_mode: {leverage_mode}")

            target_weights[selected[0]["symbol"]] = lev
...
def compute_summary(paper: pd.DataFrame) -> dict:
    ...
    if len(rets) < 20:
        return {
            ...
            "avg_gross_exposure": float(paper["gross_exposure"].mean()) if "gross_exposure" in paper.columns else 0.0,
        }
    ...
    trade_count = int((paper["turnover"] > 0).sum())
    avg_gross_exposure = float(paper["gross_exposure"].mean())
    ...
    return {
        ...
        "avg_gross_exposure": round(avg_gross_exposure, 3),
    }
...
def main() -> None:
    ...
    for leverage_mode in ["1.0x", "1.25x", "1.5x", "dynamic"]:
        print(f"testujem {leverage_mode}...", flush=True)

        paper = run_top1_model(
            rs_tables,
            model=best_model,
        ...
        paper.to_csv(OUTPUT_DIR / f"phase12_{leverage_mode.replace('.', '_')}_paper.csv")

    out = pd.DataFrame(rows).sort_values(["sharpe", "cagr_pct"], ascending=False).reset_index(drop=True)
    out.to_csv(OUTPUT_DIR / "phase12_leverage_lab.csv", index=False)

    best = out.iloc[0].to_dict()
    with open(OUTPUT_DIR / "phase12_best_leverage.json", "w", encoding="utf-8") as f:
        json.dump(best, f, indent=2)

    print("\nPHASE 12 LEVERAGE LAB")
    print(out.to_string(index=False))
    print("\nuložené:")
    print("outputs\\phase12_leverage_lab.csv")
    print("outputs\\phase12_best_leverage.json")
