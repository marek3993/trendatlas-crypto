...
def main() -> None:
    ...
    for w, start in enumerate(starts, start=1):
        ...
        for model in ["base", "rs"]:
            paper = run_top1_model(rs_tables, model=model)
        ...
        test_paper_full = run_top1_model(rs_tables, model=best_model)
    ...
    wf.to_csv(OUTPUT_DIR / "phase9_walkforward_ab_windows.csv", index=False)
    ...
    oos.to_csv(OUTPUT_DIR / "phase9_walkforward_ab_oos.csv")
    ...
    with open(OUTPUT_DIR / "phase9_walkforward_ab_summary.json", "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2)

    print("\nPHASE 9 WALK-FORWARD A/B")
    print(pd.DataFrame([result]).to_string(index=False))
    print("\nWINDOWS")
    print(wf.to_string(index=False))
    print("\nuložené:")
    print("outputs\\phase9_walkforward_ab_windows.csv")
    print("outputs\\phase9_walkforward_ab_oos.csv")
    print("outputs\\phase9_walkforward_ab_summary.json")
