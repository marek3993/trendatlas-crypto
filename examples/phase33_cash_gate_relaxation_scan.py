...
MARKET_THRESHOLDS = [10.0, 5.0, 0.0]
CONF_MINS = [35.0, 30.0]
YZ_CAPS = [0.70, 0.80, 0.90]

OUT_RESULTS_CSV = OUTPUT_DIR / "phase33_cash_gate_relaxation_results.csv"
OUT_JSON = OUTPUT_DIR / "phase33_cash_gate_relaxation_best.json"
...
def main() -> None:
    ...
    rows = []
    for market_threshold, conf_min, yz_cap in product(MARKET_THRESHOLDS, CONF_MINS, YZ_CAPS):
        variant = f"m{market_threshold:.0f}_c{conf_min:.0f}_yz{yz_cap:.2f}".replace(".", "p")
        ...
        daily_selection.to_csv(OUTPUT_DIR / f"phase33_{variant}_selection.csv")
        ...
        aggr.to_csv(OUTPUT_DIR / f"phase33_{variant}_aggressive.csv")
        bal.to_csv(OUTPUT_DIR / f"phase33_{variant}_balanced_v2.csv")
    ...
    print("\n=== PHASE 33 CASH-GATE RELAXATION SCAN ===")
