...
OUT_RESULTS_CSV = OUTPUTS_DIR / "phase26b_selected_coin_override_noleak_results.csv"
OUT_EQUITY_CSV = OUTPUTS_DIR / "phase26b_selected_coin_override_noleak_equity_curves.csv"
OUT_JSON = OUTPUTS_DIR / "phase26b_selected_coin_override_noleak_best.json"
...
OVERRIDE_CANDIDATES = [
    {"model": "phase26b_lag1_gt_0", "mode": "lag_selected_1bar_gt_0"},
    {"model": "phase26b_lag1_gt_2pct", "mode": "lag_selected_1bar_gt_0p02"},
    {"model": "phase26b_lag2_gt_3pct", "mode": "lag_selected_2bar_gt_0p03"},
    {"model": "phase26b_lag3_gt_5pct", "mode": "lag_selected_3bar_gt_0p05"},
    {"model": "phase26b_lag_turnover_only", "mode": "lag_turnover_gt_0"},
    {"model": "phase26b_lag_turnover_and_lag1_gt_0", "mode": "lag_turnover_gt_0_and_lag_selected_1bar_gt_0"},
]
...
def build_lagged_override_flags(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()

    # Realized held-coin return from previous bar, available at current bar open/decision point
    out["lag_selected_1bar_ret"] = out["strategy_ret"].shift(1).fillna(0.0)

    def rolling_comp(series: pd.Series, n: int) -> pd.Series:
        return (1.0 + series.fillna(0.0)).rolling(n).apply(np.prod, raw=True) - 1.0

    out["lag_selected_2bar_ret"] = rolling_comp(out["strategy_ret"].shift(1), 2).fillna(0.0)
    out["lag_selected_3bar_ret"] = rolling_comp(out["strategy_ret"].shift(1), 3).fillna(0.0)

    out["lag_turnover"] = out["turnover"].shift(1).fillna(0.0)

    out["ov_lag_selected_1bar_gt_0"] = out["lag_selected_1bar_ret"] > 0.0
    out["ov_lag_selected_1bar_gt_0p02"] = out["lag_selected_1bar_ret"] > 0.02
    out["ov_lag_selected_2bar_gt_0p03"] = out["lag_selected_2bar_ret"] > 0.03
    out["ov_lag_selected_3bar_gt_0p05"] = out["lag_selected_3bar_ret"] > 0.05
    out["ov_lag_turnover_gt_0"] = out["lag_turnover"] > 0.0
    out["ov_lag_turnover_gt_0_and_lag_selected_1bar_gt_0"] = out["ov_lag_turnover_gt_0"] & out["ov_lag_selected_1bar_gt_0"]
...
def run_backtest(df: pd.DataFrame, kill_signal_daily: pd.Series, override_mode: str | None, model_name: str) -> pd.DataFrame:
    ...
    if override_mode is None:
        ...
        col_map = {
            "lag_selected_1bar_gt_0": "ov_lag_selected_1bar_gt_0",
            "lag_selected_1bar_gt_0p02": "ov_lag_selected_1bar_gt_0p02",
            "lag_selected_2bar_gt_0p03": "ov_lag_selected_2bar_gt_0p03",
            "lag_selected_3bar_gt_0p05": "ov_lag_selected_3bar_gt_0p05",
            "lag_turnover_gt_0": "ov_lag_turnover_gt_0",
            "lag_turnover_gt_0_and_lag_selected_1bar_gt_0": "ov_lag_turnover_gt_0_and_lag_selected_1bar_gt_0",
        }
        out["override_day"] = out[col_map[override_mode]].astype(bool)
...
def main() -> None:
    ...
    df = build_lagged_override_flags(df)
    ...
    balanced = run_backtest(df, kill_signal, None, "phase20_btc_3d_neg_k05")
    ...
    for cfg in OVERRIDE_CANDIDATES:
        ...
        bt[
            [
                ...
                "gross_exposure",
                "strategy_ret",
                "lag_selected_1bar_ret",
                "lag_selected_2bar_ret",
                "lag_selected_3bar_ret",
                "lag_turnover",
                "kill_day",
                "override_day",
                "overlay_multiplier",
    ...
    print("\n=== PHASE 26B SELECTED COIN OVERRIDE NO-LEAK RESULTS ===")
