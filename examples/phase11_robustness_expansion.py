...
def main() -> None:
    ...
    if len(all_dates) < 1080:
        raise RuntimeError("Málo dát na phase 11")
    ...
    for total_cost_bps in [10.0, 15.0, 20.0]:
        print(f"\n=== COST {total_cost_bps} bps ===", flush=True)
        ...
        for w, start in enumerate(starts, start=1):
            ...
            for model in ["base", "rs"]:
                paper = run_top1_model(rs_tables, model=model, total_cost_bps=total_cost_bps)
            ...
            test_paper_full = run_top1_model(rs_tables, model=best_model, total_cost_bps=total_cost_bps)
            ...
            print(f"cost {total_cost_bps} | window {w}/{len(starts)} hotovo", flush=True)

        wf = pd.DataFrame(window_rows)
        wf.to_csv(OUTPUT_DIR / f"phase11_cost_{int(total_cost_bps)}_windows.csv", index=False)
        ...
        oos.to_csv(OUTPUT_DIR / f"phase11_cost_{int(total_cost_bps)}_oos.csv")

        oos_summary = compute_summary(oos)
        chosen_counts = wf["chosen_model"].value_counts().to_dict()

        scenario_rows.append(
            {
                "cost_bps": total_cost_bps,
                "universe_size": len(TARGET_SYMBOLS),
                "base_chosen": int(chosen_counts.get("base", 0)),
                "rs_chosen": int(chosen_counts.get("rs", 0)),
    ...
    out = pd.DataFrame(scenario_rows).sort_values("cost_bps").reset_index(drop=True)
    out.to_csv(OUTPUT_DIR / "phase11_robustness_expansion.csv", index=False)

    with open(OUTPUT_DIR / "phase11_robustness_expansion.json", "w", encoding="utf-8") as f:
        json.dump(out.to_dict(orient="records"), f, indent=2)

    print("\nPHASE 11 ROBUSTNESS EXPANSION")
    print(out.to_string(index=False))
    print("\nuložené:")
    print("outputs\\phase11_robustness_expansion.csv")
    print("outputs\\phase11_robustness_expansion.json")
    print("outputs\\phase11_cost_10_windows.csv")
    print("outputs\\phase11_cost_15_windows.csv")
    print("outputs\\phase11_cost_20_windows.csv")
