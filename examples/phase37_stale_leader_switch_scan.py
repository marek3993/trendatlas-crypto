...
# current winner base config from phase34
ACCEL_WEIGHT = 0.25
...
SWITCH_GAP_VALUES = [0.0, 5.0, 10.0, 15.0]
STALE_PENALTY_VALUES = [0.0, 5.0, 10.0]

OUT_RESULTS_CSV = OUTPUT_DIR / "phase37_stale_leader_switch_scan_results.csv"
OUT_JSON = OUTPUT_DIR / "phase37_stale_leader_switch_scan_best.json"
...
def is_stale_leader(row: pd.Series) -> bool:
    return (
        float(row["xs20"]) < 20.0
        and float(row["xs_accel"]) < 0.0
        and float(row["directional_bias"]) < 25.0
    )


def compute_candidate_score(row: pd.Series, stale_penalty: float, is_current: bool) -> float:
    score = (
        float(row["base_rank"])
        + 0.35 * float(row["xs20"])
        + 0.20 * float(row["xs90"])
    ...
    if is_current and stale_penalty > 0.0 and is_stale_leader(row):
        score -= stale_penalty

    return score


def select_daily_top1_with_switch_logic(
    asset_tables: dict[str, pd.DataFrame],
    switch_gap_threshold: float,
    stale_penalty: float,
) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        for symbol, tbl in asset_tables.items():
            ...
            if (
                ...
                score = compute_candidate_score(
                    ...
                    is_current=(symbol == prev_selected),
                )
        ...
        if market_score >= MARKET_THRESHOLD and candidates:
            ...
            if prev_selected != "CASH":
                for s, sc in candidates:
                    if s == prev_selected:
                        prev_score = sc
                        break

            if prev_selected != "CASH" and prev_score > -1e17:
                if (best_symbol != prev_selected) and ((best_score - prev_score) < switch_gap_threshold):
        ...
        rows.append(
            {
                ...
                "best_symbol_raw": best_symbol,
                "best_score_raw": best_score if best_score > -1e17 else np.nan,
                "prev_selected": prev_selected,
                "prev_score_raw": prev_score if prev_score > -1e17 else np.nan,
            }
        )
...
def run_daily_model(daily_selection: pd.DataFrame, daily_tables: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        if selected == "CASH":
            ...
            best_available_next = float(
                max(
                    daily_tables[s].loc[dt, "ret_next_1d"]
                    for s in daily_tables.keys()
                    if dt in daily_tables[s].index
                )
            )
...
def main() -> None:
    ...
    rows = []
    for switch_gap_threshold, stale_penalty in product(SWITCH_GAP_VALUES, STALE_PENALTY_VALUES):
        variant = f"gap{switch_gap_threshold:.0f}_pen{stale_penalty:.0f}"
        print(f"testujem {variant} ...", flush=True)

        daily_selection = select_daily_top1_with_switch_logic(
            asset_tables=daily_tables,
            switch_gap_threshold=switch_gap_threshold,
            stale_penalty=stale_penalty,
        )
        daily_selection.to_csv(OUTPUT_DIR / f"phase37_{variant}_selection.csv")

        paper = run_daily_model(daily_selection, daily_tables)
        paper.to_csv(OUTPUT_DIR / f"phase37_{variant}_paper.csv")
    ...
    print("\n=== PHASE 37 STALE LEADER SWITCH SCAN ===")
