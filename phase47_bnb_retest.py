...
OUT_DIR = OUTPUTS / "phase47_bnb_retest"
...
NO_BNB_SYMBOLS = [s for s in ALL_SYMBOLS if s != "BNBUSDT"]
...
BNB_SCORE_GAP_MIN = 8.0
BNB_LT_MIN = 45.0
BNB_ST_MIN = 35.0
BNB_XS20_MIN = 20.0

MODEL_LABELS = {
    "phase47_main_current": "Current main (No BNB)",
    "phase47_bnb_full_back": "BNB back fully",
    "phase47_bnb_gap_filter": "BNB back with score-gap filter",
    "phase47_bnb_trend_filter": "BNB back with trend filter",
}
...
def select_daily_top1_variant(asset_tables: dict[str, pd.DataFrame], variant: str) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        if market_score >= MARKET_THRESHOLD and candidates:
            ...
            if variant == "current_main":
            ...
            elif variant == "bnb_full_back":
                selected = candidates[0]["symbol"]

            elif variant == "bnb_gap_filter":
                top = candidates[0]
                if top["symbol"] == "BNBUSDT":
                    ...
                    if next_non_bnb is None:
                        ...
                        if (top["score"] - next_non_bnb["score"]) >= BNB_SCORE_GAP_MIN:
                            selected = "BNBUSDT"
                        else:
                            selected = next_non_bnb["symbol"]
                else:
                    selected = top["symbol"]

            elif variant == "bnb_trend_filter":
                top = candidates[0]
                if top["symbol"] == "BNBUSDT":
                    if (
                        top["lt_score"] >= BNB_LT_MIN
                        and top["st_score"] >= BNB_ST_MIN
                        and top["xs20"] >= BNB_XS20_MIN
                    ):
                        selected = "BNBUSDT"
                    else:
                        next_non_bnb = next((c for c in candidates if c["symbol"] != "BNBUSDT"), None)
...
def main() -> None:
    ...
    variants = [
        ("phase47_main_current", "current_main", daily_full),
        ("phase47_bnb_full_back", "bnb_full_back", daily_full),
        ("phase47_bnb_gap_filter", "bnb_gap_filter", daily_full),
        ("phase47_bnb_trend_filter", "bnb_trend_filter", daily_full),
    ]

    for model_key, variant_key, daily_tables in variants:
        print(f"Running {model_key}...", flush=True)
        selection = select_daily_top1_variant(daily_tables, variant_key)
    ...
    for model_key, df in model_runs.items():
        full_s = compute_summary(df)
        recent_s = compute_recent_summary(df)
        row = {
            "model": model_key,
    ...
    summary_df = pd.DataFrame(summary_rows).sort_values(
        ["recent_total_return_pct", "recent_cagr_pct", "recent_max_drawdown_pct"],
        ascending=[False, False, False],
    )
    summary_df.to_csv(OUT_DIR / "phase47_bnb_retest_summary.csv", index=False)
    ...
    eq = eq.reset_index()
    eq.to_csv(OUT_DIR / "phase47_bnb_retest_equity_curves.csv", index=False)

    print("\n=== PHASE47 BNB RETEST SUMMARY ===\n")
    print(summary_df.to_string(index=False))
    print(f"\nSaved summary: {OUT_DIR / 'phase47_bnb_retest_summary.csv'}")
    print(f"Saved equity curves: {OUT_DIR / 'phase47_bnb_retest_equity_curves.csv'}")
