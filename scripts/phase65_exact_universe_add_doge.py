...
PHASE65_DIR = OUTPUTS / "phase65_exact_universe_add_doge"
...
def build_variants() -> list[VariantConfig]:
    ...
    for v in [
        VariantConfig(
            name="phase65_doge_exact_default",
            score_lb=30,
            doge_fast_ma=20,
            doge_slow_ma=100,
            doge_min_score=0.12,
        ...
        VariantConfig(
            name="phase65_doge_exact_looser",
            score_lb=20,
            doge_fast_ma=20,
            doge_slow_ma=100,
            doge_min_score=0.08,
            doge_edge_vs_base=0.00,
            doge_risk_ma=150,
            doge_risk_buffer=-0.03,
            doge_vol_lb=30,
            doge_vol_cap=0.060,
        ...
        VariantConfig(
            name="phase65_doge_exact_stricter",
            score_lb=45,
            doge_fast_ma=30,
            doge_slow_ma=150,
            doge_min_score=0.16,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE65 exact universe DOGE challenger test")
    ...
    ensure_dir(PHASE65_DIR)

    log("[PHASE65] Start")
    log(f"[PHASE65] Baseline paper: {args.baseline_paper}")
    ...
    log(f"[PHASE65] DOGE file: {doge_file}")
    doge_daily, doge_quality = load_asset_daily_prices(doge_file)

    variants = build_variants()
    log(f"[PHASE65] Variants: {len(variants)}")
    ...
    for i, cfg in enumerate(variants, start=1):
        try:
            ...
            row["doge_days_pct"] = float(sim["doge_execute"].fillna(False).mean() * 100.0)
            ...
            row["source"] = "phase65_exact_doge_challenger"

            rows.append(row)
            papers_for_top[cfg.name] = sim

            if i % 25 == 0:
                log(f"[PHASE65] done {i}/{len(variants)}")
    ...
    top_compare = pd.concat([baseline_df, cand_df.head(20)], ignore_index=True)

    summary_path = PHASE65_DIR / "phase65_exact_universe_add_doge_summary.csv"
    compare_path = PHASE65_DIR / "phase65_exact_universe_add_doge_compare.csv"
    manifest_path = PHASE65_DIR / "phase65_manifest.json"
    ...
    manifest = {
        ...
        "notes": [
            "DOGE tu súťaží proti BASE dňom ako plnohodnotný challenger.",
            "Nie je to weak-base rescue override z predchádzajúcich phase.",
            "Nie je to 100% native rerun pôvodného celého ranking enginu, ale je to najbližší exact challenger test nad current winner paperom.",
    ...
    log("")
    log("=== PHASE65 TOP RESULT ===")
    if best is None:
        log("No valid DOGE variants processed.")
    else:
    ...
    log(f"[PHASE65] Saved summary -> {summary_path}")
    log(f"[PHASE65] Saved compare -> {compare_path}")
    log(f"[PHASE65] Saved manifest -> {manifest_path}")
