...
PHASE66H_DIR = OUTPUTS / "phase66h_core_regime_guardrail_sweep"


@dataclass(frozen=True)
class OverlaySweepVariant:
    model: str
    weak_base_lb: int
    weak_base_threshold_bump: float
    cooldown_days: int
    candidate_risk_buffer_bump: float
    candidate_ret_min_bump: float


VARIANTS: list[OverlaySweepVariant] = [
    OverlaySweepVariant(
        model="phase66h_core_guardrail_A",
        weak_base_lb=7,
        weak_base_threshold_bump=0.10,
        cooldown_days=7,
        candidate_risk_buffer_bump=0.00,
        candidate_ret_min_bump=0.00,
    ),
    OverlaySweepVariant(
        model="phase66h_core_guardrail_B",
        weak_base_lb=7,
        weak_base_threshold_bump=0.10,
        cooldown_days=10,
        candidate_risk_buffer_bump=0.00,
    ...
    OverlaySweepVariant(
        model="phase66h_core_guardrail_C",
        weak_base_lb=7,
        weak_base_threshold_bump=0.05,
        cooldown_days=7,
        candidate_risk_buffer_bump=0.10,
        candidate_ret_min_bump=0.00,
    ),
    OverlaySweepVariant(
        model="phase66h_core_guardrail_D",
        weak_base_lb=7,
        weak_base_threshold_bump=0.05,
        cooldown_days=7,
        candidate_risk_buffer_bump=0.10,
        candidate_ret_min_bump=0.05,
    ),
    OverlaySweepVariant(
        model="phase66h_core_guardrail_E",
        weak_base_lb=10,
        weak_base_threshold_bump=0.10,
        cooldown_days=10,
        candidate_risk_buffer_bump=0.10,
    ...
    OverlaySweepVariant(
        model="phase66h_core_guardrail_F",
        weak_base_lb=10,
        weak_base_threshold_bump=0.00,
        cooldown_days=7,
        candidate_risk_buffer_bump=0.05,
        candidate_ret_min_bump=0.05,
    ),
]

SUCCESS_RULE = {
    "min_since2025_cagr_pct": 113.90,
...
def build_overlay_config(base_cfg: object, variant: OverlaySweepVariant) -> object:
    ...
    cfg.weak_base_lb = int(variant.weak_base_lb)
    cfg.cooldown_days = int(variant.cooldown_days)
    cfg.weak_base_threshold = float(getattr(base_cfg, "weak_base_threshold")) + float(variant.weak_base_threshold_bump)
    cfg.candidate_risk_buffer = float(getattr(base_cfg, "candidate_risk_buffer")) + float(variant.candidate_risk_buffer_bump)
    cfg.candidate_ret_min = float(getattr(base_cfg, "candidate_ret_min")) + float(variant.candidate_ret_min_bump)

    return cfg


def load_asset_cache(best_files: dict[str, Path], remove_assets: set[str], min_history_days: int) -> tuple[dict[str, pd.DataFrame], list[dict], list[dict]]:
...
def run_variant(
    ...
    live_status_row = {
        ...
        "suspended_assets_now": int(
            len(leaderboard_out[(leaderboard_out["decision_date"].astype(str) == latest_decision_date) & (leaderboard_out["suspended"] == True)])
        ) if (not leaderboard_out.empty and "decision_date" in leaderboard_out.columns and "suspended" in leaderboard_out.columns and latest_decision_date) else 0,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE66H core regime guardrail sweep over phase66e overlay config")
    ...
    log("[PHASE66H] Start")
    log(f"[PHASE66H] Baseline paper: {args.baseline_paper}")
    log(f"[PHASE66H] Phase66G summary ref: {args.phase66g_summary}")
    log(f"[PHASE66H] Remove assets: {remove_assets}")
    ...
    log(f"[PHASE66H] Candidate assets cached: {len(asset_daily_cache)}")
    ...
    for variant in VARIANTS:
        log(
            f"[PHASE66H] running {variant.model} | "
            f"weak_base_lb={variant.weak_base_lb} "
            f"weak_base_threshold_bump={variant.weak_base_threshold_bump:+.2f} "
            f"cooldown_days={variant.cooldown_days} "
            f"candidate_risk_buffer_bump={variant.candidate_risk_buffer_bump:+.2f} "
            f"candidate_ret_min_bump={variant.candidate_ret_min_bump:+.2f}"
        )
    ...
    if top_model and not leaderboard_combined.empty:
        ...
        if not board.empty:
            ...
            if sort_cols:
                ...
                board = board.sort_values(by=sort_cols, ascending=asc, na_position="last").reset_index(drop=True)
        ...
        if not board.empty and "suspended" in board.columns:
            ...
            if sort_cols:
                asc = []
                for c in sort_cols:
                    asc.append(False if c == "suspended_until_rebalance_idx" else True)
                suspended_assets_now = suspended_assets_now.sort_values(by=sort_cols, ascending=asc, na_position="last").reset_index(drop=True)
    ...
    summary_path = PHASE66H_DIR / "phase66h_core_guardrail_summary.csv"
    compare_path = PHASE66H_DIR / "phase66h_core_guardrail_compare.csv"
    live_status_path = PHASE66H_DIR / "phase66h_live_status.csv"
    decisions_path = PHASE66H_DIR / "phase66h_core_guardrail_decisions.csv"
    leaderboard_path = PHASE66H_DIR / "phase66h_core_guardrail_leaderboard.csv"
    latest_top10_path = PHASE66H_DIR / "phase66h_latest_decision_top10.csv"
    suspended_now_path = PHASE66H_DIR / "phase66h_suspended_assets_now.csv"
    asset_quality_path = PHASE66H_DIR / "phase66h_core_guardrail_asset_quality.csv"
    asset_usage_path = PHASE66H_DIR / "phase66h_core_guardrail_asset_usage.csv"
    failed_assets_path = PHASE66H_DIR / "phase66h_core_guardrail_failed_assets.csv"
    baseline_copy_path = PHASE66H_DIR / f"{CURRENT_WINNER_KEY}_paper.csv"
    manifest_path = PHASE66H_DIR / "phase66h_manifest.json"
    ...
    manifest = {
        "phase": "phase66h_core_regime_guardrail_sweep",
        ...
        "notes": [
            "Phase66H nemení GovernanceConfig ani universe/drop logic.",
            "Sweepuje len OverlayConfig fieldy weak_base_lb, weak_base_threshold, cooldown_days, candidate_risk_buffer, candidate_ret_min.",
    ...
    log(f"[PHASE66H] Saved summary -> {summary_path}")
    log(f"[PHASE66H] Saved compare -> {compare_path}")
    log(f"[PHASE66H] Saved live status -> {live_status_path}")
    log(f"[PHASE66H] Saved decisions -> {decisions_path}")
    log(f"[PHASE66H] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE66H] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE66H] Saved suspended now -> {suspended_now_path}")
    log(f"[PHASE66H] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE66H] Saved asset usage -> {asset_usage_path}")
    log(f"[PHASE66H] Saved failed assets -> {failed_assets_path}")
    log(f"[PHASE66H] Saved baseline paper -> {baseline_copy_path}")
    log(f"[PHASE66H] Saved manifest -> {manifest_path}")
