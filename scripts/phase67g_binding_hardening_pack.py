...
PHASE67G_DIR = OUTPUTS / "phase67g_binding_hardening_pack"
    ...
    return [
        (
            ...
            WeeklyGovernanceConfig(
                profile_name="phase67g_pm002_pw2_cd14_dl42_bs001",
                recent_days=42,
                min_hold_periods=2,
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67g_pm002_pw2_cd7_dl84_bs001",
                recent_days=42,
                min_hold_periods=2,
                reentry_cooldown_days=7,
                downside_lookback_days=84,
                downside_max_worsen_pct=2.5,
                bnb_shield_margin_pct=1.0,
                **common_governance,
            ),
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67g_pm002_pw2_cd7_dl42_bs003",
                recent_days=42,
                min_hold_periods=2,
                reentry_cooldown_days=7,
                downside_lookback_days=42,
                downside_max_worsen_pct=2.5,
                bnb_shield_margin_pct=3.0,
                **common_governance,
            ),
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67g_pm002_pw2_cd14_dl84_bs003",
                recent_days=42,
                min_hold_periods=2,
                reentry_cooldown_days=14,
                downside_lookback_days=84,
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67g_pm002_pw2_cd14_dl42_bs001_hold3",
                recent_days=42,
                min_hold_periods=3,
                reentry_cooldown_days=14,
                downside_lookback_days=42,
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67g_pm002_pw2_cd7_dl42_bs001_dd20",
                recent_days=42,
                min_hold_periods=2,
                reentry_cooldown_days=7,
                downside_lookback_days=42,
                downside_max_worsen_pct=2.0,
                bnb_shield_margin_pct=1.0,
                **common_governance,
            ),
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE67G binding hardening pack over 66G core")
    ...
    log("[PHASE67G] Start")
    log(f"[PHASE67G] Core paper: {args.core_paper}")
    log(f"[PHASE67G] Shortlist assets: {[x['asset'] for x in shortlist]}")
    ...
    for overlay_cfg, gov_cfg in profiles:
        ...
        log(f"[PHASE67G] done {gov_cfg.profile_name}")
    ...
    summary_path = PHASE67G_DIR / "phase67g_binding_hardening_summary.csv"
    compare_path = PHASE67G_DIR / "phase67g_binding_hardening_compare.csv"
    decisions_path = PHASE67G_DIR / "phase67g_binding_hardening_decisions.csv"
    leaderboard_path = PHASE67G_DIR / "phase67g_binding_hardening_leaderboard.csv"
    asset_quality_path = PHASE67G_DIR / "phase67g_binding_hardening_asset_quality.csv"
    live_status_path = PHASE67G_DIR / "phase67g_live_status.csv"
    latest_top10_path = PHASE67G_DIR / "phase67g_latest_decision_top10.csv"
    manifest_path = PHASE67G_DIR / "phase67g_manifest.json"
    ...
    manifest = {
        ...
        "notes": [
            "67G je binding DD-hardening pack okolo 67F winnera.",
            "Zachováva pm2/pw2 a testuje tvrdšie cooldown/downside/BNB shield/min hold kombinácie.",
            "Cieľ: nezhoršiť since2025 a zároveň stlačiť DD bližšie k 66G.",
        ],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    log("")
    log("=== PHASE67G TOP RESULT ===")
    if best.empty:
        log("No phase67g profile processed.")
    else:
        row = best.iloc[0]
        ...
        log(f"downside_max_worsen_pct: {row['downside_max_worsen_pct']:.2f}")
        log(f"bnb_shield_margin_pct: {row['bnb_shield_margin_pct']:.2f}")
        log(f"min_hold_periods: {int(row['min_hold_periods'])}")
        log("")

    if not live_status.empty:
        ls = live_status.iloc[0]
        log("=== PHASE67G LIVE STATUS ===")
    ...
    log(f"[PHASE67G] Saved summary -> {summary_path}")
    log(f"[PHASE67G] Saved compare -> {compare_path}")
    log(f"[PHASE67G] Saved decisions -> {decisions_path}")
    log(f"[PHASE67G] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE67G] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE67G] Saved live status -> {live_status_path}")
    log(f"[PHASE67G] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE67G] Saved manifest -> {manifest_path}")
