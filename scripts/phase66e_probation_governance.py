...
PHASE66E_DIR = OUTPUTS / "phase66e_probation_governance"
...
def simulate_governance_strategy_probation(
    baseline: pd.DataFrame,
    asset_strategies: dict[str, pd.DataFrame],
    gov_cfg: GovernanceConfig,
    ...
    for i, decision_date in enumerate(rebalance_dates):
        ...
        eval_rows = []
        for asset in assets:
            meta = evaluate_asset_for_date(asset, decision_date, baseline, asset_strategies, gov_cfg)
            ...
            base_prob_total, _ = slice_metrics_from_returns(baseline.loc[probation_idx, "strategy_return"])
            probation_delta = asset_prob_total - base_prob_total
...
def build_profiles(min_history_days: int) -> list[GovernanceConfig]:
    return [
        GovernanceConfig(
            profile_name="phase66e_probation_base",
            ...
            probation_min_delta_pct=-1.0,
            probation_ban_periods=4,
        ),
        GovernanceConfig(
            profile_name="phase66e_probation_strict",
            trailing_train_days=365,
            recent_days=60,
            rebalance_every_days=7,
            min_history_days=min_history_days,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE66E probation weekly governance")
    ...
    log("[PHASE66E] Start")
    log(f"[PHASE66E] Baseline paper: {args.baseline_paper}")
    log(f"[PHASE66E] Remove assets: {remove_assets}")
    ...
    log(f"[PHASE66E] Candidate assets loaded: {len(asset_strategies)}")
    ...
    for gov_cfg in profiles:
        governance, decisions_df, leaderboard_df = simulate_governance_strategy_probation(
            baseline=baseline,
        ...
        log(f"[PHASE66E] done {gov_cfg.profile_name}")
    ...
    summary_path = PHASE66E_DIR / "phase66e_probation_governance_summary.csv"
    compare_path = PHASE66E_DIR / "phase66e_probation_governance_compare.csv"
    decisions_path = PHASE66E_DIR / "phase66e_probation_governance_decisions.csv"
    leaderboard_path = PHASE66E_DIR / "phase66e_probation_governance_leaderboard.csv"
    asset_quality_path = PHASE66E_DIR / "phase66e_probation_governance_asset_quality.csv"
    asset_usage_path = PHASE66E_DIR / "phase66e_probation_governance_asset_usage.csv"
    failed_assets_path = PHASE66E_DIR / "phase66e_probation_governance_failed_assets.csv"
    manifest_path = PHASE66E_DIR / "phase66e_manifest.json"
    ...
    manifest = {
        ...
        "notes": [
            ...
            "Asset dostane ban na pár rebalancov, keď jeho nedávna edge padne pod threshold.",
            "Future-safe asset retirement / return mechanizmus.",
        ],
    }
    ...
    log(f"[PHASE66E] Saved summary -> {summary_path}")
    log(f"[PHASE66E] Saved compare -> {compare_path}")
    log(f"[PHASE66E] Saved decisions -> {decisions_path}")
    log(f"[PHASE66E] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE66E] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE66E] Saved asset usage -> {asset_usage_path}")
    log(f"[PHASE66E] Saved failed assets -> {failed_assets_path}")
    log(f"[PHASE66E] Saved manifest -> {manifest_path}")
