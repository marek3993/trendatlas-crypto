...
PHASE66C_DIR = OUTPUTS / "phase66c_pruned_governance"
...
def choose_asset_for_date(
    ...
    choice_meta = {}
    if best_asset:
        choice_meta = leaderboard.loc[leaderboard["asset"] == best_asset].iloc[0].to_dict()
...
def build_profiles(min_history_days: int) -> list[GovernanceConfig]:
    return [
        GovernanceConfig(
            profile_name="phase66c_pruned_base",
        ...
        GovernanceConfig(
            profile_name="phase66c_pruned_strict",
            trailing_train_days=365,
            recent_days=60,
            rebalance_every_days=7,
            min_history_days=min_history_days,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE66C pruned weekly governance")
    ...
    log("[PHASE66C] Start")
    log(f"[PHASE66C] Baseline paper: {args.baseline_paper}")
    log(f"[PHASE66C] Remove assets: {remove_assets}")
    ...
    log(f"[PHASE66C] Candidate assets loaded: {len(asset_strategies)}")
    ...
    for gov_cfg in profiles:
        ...
        log(f"[PHASE66C] done {gov_cfg.profile_name}")
    ...
    summary_path = PHASE66C_DIR / "phase66c_pruned_governance_summary.csv"
    compare_path = PHASE66C_DIR / "phase66c_pruned_governance_compare.csv"
    decisions_path = PHASE66C_DIR / "phase66c_pruned_governance_decisions.csv"
    leaderboard_path = PHASE66C_DIR / "phase66c_pruned_governance_leaderboard.csv"
    asset_quality_path = PHASE66C_DIR / "phase66c_pruned_governance_asset_quality.csv"
    asset_usage_path = PHASE66C_DIR / "phase66c_pruned_governance_asset_usage.csv"
    failed_assets_path = PHASE66C_DIR / "phase66c_pruned_governance_failed_assets.csv"
    manifest_path = PHASE66C_DIR / "phase66c_manifest.json"
    ...
    manifest = {
        ...
        "notes": [
            "2 kroky naraz: pruning REMOVE assetov + 2 governance profily.",
    ...
    log(f"[PHASE66C] Saved summary -> {summary_path}")
    log(f"[PHASE66C] Saved compare -> {compare_path}")
    log(f"[PHASE66C] Saved decisions -> {decisions_path}")
    log(f"[PHASE66C] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE66C] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE66C] Saved asset usage -> {asset_usage_path}")
    log(f"[PHASE66C] Saved failed assets -> {failed_assets_path}")
    log(f"[PHASE66C] Saved manifest -> {manifest_path}")
