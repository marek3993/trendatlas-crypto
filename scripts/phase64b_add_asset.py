...
PHASE64B_DIR = OUTPUTS / "phase64b_add_asset"
...
def add_deltas(target: dict, base: dict) -> dict:
    ...
    for metric in [
        ...
        out[f"delta_vs_phase63_{metric}"] = pd.to_numeric(out.get(metric), errors="coerce") - pd.to_numeric(base.get(metric), errors="coerce")
    return out


def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE64B exact add-one rerun for one asset")
    parser.add_argument("--asset", type=str, default="DOGE")
    parser.add_argument("--baseline-paper", type=str, default=str(CURRENT_WINNER_PAPER))
    args = parser.parse_args()

    asset = normalize_asset_name(args.asset)
    out_dir = PHASE64B_DIR / asset.lower()
    ensure_dir(out_dir)

    cfg = AddAssetConfig()

    log("[PHASE64B] Start")
    log(f"[PHASE64B] Asset: {asset}")
    log(f"[PHASE64B] Baseline paper: {args.baseline_paper}")
    ...
    add_row["asset"] = asset
    add_row["asset_file"] = str(asset_file)
    add_row["candidate_days_pct"] = float(sim["candidate_execute"].fillna(False).mean() * 100.0)
    add_row["signal_days_pct"] = float(sim["candidate_signal"].fillna(False).mean() * 100.0)
    add_row["trigger_days"] = int(sim["candidate_signal_raw"].fillna(False).sum())
    add_row["raw_rows"] = quality["raw_rows"]
    add_row["daily_rows"] = quality["daily_rows"]
    add_row["history_days"] = quality["history_days"]
    add_row["data_start"] = quality["start_date"]
    add_row["data_end"] = quality["end_date"]
    add_row["non_na_close_ratio"] = quality["non_na_close_ratio"]
    add_row["max_gap_days"] = quality["max_gap_days"]

    summary = pd.DataFrame([baseline_row, add_row])

    summary_path = out_dir / f"phase64b_add_{asset.lower()}_summary.csv"
    compare_path = out_dir / f"phase64b_add_{asset.lower()}_compare.csv"
    baseline_paper_path = out_dir / f"{CURRENT_WINNER_KEY}_paper.csv"
    add_paper_path = out_dir / f"add_{asset}_paper.csv"
    manifest_path = out_dir / f"phase64b_add_{asset.lower()}_manifest.json"
    ...
    sim.reset_index().rename(columns={sim.index.name or "index": "date"}).to_csv(add_paper_path, index=False)

    manifest = {
        "phase": "phase64b_add_asset",
        "asset": asset,
        ...
        "add_asset_paper_saved": str(add_paper_path),
        "config": {
        ...
        "notes": [
            "Presný add-one rerun nad current Phase63 winner paperom.",
            "Nie broad scan.",
            "DOGE override sa aktivuje len keď baseline BASE vyzerá weak a DOGE je technicky strong.",
        ],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    log("")
    log("=== PHASE64B TOP RESULT ===")
    log(f"model: add_{asset}")
    log(f"cagr_pct: {add_row['cagr_pct']:.2f}")
    log(f"max_drawdown_pct: {add_row['max_drawdown_pct']:.2f}")
    log(f"since2023_cagr_pct: {add_row['since2023_cagr_pct']:.2f}")
    log(f"since2025_cagr_pct: {add_row['since2025_cagr_pct']:.2f}")
    log(f"delta_vs_phase63_cagr_pct: {add_row['delta_vs_phase63_cagr_pct']:.2f}")
    log(f"delta_vs_phase63_since2023_cagr_pct: {add_row['delta_vs_phase63_since2023_cagr_pct']:.2f}")
    log(f"delta_vs_phase63_since2025_cagr_pct: {add_row['delta_vs_phase63_since2025_cagr_pct']:.2f}")
    log(f"delta_vs_phase63_max_drawdown_pct: {add_row['delta_vs_phase63_max_drawdown_pct']:.2f}")
    log(f"candidate_days_pct: {add_row['candidate_days_pct']:.2f}")
    log(f"signal_days_pct: {add_row['signal_days_pct']:.2f}")
    log(f"trigger_days: {int(add_row['trigger_days'])}")
    log("")
    log(f"[PHASE64B] Saved summary -> {summary_path}")
    log(f"[PHASE64B] Saved compare -> {compare_path}")
    log(f"[PHASE64B] Saved baseline paper -> {baseline_paper_path}")
    log(f"[PHASE64B] Saved add paper -> {add_paper_path}")
    log(f"[PHASE64B] Saved manifest -> {manifest_path}")
