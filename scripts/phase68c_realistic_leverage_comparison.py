...
PHASE68C_DIR = OUTPUTS / "phase68c_realistic_leverage_comparison"
...
VARIANTS: list[RealisticVariant] = [
    RealisticVariant(
        model="phase68c_66g_1p00x_realistic_baseline",
        target_leverage=1.00,
    ),
    RealisticVariant(
        model="phase68c_realistic_1p25x_candidate",
        target_leverage=1.25,
    ),
    RealisticVariant(
        model="phase68c_realistic_1p50x_candidate",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE68C realistic leverage comparison over current 66G baseline")
    ...
    ensure_dir(PHASE68C_DIR)
    papers_dir = PHASE68C_DIR / "papers"
    ...
    log("[PHASE68C] Start")
    log(f"[PHASE68C] Input paper: {paper_path}")
    log(f"[PHASE68C] Trend history: {trend_path}")
    log(f"[PHASE68C] Annual borrow cost: {args.annual_borrow_cost:.4f}")
    log(f"[PHASE68C] Trend activation threshold: {args.trend_activation_threshold:.4f}")
    log(f"[PHASE68C] Stress off / on: {args.stress_off_threshold:.4f} / {args.stress_on_threshold:.4f}")
    ...
    for variant in VARIANTS:
        log(f"[PHASE68C] running {variant.model} | target_leverage={variant.target_leverage:.2f}x")
        ...
        log(f"[PHASE68C] done {variant.model}")

    summary_df = pd.DataFrame(summary_rows)

    baseline_row = summary_df.loc[summary_df["model"] == "phase68c_66g_1p00x_realistic_baseline"]
    ...
    summary_path = PHASE68C_DIR / "phase68c_realistic_leverage_summary.csv"
    compare_path = PHASE68C_DIR / "phase68c_realistic_leverage_compare.csv"
    manifest_path = PHASE68C_DIR / "phase68c_realistic_leverage_manifest.json"

    summary_df.to_csv(summary_path, index=False)
    compare_df.to_csv(compare_path, index=False)

    manifest = {
        "phase": "phase68c_realistic_leverage_comparison",
        "official_compare_baseline": "phase68c_66g_1p00x_realistic_baseline",
        ...
        "notes": [
            "Phase68C porovnáva realistic 1.25x a 1.50x pod rovnakými guardrailmi ako 68B.",
            "Cash = vždy 1.00x.",
            "Switch day = 1.00x.",
            ...
            "Official compare baseline je phase68c_66g_1p00x_realistic_baseline.",
        ],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    log("")
    log("=== PHASE68C TOP RESULT ===")
    ...
    log(f"[PHASE68C] Saved summary -> {summary_path}")
    log(f"[PHASE68C] Saved compare -> {compare_path}")
    log(f"[PHASE68C] Saved manifest -> {manifest_path}")
    log(f"[PHASE68C] Saved papers -> {papers_dir}")
