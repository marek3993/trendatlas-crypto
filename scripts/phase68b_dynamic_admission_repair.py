from __future__ import annotations

import argparse
import json
from dataclasses import dataclass, asdict
from pathlib import Path
from typing import Optional

import numpy as np
import pandas as pd

import phase66e_probation_governance as core


ROOT = Path(__file__).resolve().parents[1]
OUTPUTS = ROOT / "outputs"

CORE_MODEL_KEY = "phase66g_production_soft_filters"
CORE_PAPER = OUTPUTS / "phase66g_production_candidate_live" / f"{CORE_MODEL_KEY}_paper.csv"

PHASE63_MODEL_KEY = core.CURRENT_WINNER_KEY
PHASE63_PAPER = core.CURRENT_WINNER_PAPER

PHASE67B_SHORTLIST = OUTPUTS / "phase67b_top100_forensic_prune_and_rerun" / "phase67b_asset_shortlist.csv"
PHASE67_TOP100_MAPPING = OUTPUTS / "phase67_top100_build_and_governance" / "phase67_top100_mapping.csv"
PHASE67_TOP100_DOWNLOADS = OUTPUTS / "phase67_top100_build_and_governance" / "phase67_top100_downloads.csv"

PHASE68B_DIR = OUTPUTS / "phase68b_dynamic_admission_repair"

STABLE_EXCLUDE = {
    "USDT", "USDC", "DAI", "FDUSD", "TUSD", "USDP", "USDE", "USDD", "FRAX",
    "LUSD", "PYUSD", "GUSD", "BUSD", "EURC", "EURS", "USDX", "USD0", "AEUR",
    "PAX", "UST", "USTC", "CUSD",
}
PROXY_EXCLUDE = {
    "PAXG", "XAUT",
}
EXACT_EXCLUDE = STABLE_EXCLUDE | PROXY_EXCLUDE


@dataclass
class ChallengerOverlayConfig:
    score_lb: int
    fast_ma: int
    slow_ma: int
    min_score: float
    edge_vs_core: float
    risk_ma: int
    risk_buffer: float
    vol_lb: int
    vol_cap: float
    rel_recent_lb: int
    rel_recent_min_edge: float
    cooldown_days: int


@dataclass
class DynamicUniverseConfig:
    profile_name: str

    trailing_train_days: int
    recent_days: int
    rebalance_every_days: int
    min_triggers_in_train: int
    min_total_delta_pct: float
    min_recent_delta_pct: float
    max_allowed_dd_worsen_pct: float
    switch_score_margin: float
    min_hold_periods: int

    promotion_margin_pct: float
    persistence_weeks: int
    reentry_cooldown_days: int
    downside_lookback_days: int
    downside_max_worsen_pct: float
    bnb_shield_margin_pct: float

    admission_train_days: int
    admission_recent_days: int
    admission_min_train_delta_pct: float
    admission_min_recent_delta_pct: float
    admission_max_dd_worsen_pct: float
    admission_min_trigger_days: int
    admission_min_history_days: int
    admission_min_quality_score: float

    probation_lookback_days: int
    probation_min_delta_pct: float
    probation_ban_periods: int

    demotion_train_delta_pct: float
    demotion_recent_delta_pct: float
    demotion_trigger_days: int
    probation_required_fail_weeks: int
    suspend_required_fail_weeks: int
    reapprove_pass_weeks: int

    max_approved_assets: int


def log(msg: str) -> None:
    print(msg, flush=True)


def ensure_dir(path: Path) -> None:
    path.mkdir(parents=True, exist_ok=True)


def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out.columns = [str(c).strip() for c in out.columns]
    return out


def detect_position_column(df: pd.DataFrame) -> Optional[str]:
    preferred = [
        "executed_position",
        "final_position",
        "signal_position",
        "position",
        "selected_symbol",
        "symbol",
        "asset",
    ]
    for col in preferred:
        if col in df.columns:
            return col
    for col in df.columns:
        lc = str(col).lower()
        if "position" in lc or "symbol" in lc or "asset" in lc:
            return col
    return None


def detect_asset_column(df: pd.DataFrame) -> Optional[str]:
    for col in ["asset", "symbol", "ticker", "coin", "base_symbol"]:
        if col in df.columns:
            return col
    return None


def detect_file_column(df: pd.DataFrame) -> Optional[str]:
    for col in ["file", "csv_path", "path", "local_file", "daily_file", "ohlcv_file"]:
        if col in df.columns:
            return col
    return None


def maybe_make_abs(path_str: str) -> Path:
    p = Path(str(path_str))
    if p.is_absolute():
        return p
    return (ROOT / p).resolve()


def asset_is_excluded(asset: str) -> bool:
    a = str(asset).upper().strip()
    if a in EXACT_EXCLUDE:
        return True
    if a.endswith("USD") or a.endswith("EUR"):
        if a in {"BCH", "NEAR"}:
            return False
    if "USD" in a and a in STABLE_EXCLUDE:
        return True
    return False


def load_strategy_paper(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Missing paper: {path}")

    raw = pd.read_csv(path)
    raw = normalize_columns(raw)

    if "date" not in raw.columns:
        raise ValueError(f"{path.name}: missing date column")

    raw["date"] = pd.to_datetime(raw["date"], errors="coerce")
    raw = raw.dropna(subset=["date"]).sort_values("date").drop_duplicates(subset=["date"], keep="last")
    raw = raw.set_index("date")

    for col in ["strategy_return", "equity"]:
        if col not in raw.columns:
            raise ValueError(f"{path.name}: missing {col}")

    regime_col = "executed_regime" if "executed_regime" in raw.columns else "final_regime"
    if regime_col not in raw.columns:
        raise ValueError(f"{path.name}: missing executed_regime/final_regime")

    pos_col = detect_position_column(raw)
    if pos_col is None:
        raise ValueError(f"{path.name}: missing position column")

    out = pd.DataFrame(index=raw.index.copy())
    out["strategy_return"] = pd.to_numeric(raw["strategy_return"], errors="coerce").fillna(0.0)
    out["equity"] = pd.to_numeric(raw["equity"], errors="coerce").ffill().bfill()
    out["executed_regime"] = raw[regime_col].astype(str).fillna("NA").str.upper()
    out["executed_position"] = raw[pos_col].astype(str).fillna("NA").str.upper().str.strip()
    return out


def load_local_daily_for_core(path: Path) -> tuple[pd.DataFrame, dict]:
    df = pd.read_csv(path)
    df = normalize_columns(df)
    if "date" not in df.columns or "close" not in df.columns:
        raise ValueError(f"{path.name}: missing date/close")

    df["date"] = pd.to_datetime(df["date"], errors="coerce")
    df["close"] = pd.to_numeric(df["close"], errors="coerce")
    df = (
        df.dropna(subset=["date", "close"])
        .sort_values("date")
        .drop_duplicates(subset=["date"], keep="last")
        .reset_index(drop=True)
    )

    daily = df[["date", "close"]].rename(columns={"date": "ts"}).copy()
    daily["day"] = pd.to_datetime(daily["ts"]).dt.normalize()
    daily = daily.groupby("day", as_index=True)["close"].last().to_frame("candidate_close").sort_index()

    q = {
        "history_days": int((daily.index.max() - daily.index.min()).days + 1) if len(daily) else 0,
        "start_date": daily.index.min().date().isoformat() if len(daily) else "",
        "end_date": daily.index.max().date().isoformat() if len(daily) else "",
        "daily_rows": int(len(daily)),
        "max_gap_days": 0,
        "non_na_close_ratio": float(daily["candidate_close"].notna().mean()) * 100.0 if len(daily) else 0.0,
    }
    if len(daily) >= 2:
        gaps = pd.Series(daily.index).diff().dt.days.dropna()
        q["max_gap_days"] = int(gaps.max()) if not gaps.empty else 0
    return daily, q


def annualize_return(total_return: float, n_days: int) -> float:
    if n_days <= 1:
        return 0.0
    years = n_days / 365.25
    if years <= 0:
        return 0.0
    if total_return <= -1:
        return -1.0
    return (1.0 + total_return) ** (1.0 / years) - 1.0


def max_drawdown_from_equity(equity: pd.Series) -> float:
    peak = equity.cummax()
    dd = equity / peak - 1.0
    return float(dd.min()) if len(dd) else 0.0


def sharpe_ratio(daily_ret: pd.Series) -> float:
    x = pd.to_numeric(daily_ret, errors="coerce").dropna()
    if len(x) < 2:
        return 0.0
    vol = x.std(ddof=0)
    if vol == 0 or np.isnan(vol):
        return 0.0
    return float((x.mean() / vol) * np.sqrt(365.25))


def sortino_ratio(daily_ret: pd.Series) -> float:
    x = pd.to_numeric(daily_ret, errors="coerce").dropna()
    if len(x) < 2:
        return 0.0
    downside = x[x < 0]
    if len(downside) == 0:
        return 0.0
    dd = downside.std(ddof=0)
    if dd == 0 or np.isnan(dd):
        return 0.0
    return float((x.mean() / dd) * np.sqrt(365.25))


def calc_metrics(df: pd.DataFrame, model_name: str) -> dict:
    x = df.copy()
    x["strategy_return"] = pd.to_numeric(x["strategy_return"], errors="coerce").fillna(0.0)
    x["equity"] = (1.0 + x["strategy_return"]).cumprod()

    total_return = float(x["equity"].iloc[-1] / x["equity"].iloc[0] - 1.0) if len(x) > 1 else 0.0
    cagr = annualize_return(total_return, len(x))
    max_dd = max_drawdown_from_equity(x["equity"])
    sharpe = sharpe_ratio(x["strategy_return"])
    sortino = sortino_ratio(x["strategy_return"])

    out = {
        "model": model_name,
        "days": int(len(x)),
        "total_return_pct": total_return * 100.0,
        "cagr_pct": cagr * 100.0,
        "max_drawdown_pct": max_dd * 100.0,
        "sharpe": sharpe,
        "sortino": sortino,
    }
    if "executed_regime" in x.columns:
        r = x["executed_regime"].astype(str).fillna("NA")
        out["cash_days_pct"] = float((r == "CASH").mean() * 100.0)
        out["btc_days_pct"] = float((r == "BTC").mean() * 100.0)
        out["base_days_pct"] = float((r == "BASE").mean() * 100.0)
        out["candidate_days_pct"] = float((r == "CANDIDATE").mean() * 100.0)
    if "executed_position" in x.columns:
        p = x["executed_position"].astype(str).fillna("NA")
        out["trade_count"] = int((p != p.shift(1)).sum() - 1) if len(p) else 0
    return out


def window_metrics(df: pd.DataFrame, start_date: str) -> dict:
    sub = df[df.index >= pd.Timestamp(start_date)].copy()
    if sub.empty:
        return {
            f"since{start_date[:4]}_total_return_pct": np.nan,
            f"since{start_date[:4]}_cagr_pct": np.nan,
            f"since{start_date[:4]}_max_drawdown_pct": np.nan,
        }
    m = calc_metrics(sub, f"since{start_date[:4]}")
    return {
        f"since{start_date[:4]}_total_return_pct": m["total_return_pct"],
        f"since{start_date[:4]}_cagr_pct": m["cagr_pct"],
        f"since{start_date[:4]}_max_drawdown_pct": m["max_drawdown_pct"],
    }


def rolling_compound_return(ret: pd.Series, lb: int) -> pd.Series:
    return (1.0 + ret).rolling(lb, min_periods=lb).apply(np.prod, raw=True) - 1.0


def align_candidate_to_core(core_df: pd.DataFrame, candidate_daily: pd.DataFrame) -> pd.DataFrame:
    x = core_df.copy()
    c = candidate_daily.copy().reindex(x.index)
    c["candidate_close"] = c["candidate_close"].ffill()
    x["candidate_close"] = c["candidate_close"]
    x["candidate_return"] = x["candidate_close"].pct_change().replace([np.inf, -np.inf], np.nan).fillna(0.0)
    return x


def compute_candidate_signal(df: pd.DataFrame, cfg: ChallengerOverlayConfig) -> pd.DataFrame:
    x = df.copy()

    x["core_score"] = rolling_compound_return(x["strategy_return"], cfg.score_lb)
    x["cand_score"] = x["candidate_close"].pct_change(cfg.score_lb)
    x["cand_recent_rel"] = x["candidate_close"].pct_change(cfg.rel_recent_lb) - x["equity"].pct_change(cfg.rel_recent_lb)

    x["cand_fast_ma"] = x["candidate_close"].rolling(cfg.fast_ma, min_periods=cfg.fast_ma).mean()
    x["cand_slow_ma"] = x["candidate_close"].rolling(cfg.slow_ma, min_periods=cfg.slow_ma).mean()
    x["cand_risk_ma"] = x["candidate_close"].rolling(cfg.risk_ma, min_periods=cfg.risk_ma).mean()
    x["cand_vol"] = x["candidate_return"].rolling(cfg.vol_lb, min_periods=cfg.vol_lb).std(ddof=0)

    x["cand_trend_ok"] = (
        (x["candidate_close"] > x["cand_fast_ma"])
        & (x["cand_fast_ma"] > x["cand_slow_ma"])
        & (x["cand_score"] >= cfg.min_score)
    )

    x["cand_risk_off"] = (
        (x["candidate_close"] < (x["cand_risk_ma"] * (1.0 + cfg.risk_buffer)))
        | (x["cand_vol"] > cfg.vol_cap)
    )

    x["cand_beats_core"] = x["cand_score"] >= (x["core_score"] + cfg.edge_vs_core)
    x["cand_recent_rel_ok"] = x["cand_recent_rel"] >= cfg.rel_recent_min_edge

    x["candidate_signal_raw"] = (
        x["cand_trend_ok"].fillna(False)
        & (~x["cand_risk_off"].fillna(True))
        & x["cand_beats_core"].fillna(False)
        & x["cand_recent_rel_ok"].fillna(False)
        & x["candidate_close"].notna()
    )

    raw = x["candidate_signal_raw"].fillna(False).astype(bool).values
    if cfg.cooldown_days > 0:
        locked = np.zeros(len(raw), dtype=bool)
        hold = 0
        for i, flag in enumerate(raw):
            if flag:
                hold = cfg.cooldown_days
            elif hold > 0:
                hold -= 1
            locked[i] = flag or hold > 0
        x["candidate_signal"] = locked
    else:
        x["candidate_signal"] = raw

    x["candidate_execute"] = pd.Series(x["candidate_signal"], index=x.index).shift(1, fill_value=False)
    return x


def build_asset_overlay_strategy(
    core_df: pd.DataFrame,
    candidate_daily: pd.DataFrame,
    cfg: ChallengerOverlayConfig,
    asset: str,
) -> pd.DataFrame:
    x = align_candidate_to_core(core_df, candidate_daily)
    x = compute_candidate_signal(x, cfg)

    out = x.copy()
    out["executed_regime"] = np.where(out["candidate_execute"], "CANDIDATE", out["executed_regime"])
    out["executed_position"] = np.where(out["candidate_execute"], asset, out["executed_position"])
    out["strategy_return"] = np.where(out["candidate_execute"], out["candidate_return"], out["strategy_return"])
    out["equity"] = (1.0 + pd.to_numeric(out["strategy_return"], errors="coerce").fillna(0.0)).cumprod()
    return out


def slice_metrics_from_returns(returns: pd.Series) -> tuple[float, float]:
    x = pd.to_numeric(returns, errors="coerce").fillna(0.0)
    if len(x) == 0:
        return np.nan, np.nan
    eq = (1.0 + x).cumprod()
    total = float(eq.iloc[-1] / eq.iloc[0] - 1.0) if len(eq) > 1 else 0.0
    dd = max_drawdown_from_equity(eq)
    return total * 100.0, dd * 100.0


def build_rebalance_dates(index: pd.DatetimeIndex, train_days: int, step_days: int) -> list[pd.Timestamp]:
    dates = []
    if len(index) <= train_days:
        return dates
    pos = train_days
    while pos < len(index) - 1:
        dates.append(index[pos])
        pos += step_days
    return dates


def periods_from_days(days: int, rebalance_every_days: int) -> int:
    if days <= 0:
        return 0
    return max(1, int(np.ceil(days / rebalance_every_days)))


def position_is_bnb(pos: object) -> bool:
    return "BNB" in str(pos).upper()


def load_broad_universe(shortlist_fallback_path: Path) -> tuple[list[dict], list[str]]:
    records: list[dict] = []
    excluded_assets: list[str] = []

    if PHASE67_TOP100_MAPPING.exists():
        df = normalize_columns(pd.read_csv(PHASE67_TOP100_MAPPING))
        asset_col = detect_asset_column(df)
        file_col = detect_file_column(df)
        if asset_col and file_col:
            for _, row in df.iterrows():
                asset = str(row[asset_col]).strip().upper()
                file_path = maybe_make_abs(str(row[file_col]))
                if not asset or asset == "NAN":
                    continue
                if asset_is_excluded(asset):
                    excluded_assets.append(asset)
                    continue
                if file_path.exists():
                    records.append({"asset": asset, "file": str(file_path), "source": "phase67_top100_mapping"})

    if not records and PHASE67_TOP100_DOWNLOADS.exists():
        df = normalize_columns(pd.read_csv(PHASE67_TOP100_DOWNLOADS))
        asset_col = detect_asset_column(df)
        file_col = detect_file_column(df)
        if asset_col and file_col:
            for _, row in df.iterrows():
                asset = str(row[asset_col]).strip().upper()
                file_path = maybe_make_abs(str(row[file_col]))
                if not asset or asset == "NAN":
                    continue
                if asset_is_excluded(asset):
                    excluded_assets.append(asset)
                    continue
                if file_path.exists():
                    records.append({"asset": asset, "file": str(file_path), "source": "phase67_top100_downloads"})

    if not records:
        df = normalize_columns(pd.read_csv(shortlist_fallback_path))
        if "asset" not in df.columns or "file" not in df.columns:
            raise ValueError("Shortlist fallback file must contain asset and file columns")
        for _, row in df.iterrows():
            asset = str(row["asset"]).strip().upper()
            file_path = maybe_make_abs(str(row["file"]))
            if not asset or asset == "NAN":
                continue
            if asset_is_excluded(asset):
                excluded_assets.append(asset)
                continue
            if file_path.exists():
                records.append({"asset": asset, "file": str(file_path), "source": "phase67b_shortlist_fallback"})

    dedup: dict[str, dict] = {}
    for row in records:
        dedup[str(row["asset"]).upper()] = row

    out = sorted(dedup.values(), key=lambda x: x["asset"])
    return out, sorted(set(excluded_assets))


def evaluate_asset_for_date(
    asset: str,
    decision_date: pd.Timestamp,
    core_df: pd.DataFrame,
    asset_strategies: dict[str, pd.DataFrame],
    cfg: DynamicUniverseConfig,
) -> dict:
    idx = core_df.index
    decision_loc = idx.get_loc(decision_date)

    train_start_loc = max(0, decision_loc - cfg.trailing_train_days + 1)
    recent_start_loc = max(0, decision_loc - cfg.recent_days + 1)
    downside_start_loc = max(0, decision_loc - cfg.downside_lookback_days + 1)

    train_idx = idx[train_start_loc:decision_loc + 1]
    recent_idx = idx[recent_start_loc:decision_loc + 1]
    downside_idx = idx[downside_start_loc:decision_loc + 1]

    df = asset_strategies[asset]

    core_train_total, core_train_dd = slice_metrics_from_returns(core_df.loc[train_idx, "strategy_return"])
    core_recent_total, _ = slice_metrics_from_returns(core_df.loc[recent_idx, "strategy_return"])
    _, core_downside_dd = slice_metrics_from_returns(core_df.loc[downside_idx, "strategy_return"])

    train_total, train_dd = slice_metrics_from_returns(df.loc[train_idx, "strategy_return"])
    recent_total, _ = slice_metrics_from_returns(df.loc[recent_idx, "strategy_return"])
    _, cand_downside_dd = slice_metrics_from_returns(df.loc[downside_idx, "strategy_return"])

    train_delta = train_total - core_train_total
    recent_delta = recent_total - core_recent_total
    dd_worsen = train_dd - core_train_dd
    downside_dd_worsen = cand_downside_dd - core_downside_dd
    triggers = int(pd.to_numeric(df.loc[train_idx, "candidate_execute"], errors="coerce").fillna(0.0).sum())

    passed = (
        triggers >= cfg.min_triggers_in_train
        and train_delta >= cfg.min_total_delta_pct
        and recent_delta >= cfg.min_recent_delta_pct
        and recent_delta >= cfg.promotion_margin_pct
        and dd_worsen <= cfg.max_allowed_dd_worsen_pct
        and downside_dd_worsen <= cfg.downside_max_worsen_pct
    )

    score = (
        (recent_delta * 4.0)
        + (train_delta * 1.5)
        - max(0.0, dd_worsen) * 1.25
        - max(0.0, downside_dd_worsen) * 1.50
        + triggers * 0.15
    )

    return {
        "asset": asset,
        "train_total_delta_pct": train_delta,
        "recent_total_delta_pct": recent_delta,
        "train_dd_worsen_pct": dd_worsen,
        "downside_dd_worsen_pct": downside_dd_worsen,
        "train_triggers": triggers,
        "score": score,
        "passed_filters": passed,
    }


def evaluate_admission_for_date(
    asset: str,
    decision_date: pd.Timestamp,
    core_df: pd.DataFrame,
    asset_strategies: dict[str, pd.DataFrame],
    asset_meta: dict[str, dict],
    cfg: DynamicUniverseConfig,
) -> dict:
    idx = core_df.index
    decision_loc = idx.get_loc(decision_date)

    train_start_loc = max(0, decision_loc - cfg.admission_train_days + 1)
    recent_start_loc = max(0, decision_loc - cfg.admission_recent_days + 1)

    train_idx = idx[train_start_loc:decision_loc + 1]
    recent_idx = idx[recent_start_loc:decision_loc + 1]

    df = asset_strategies[asset]

    core_train_total, core_train_dd = slice_metrics_from_returns(core_df.loc[train_idx, "strategy_return"])
    core_recent_total, _ = slice_metrics_from_returns(core_df.loc[recent_idx, "strategy_return"])

    train_total, train_dd = slice_metrics_from_returns(df.loc[train_idx, "strategy_return"])
    recent_total, _ = slice_metrics_from_returns(df.loc[recent_idx, "strategy_return"])

    train_delta = train_total - core_train_total
    recent_delta = recent_total - core_recent_total
    dd_worsen = train_dd - core_train_dd
    trigger_days = int(pd.to_numeric(df.loc[train_idx, "candidate_execute"], errors="coerce").fillna(0.0).sum())

    meta = asset_meta.get(asset, {})
    history_days = int(meta.get("history_days", 0))
    max_gap_days = int(meta.get("max_gap_days", 999999))
    non_na_close_ratio = float(meta.get("non_na_close_ratio", 0.0))

    quality_score = (
        recent_delta * 3.0
        + train_delta * 1.25
        - max(0.0, dd_worsen) * 1.25
        + trigger_days * 0.10
        + min(non_na_close_ratio / 100.0, 1.0) * 2.0
        - min(max_gap_days, 30) * 0.05
    )

    eligible = (
        history_days >= cfg.admission_min_history_days
        and trigger_days >= cfg.admission_min_trigger_days
        and train_delta >= cfg.admission_min_train_delta_pct
        and recent_delta >= cfg.admission_min_recent_delta_pct
        and dd_worsen <= cfg.admission_max_dd_worsen_pct
        and quality_score >= cfg.admission_min_quality_score
        and non_na_close_ratio >= 95.0
    )

    failing = (
        train_delta < cfg.demotion_train_delta_pct
        or recent_delta < cfg.demotion_recent_delta_pct
        or trigger_days < cfg.demotion_trigger_days
    )

    return {
        "asset": asset,
        "history_days": history_days,
        "max_gap_days": max_gap_days,
        "non_na_close_ratio": non_na_close_ratio,
        "admission_train_delta_pct": train_delta,
        "admission_recent_delta_pct": recent_delta,
        "admission_dd_worsen_pct": dd_worsen,
        "admission_trigger_days": trigger_days,
        "admission_quality_score": quality_score,
        "admission_eligible": eligible,
        "admission_failing": failing,
    }


def simulate_dynamic_universe_governance(
    core_df: pd.DataFrame,
    asset_strategies: dict[str, pd.DataFrame],
    asset_meta: dict[str, dict],
    cfg: DynamicUniverseConfig,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    idx = core_df.index
    rebalance_dates = build_rebalance_dates(idx, cfg.trailing_train_days, cfg.rebalance_every_days)

    governance = core_df.copy()
    governance["chosen_asset"] = ""
    governance["weekly_authorized_asset"] = ""
    governance["approved_shortlist"] = ""
    governance["strategy_return"] = pd.to_numeric(governance["strategy_return"], errors="coerce").fillna(0.0)
    governance["executed_regime"] = governance["executed_regime"].astype(str)
    governance["executed_position"] = governance["executed_position"].astype(str)

    decision_rows = []
    leaderboard_rows = []
    universe_rows = []

    incumbent_asset = ""
    incumbent_hold_periods = 0
    blocked_reentry_until: dict[str, int] = {}
    consecutive_passes: dict[str, int] = {}

    asset_state = {asset: "WATCHLIST" for asset in asset_strategies.keys()}
    fail_streaks = {asset: 0 for asset in asset_strategies.keys()}
    pass_streaks = {asset: 0 for asset in asset_strategies.keys()}
    suspension_until: dict[str, int] = {}

    reentry_cooldown_periods = periods_from_days(cfg.reentry_cooldown_days, cfg.rebalance_every_days)
    assets = sorted(asset_strategies.keys())

    for i, decision_date in enumerate(rebalance_dates):
        start_loc = idx.get_loc(decision_date) + 1
        if start_loc >= len(idx):
            continue

        if i + 1 < len(rebalance_dates):
            next_date = rebalance_dates[i + 1]
            end_loc = idx.get_loc(next_date)
        else:
            next_date = idx[-1]
            end_loc = len(idx)

        core_asset_today = str(core_df.loc[decision_date, "executed_position"]).upper() if decision_date in core_df.index else ""
        core_is_bnb = position_is_bnb(core_asset_today)

        admission_rows = []
        for asset in assets:
            admission = evaluate_admission_for_date(asset, decision_date, core_df, asset_strategies, asset_meta, cfg)

            prev_state = asset_state.get(asset, "WATCHLIST")
            suspended = suspension_until.get(asset, -1) >= i

            if admission["admission_eligible"]:
                pass_streaks[asset] = pass_streaks.get(asset, 0) + 1
                fail_streaks[asset] = 0
            elif admission["admission_failing"]:
                fail_streaks[asset] = fail_streaks.get(asset, 0) + 1
                pass_streaks[asset] = 0
            else:
                pass_streaks[asset] = 0
                fail_streaks[asset] = 0

            new_state = prev_state

            if suspended:
                new_state = "SUSPENDED"
            else:
                if prev_state == "SUSPENDED":
                    if pass_streaks.get(asset, 0) >= cfg.reapprove_pass_weeks:
                        new_state = "WATCHLIST"
                    else:
                        new_state = "SUSPENDED"
                elif prev_state == "APPROVED":
                    if fail_streaks.get(asset, 0) >= cfg.suspend_required_fail_weeks:
                        new_state = "SUSPENDED"
                        suspension_until[asset] = i + cfg.probation_ban_periods - 1
                    elif fail_streaks.get(asset, 0) >= cfg.probation_required_fail_weeks:
                        new_state = "PROBATION"
                    elif admission["admission_eligible"]:
                        new_state = "APPROVED"
                    else:
                        new_state = "APPROVED"
                elif prev_state == "PROBATION":
                    if fail_streaks.get(asset, 0) >= cfg.suspend_required_fail_weeks:
                        new_state = "SUSPENDED"
                        suspension_until[asset] = i + cfg.probation_ban_periods - 1
                    elif admission["admission_eligible"] and pass_streaks.get(asset, 0) >= cfg.reapprove_pass_weeks:
                        new_state = "APPROVED"
                    else:
                        new_state = "PROBATION"
                else:  # WATCHLIST
                    if admission["admission_eligible"] and pass_streaks.get(asset, 0) >= cfg.reapprove_pass_weeks:
                        new_state = "APPROVED"
                    elif fail_streaks.get(asset, 0) >= cfg.probation_required_fail_weeks:
                        new_state = "PROBATION"
                    else:
                        new_state = "WATCHLIST"

            asset_state[asset] = new_state

            admission["state"] = new_state
            admission["state_prev"] = prev_state
            admission["fail_streaks"] = fail_streaks.get(asset, 0)
            admission["pass_streaks"] = pass_streaks.get(asset, 0)
            admission["suspended_now"] = new_state == "SUSPENDED"
            admission_rows.append(admission)

            universe_rows.append(
                {
                    "profile": cfg.profile_name,
                    "decision_date": decision_date.strftime("%Y-%m-%d"),
                    **admission,
                }
            )

        approved_df = pd.DataFrame([r for r in admission_rows if r["state"] == "APPROVED"])
        if approved_df.empty:
            approved_assets = []
        else:
            approved_df = approved_df.sort_values(
                by=["admission_quality_score", "admission_recent_delta_pct", "admission_train_delta_pct"],
                ascending=[False, False, False],
                na_position="last",
            ).head(cfg.max_approved_assets)
            approved_assets = approved_df["asset"].astype(str).tolist()

        eval_rows = []
        for asset in approved_assets:
            meta = evaluate_asset_for_date(asset, decision_date, core_df, asset_strategies, cfg)

            reentry_blocked = blocked_reentry_until.get(asset, -1) >= i and asset != incumbent_asset
            if bool(meta["passed_filters"]):
                consecutive_passes[asset] = consecutive_passes.get(asset, 0) + 1
            else:
                consecutive_passes[asset] = 0

            persistence_ok = consecutive_passes.get(asset, 0) >= cfg.persistence_weeks
            required_recent_delta = cfg.promotion_margin_pct + (cfg.bnb_shield_margin_pct if core_is_bnb else 0.0)
            promotion_ok = float(meta["recent_total_delta_pct"]) >= required_recent_delta

            if reentry_blocked or (not persistence_ok) or (not promotion_ok):
                meta["passed_filters"] = False
                meta["score"] = -1e18

            meta["profile"] = cfg.profile_name
            meta["decision_date"] = decision_date.strftime("%Y-%m-%d")
            meta["next_date_exclusive"] = next_date.strftime("%Y-%m-%d")
            meta["reentry_blocked"] = reentry_blocked
            meta["persistence_passes"] = consecutive_passes.get(asset, 0)
            meta["persistence_ok"] = persistence_ok
            meta["core_asset_today"] = core_asset_today
            meta["core_is_bnb"] = core_is_bnb
            meta["required_recent_delta_pct"] = required_recent_delta
            meta["promotion_ok"] = promotion_ok
            eval_rows.append(meta)

        leaderboard = pd.DataFrame(eval_rows)
        if not leaderboard.empty:
            leaderboard = leaderboard.sort_values(
                by=["passed_filters", "score", "recent_total_delta_pct", "train_total_delta_pct"],
                ascending=[False, False, False, False],
                na_position="last",
            ).reset_index(drop=True)

        best_asset = ""
        best_meta = {}
        if not leaderboard.empty:
            passed_df = leaderboard[leaderboard["passed_filters"] == True]
            if not passed_df.empty:
                best_asset = str(passed_df.iloc[0]["asset"])
                best_meta = passed_df.iloc[0].to_dict()

        selected_asset = best_asset
        keep_reason = "best_passed"

        incumbent_meta = None
        if incumbent_asset and not leaderboard.empty:
            incumbent_rows = leaderboard[leaderboard["asset"] == incumbent_asset]
            if not incumbent_rows.empty:
                incumbent_meta = incumbent_rows.iloc[0].to_dict()

        if incumbent_asset and incumbent_meta is not None and bool(incumbent_meta.get("passed_filters", False)):
            incumbent_score = float(pd.to_numeric(incumbent_meta.get("score"), errors="coerce"))
            best_score = float(pd.to_numeric(best_meta.get("score"), errors="coerce")) if best_asset else -1e18

            if incumbent_hold_periods < cfg.min_hold_periods:
                selected_asset = incumbent_asset
                keep_reason = "min_hold"
            elif not best_asset:
                selected_asset = incumbent_asset
                keep_reason = "incumbent_only_passed"
            elif best_asset != incumbent_asset and best_score < incumbent_score + cfg.switch_score_margin:
                selected_asset = incumbent_asset
                keep_reason = "switch_margin_not_met"
            elif best_asset == incumbent_asset:
                selected_asset = incumbent_asset
                keep_reason = "incumbent_best"

        if incumbent_asset and selected_asset != incumbent_asset and reentry_cooldown_periods > 0:
            blocked_reentry_until[incumbent_asset] = i + reentry_cooldown_periods - 1

        if not leaderboard.empty:
            leaderboard["incumbent_asset_before"] = incumbent_asset
            leaderboard["selected_asset"] = selected_asset
            leaderboard["keep_reason"] = keep_reason
            leaderboard["approved_shortlist_size"] = len(approved_assets)
            leaderboard["approved_shortlist"] = ",".join(sorted(approved_assets))
            leaderboard_rows.append(leaderboard)

        period_idx = idx[start_loc:end_loc]
        if len(period_idx) == 0:
            continue

        approved_shortlist_str = ",".join(sorted(approved_assets))
        governance.loc[period_idx, "approved_shortlist"] = approved_shortlist_str
        governance.loc[period_idx, "weekly_authorized_asset"] = selected_asset

        if selected_asset:
            chosen_df = asset_strategies[selected_asset]
            governance.loc[period_idx, "strategy_return"] = chosen_df.loc[period_idx, "strategy_return"].values
            governance.loc[period_idx, "executed_regime"] = chosen_df.loc[period_idx, "executed_regime"].values
            governance.loc[period_idx, "executed_position"] = chosen_df.loc[period_idx, "executed_position"].values
            governance.loc[period_idx, "chosen_asset"] = selected_asset

        decision_rows.append(
            {
                "profile": cfg.profile_name,
                "decision_date": decision_date.strftime("%Y-%m-%d"),
                "period_start": period_idx[0].strftime("%Y-%m-%d"),
                "period_end": period_idx[-1].strftime("%Y-%m-%d"),
                "incumbent_asset_before": incumbent_asset,
                "incumbent_hold_periods_before": incumbent_hold_periods,
                "core_asset_today": core_asset_today,
                "core_is_bnb": core_is_bnb,
                "approved_shortlist_size": len(approved_assets),
                "approved_shortlist": approved_shortlist_str,
                "selected_asset": selected_asset,
                "selected": bool(selected_asset),
                "keep_reason": keep_reason,
                "best_passed_asset": best_asset,
                "best_passed_score": pd.to_numeric(best_meta.get("score"), errors="coerce") if best_asset else np.nan,
                "required_recent_delta_pct": cfg.promotion_margin_pct + (cfg.bnb_shield_margin_pct if core_is_bnb else 0.0),
            }
        )

        if selected_asset:
            if selected_asset == incumbent_asset:
                incumbent_hold_periods += 1
            else:
                incumbent_asset = selected_asset
                incumbent_hold_periods = 1
        else:
            incumbent_asset = ""
            incumbent_hold_periods = 0

    governance["equity"] = (1.0 + pd.to_numeric(governance["strategy_return"], errors="coerce").fillna(0.0)).cumprod()

    decisions_df = pd.DataFrame(decision_rows)
    leaderboard_df = pd.concat(leaderboard_rows, ignore_index=True) if leaderboard_rows else pd.DataFrame()
    universe_df = pd.DataFrame(universe_rows)

    return governance, decisions_df, leaderboard_df, universe_df


def add_delta_cols(row: dict, ref: dict, prefix: str) -> dict:
    out = row.copy()
    for metric in ["cagr_pct", "max_drawdown_pct", "since2021_cagr_pct", "since2023_cagr_pct", "since2025_cagr_pct"]:
        out[f"delta_vs_{prefix}_{metric}"] = pd.to_numeric(out.get(metric), errors="coerce") - pd.to_numeric(ref.get(metric), errors="coerce")
    return out


def build_profiles() -> list[tuple[ChallengerOverlayConfig, DynamicUniverseConfig]]:
    common_overlay = dict(
        score_lb=30,
        fast_ma=20,
        slow_ma=100,
        min_score=0.12,
        edge_vs_core=0.03,
        risk_ma=150,
        risk_buffer=-0.03,
        vol_lb=30,
        vol_cap=0.045,
        rel_recent_lb=30,
        rel_recent_min_edge=0.00,
        cooldown_days=3,
    )

    common_cfg = dict(
        trailing_train_days=365,
        recent_days=42,
        rebalance_every_days=7,
        min_triggers_in_train=3,
        min_total_delta_pct=0.0,
        min_recent_delta_pct=0.0,
        max_allowed_dd_worsen_pct=6.0,
        switch_score_margin=2.0,
        min_hold_periods=2,
        promotion_margin_pct=2.0,
        persistence_weeks=2,
        reentry_cooldown_days=7,
        downside_lookback_days=42,
        downside_max_worsen_pct=2.5,
        bnb_shield_margin_pct=1.0,
        probation_lookback_days=45,
        probation_min_delta_pct=-0.5,
        probation_ban_periods=4,
        demotion_train_delta_pct=-2.0,
        demotion_recent_delta_pct=-1.0,
        demotion_trigger_days=1,
        probation_required_fail_weeks=2,
        suspend_required_fail_weeks=4,
        reapprove_pass_weeks=2,
    )

    return [
        (
            ChallengerOverlayConfig(**common_overlay),
            DynamicUniverseConfig(
                profile_name="phase68b_dynamic_repair_base",
                admission_train_days=180,
                admission_recent_days=42,
                admission_min_train_delta_pct=0.0,
                admission_min_recent_delta_pct=0.0,
                admission_max_dd_worsen_pct=5.5,
                admission_min_trigger_days=2,
                admission_min_history_days=240,
                admission_min_quality_score=0.20,
                max_approved_assets=6,
                **common_cfg,
            ),
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            DynamicUniverseConfig(
                profile_name="phase68b_dynamic_repair_tight5",
                admission_train_days=180,
                admission_recent_days=42,
                admission_min_train_delta_pct=0.25,
                admission_min_recent_delta_pct=0.0,
                admission_max_dd_worsen_pct=5.0,
                admission_min_trigger_days=2,
                admission_min_history_days=240,
                admission_min_quality_score=0.35,
                max_approved_assets=5,
                **common_cfg,
            ),
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            DynamicUniverseConfig(
                profile_name="phase68b_dynamic_repair_fast6",
                admission_train_days=140,
                admission_recent_days=35,
                admission_min_train_delta_pct=0.0,
                admission_min_recent_delta_pct=0.0,
                admission_max_dd_worsen_pct=5.5,
                admission_min_trigger_days=2,
                admission_min_history_days=200,
                admission_min_quality_score=0.10,
                max_approved_assets=6,
                **common_cfg,
            ),
        ),
    ]


def compute_next_rebalance_date(decisions_df: pd.DataFrame, rebalance_days: int) -> str:
    if decisions_df.empty or "decision_date" not in decisions_df.columns:
        return ""
    last_decision = pd.to_datetime(decisions_df["decision_date"], errors="coerce").dropna()
    if last_decision.empty:
        return ""
    return (last_decision.iloc[-1] + pd.Timedelta(days=rebalance_days)).strftime("%Y-%m-%d")


def print_compare_rows(compare: pd.DataFrame) -> None:
    if compare.empty:
        log("[PHASE68B] No compare rows")
        return
    log("")
    log("=== PHASE68B ALL PROFILE RESULTS ===")
    for _, row in compare.iterrows():
        log(
            f"{row['model']} | "
            f"CAGR {pd.to_numeric(row['cagr_pct'], errors='coerce'):.2f} | "
            f"DD {pd.to_numeric(row['max_drawdown_pct'], errors='coerce'):.2f} | "
            f"since2023 {pd.to_numeric(row['since2023_cagr_pct'], errors='coerce'):.2f} | "
            f"since2025 {pd.to_numeric(row['since2025_cagr_pct'], errors='coerce'):.2f} | "
            f"vs66G CAGR {pd.to_numeric(row['delta_vs_phase66g_core_cagr_pct'], errors='coerce'):.2f} | "
            f"vs66G since2023 {pd.to_numeric(row['delta_vs_phase66g_core_since2023_cagr_pct'], errors='coerce'):.2f} | "
            f"vs66G since2025 {pd.to_numeric(row['delta_vs_phase66g_core_since2025_cagr_pct'], errors='coerce'):.2f} | "
            f"vs66G DD {pd.to_numeric(row['delta_vs_phase66g_core_max_drawdown_pct'], errors='coerce'):.2f} | "
            f"sel {int(pd.to_numeric(row['selection_count'], errors='coerce'))} | "
            f"sw {int(pd.to_numeric(row['switch_count'], errors='coerce'))} | "
            f"avgApproved {pd.to_numeric(row['approved_shortlist_avg_size'], errors='coerce'):.2f} | "
            f"lastApproved {int(pd.to_numeric(row['approved_shortlist_last_size'], errors='coerce'))}"
        )


def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE68B dynamic admission repair")
    parser.add_argument("--core-paper", type=str, default=str(CORE_PAPER))
    parser.add_argument("--phase63-paper", type=str, default=str(PHASE63_PAPER))
    parser.add_argument("--shortlist-file", type=str, default=str(PHASE67B_SHORTLIST))
    args = parser.parse_args()

    ensure_dir(PHASE68B_DIR)

    core_df = load_strategy_paper(Path(args.core_paper))
    phase63_df = load_strategy_paper(Path(args.phase63_paper))
    broad_universe, excluded_assets = load_broad_universe(Path(args.shortlist_file))
    profiles = build_profiles()

    core_row = calc_metrics(core_df, CORE_MODEL_KEY)
    core_row.update(window_metrics(core_df, "2021-01-01"))
    core_row.update(window_metrics(core_df, "2023-01-01"))
    core_row.update(window_metrics(core_df, "2025-01-01"))

    phase63_row = calc_metrics(phase63_df, PHASE63_MODEL_KEY)
    phase63_row.update(window_metrics(phase63_df, "2021-01-01"))
    phase63_row.update(window_metrics(phase63_df, "2023-01-01"))
    phase63_row.update(window_metrics(phase63_df, "2025-01-01"))

    log("[PHASE68B] Start")
    log(f"[PHASE68B] Core paper: {args.core_paper}")
    log(f"[PHASE68B] Excluded assets count: {len(excluded_assets)}")
    if excluded_assets:
        log(f"[PHASE68B] Excluded assets: {excluded_assets}")
    log(f"[PHASE68B] Broad universe size after exclude: {len(broad_universe)}")

    summary_rows = [phase63_row, core_row]
    compare_rows = []
    decisions_all = []
    leaderboards_all = []
    universe_all = []
    asset_quality_rows = []
    papers = {
        PHASE63_MODEL_KEY: phase63_df.copy(),
        CORE_MODEL_KEY: core_df.copy(),
    }

    for overlay_cfg, cfg in profiles:
        asset_strategies: dict[str, pd.DataFrame] = {}
        asset_meta: dict[str, dict] = {}
        failed_assets = []

        for item in broad_universe:
            asset = str(item["asset"]).strip().upper()
            file_path = Path(str(item["file"]))
            try:
                daily, q = load_local_daily_for_core(file_path)
                strat = build_asset_overlay_strategy(core_df, daily, overlay_cfg, asset)
                asset_strategies[asset] = strat
                asset_meta[asset] = q
                asset_quality_rows.append(
                    {
                        "profile": cfg.profile_name,
                        "asset": asset,
                        "file": str(file_path),
                        "source": item.get("source", ""),
                        **q,
                    }
                )
            except Exception as e:
                failed_assets.append({"profile": cfg.profile_name, "asset": asset, "reason": str(e), "file": str(file_path)})

        governance, decisions_df, leaderboard_df, universe_df = simulate_dynamic_universe_governance(
            core_df=core_df,
            asset_strategies=asset_strategies,
            asset_meta=asset_meta,
            cfg=cfg,
        )

        row = calc_metrics(governance, cfg.profile_name)
        row.update(window_metrics(governance, "2021-01-01"))
        row.update(window_metrics(governance, "2023-01-01"))
        row.update(window_metrics(governance, "2025-01-01"))
        row = add_delta_cols(row, core_row, "phase66g_core")
        row = add_delta_cols(row, phase63_row, "phase63")
        row["universe_size_loaded"] = len(asset_strategies)
        row["selection_count"] = int(decisions_df["selected"].sum()) if not decisions_df.empty else 0
        row["switch_count"] = int((decisions_df["selected_asset"].astype(str) != decisions_df["selected_asset"].astype(str).shift(1)).sum() - 1) if not decisions_df.empty else 0
        row["unique_selected_assets"] = int(governance["chosen_asset"].astype(str).replace("", np.nan).dropna().nunique())
        row["excluded_assets_count"] = len(excluded_assets)

        if not decisions_df.empty:
            row["approved_shortlist_avg_size"] = float(pd.to_numeric(decisions_df["approved_shortlist_size"], errors="coerce").mean())
            row["approved_shortlist_last_size"] = int(pd.to_numeric(decisions_df["approved_shortlist_size"], errors="coerce").iloc[-1])
            row["approved_shortlist_last"] = str(decisions_df["approved_shortlist"].iloc[-1])
        else:
            row["approved_shortlist_avg_size"] = 0.0
            row["approved_shortlist_last_size"] = 0
            row["approved_shortlist_last"] = ""

        if not universe_df.empty:
            last_date = str(universe_df["decision_date"].iloc[-1])
            last_states = universe_df[universe_df["decision_date"].astype(str) == last_date].copy()
            state_counts = last_states.groupby("state").size().to_dict()
            row["approved_assets_now"] = int(state_counts.get("APPROVED", 0))
            row["watchlist_assets_now"] = int(state_counts.get("WATCHLIST", 0))
            row["probation_assets_now"] = int(state_counts.get("PROBATION", 0))
            row["suspended_assets_now"] = int(state_counts.get("SUSPENDED", 0))
            approved_now = sorted(last_states.loc[last_states["state"] == "APPROVED", "asset"].astype(str).tolist())
            row["approved_assets_now_list"] = ",".join(approved_now)
        else:
            row["approved_assets_now"] = 0
            row["watchlist_assets_now"] = 0
            row["probation_assets_now"] = 0
            row["suspended_assets_now"] = 0
            row["approved_assets_now_list"] = ""

        summary_rows.append(row)
        compare_rows.append(row)
        papers[cfg.profile_name] = governance.copy()

        if not decisions_df.empty:
            decisions_all.append(decisions_df.copy())
        if not leaderboard_df.empty:
            leaderboards_all.append(leaderboard_df.copy())
        if not universe_df.empty:
            universe_all.append(universe_df.copy())
        for x in failed_assets:
            asset_quality_rows.append(x)

        log(f"[PHASE68B] done {cfg.profile_name} | loaded_assets={len(asset_strategies)} | failed_assets={len(failed_assets)}")

    summary = pd.DataFrame(summary_rows)
    compare = pd.DataFrame(compare_rows)
    if not compare.empty:
        compare = compare.sort_values(
            by=[
                "delta_vs_phase66g_core_since2025_cagr_pct",
                "delta_vs_phase66g_core_max_drawdown_pct",
                "delta_vs_phase66g_core_since2023_cagr_pct",
                "delta_vs_phase66g_core_cagr_pct",
            ],
            ascending=[False, False, False, False],
            na_position="last",
        ).reset_index(drop=True)

    decisions_out = pd.concat(decisions_all, ignore_index=True) if decisions_all else pd.DataFrame()
    leaderboards_out = pd.concat(leaderboards_all, ignore_index=True) if leaderboards_all else pd.DataFrame()
    universe_out = pd.concat(universe_all, ignore_index=True) if universe_all else pd.DataFrame()
    asset_quality_df = pd.DataFrame(asset_quality_rows)

    best = compare.head(1)
    best_model = str(best.iloc[0]["model"]) if not best.empty else ""
    best_cfg = None
    for _, cfg in profiles:
        if cfg.profile_name == best_model:
            best_cfg = cfg
            break

    live_status = pd.DataFrame()
    latest_top10 = pd.DataFrame()
    latest_universe = pd.DataFrame()

    if best_model and best_model in papers:
        best_paper = papers[best_model]
        latest_available_date = best_paper.index.max().strftime("%Y-%m-%d") if len(best_paper) else ""
        current_asset = str(best_paper["weekly_authorized_asset"].astype(str).iloc[-1]) if len(best_paper) else ""
        current_asset = current_asset if current_asset else "CORE"

        best_decisions = decisions_out[decisions_out["profile"].astype(str) == best_model].copy() if not decisions_out.empty else pd.DataFrame()
        next_rebalance_date = compute_next_rebalance_date(best_decisions, best_cfg.rebalance_every_days if best_cfg else 7)

        live_status = pd.DataFrame([{
            "model": best_model,
            "latest_available_date": latest_available_date,
            "current_asset": current_asset,
            "next_rebalance_date": next_rebalance_date,
            "approved_shortlist_last_size": int(pd.to_numeric(best.iloc[0]["approved_shortlist_last_size"], errors="coerce")) if not best.empty else 0,
            "approved_shortlist_last": str(best.iloc[0]["approved_shortlist_last"]) if not best.empty else "",
        }])

        best_leaderboard = leaderboards_out[leaderboards_out["profile"].astype(str) == best_model].copy() if not leaderboards_out.empty else pd.DataFrame()
        if not best_leaderboard.empty:
            last_decision_date = str(best_leaderboard["decision_date"].astype(str).iloc[-1])
            latest_top10 = (
                best_leaderboard[best_leaderboard["decision_date"].astype(str) == last_decision_date]
                .sort_values(by=["passed_filters", "score", "recent_total_delta_pct", "train_total_delta_pct"], ascending=[False, False, False, False], na_position="last")
                .head(10)
                .reset_index(drop=True)
            )

        best_universe = universe_out[universe_out["profile"].astype(str) == best_model].copy() if not universe_out.empty else pd.DataFrame()
        if not best_universe.empty:
            last_decision_date = str(best_universe["decision_date"].astype(str).iloc[-1])
            latest_universe = (
                best_universe[best_universe["decision_date"].astype(str) == last_decision_date]
                .sort_values(by=["state", "admission_quality_score", "admission_recent_delta_pct"], ascending=[True, False, False], na_position="last")
                .reset_index(drop=True)
            )

    summary_path = PHASE68B_DIR / "phase68b_dynamic_admission_summary.csv"
    compare_path = PHASE68B_DIR / "phase68b_dynamic_admission_compare.csv"
    decisions_path = PHASE68B_DIR / "phase68b_dynamic_admission_decisions.csv"
    leaderboard_path = PHASE68B_DIR / "phase68b_dynamic_admission_leaderboard.csv"
    universe_path = PHASE68B_DIR / "phase68b_dynamic_universe_states.csv"
    asset_quality_path = PHASE68B_DIR / "phase68b_dynamic_admission_asset_quality.csv"
    live_status_path = PHASE68B_DIR / "phase68b_live_status.csv"
    latest_top10_path = PHASE68B_DIR / "phase68b_latest_decision_top10.csv"
    latest_universe_path = PHASE68B_DIR / "phase68b_latest_universe_states.csv"
    manifest_path = PHASE68B_DIR / "phase68b_manifest.json"

    summary.to_csv(summary_path, index=False)
    compare.to_csv(compare_path, index=False)
    decisions_out.to_csv(decisions_path, index=False)
    leaderboards_out.to_csv(leaderboard_path, index=False)
    universe_out.to_csv(universe_path, index=False)
    asset_quality_df.to_csv(asset_quality_path, index=False)
    live_status.to_csv(live_status_path, index=False)
    latest_top10.to_csv(latest_top10_path, index=False)
    latest_universe.to_csv(latest_universe_path, index=False)

    for model, paper in papers.items():
        out_path = PHASE68B_DIR / f"{model}_paper.csv"
        paper.reset_index().rename(columns={paper.index.name or "index": "date"}).to_csv(out_path, index=False)

    manifest = {
        "phase": "phase68b_dynamic_admission_repair",
        "phase63_paper": str(args.phase63_paper),
        "core_paper": str(args.core_paper),
        "excluded_assets": excluded_assets,
        "profiles": [{"overlay": asdict(o), "dynamic_universe": asdict(c)} for o, c in profiles],
        "summary_file": str(summary_path),
        "compare_file": str(compare_path),
        "decisions_file": str(decisions_path),
        "leaderboard_file": str(leaderboard_path),
        "universe_states_file": str(universe_path),
        "asset_quality_file": str(asset_quality_path),
        "live_status_file": str(live_status_path),
        "latest_top10_file": str(latest_top10_path),
        "latest_universe_file": str(latest_universe_path),
        "best_model": best_model,
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    log("")
    log("=== PHASE68B TOP RESULT ===")
    if best.empty:
        log("No phase68b profile processed.")
    else:
        row = best.iloc[0]
        log(f"model: {row['model']}")
        log(f"cagr_pct: {row['cagr_pct']:.2f}")
        log(f"max_drawdown_pct: {row['max_drawdown_pct']:.2f}")
        log(f"since2023_cagr_pct: {row['since2023_cagr_pct']:.2f}")
        log(f"since2025_cagr_pct: {row['since2025_cagr_pct']:.2f}")
        log(f"delta_vs_phase66g_core_cagr_pct: {row['delta_vs_phase66g_core_cagr_pct']:.2f}")
        log(f"delta_vs_phase66g_core_since2023_cagr_pct: {row['delta_vs_phase66g_core_since2023_cagr_pct']:.2f}")
        log(f"delta_vs_phase66g_core_since2025_cagr_pct: {row['delta_vs_phase66g_core_since2025_cagr_pct']:.2f}")
        log(f"delta_vs_phase66g_core_max_drawdown_pct: {row['delta_vs_phase66g_core_max_drawdown_pct']:.2f}")
        log(f"selection_count: {int(pd.to_numeric(row['selection_count'], errors='coerce'))}")
        log(f"switch_count: {int(pd.to_numeric(row['switch_count'], errors='coerce'))}")
        log(f"unique_selected_assets: {int(pd.to_numeric(row['unique_selected_assets'], errors='coerce'))}")
        log(f"approved_shortlist_avg_size: {pd.to_numeric(row['approved_shortlist_avg_size'], errors='coerce'):.2f}")
        log(f"approved_shortlist_last_size: {int(pd.to_numeric(row['approved_shortlist_last_size'], errors='coerce'))}")
        log(f"approved_shortlist_last: {row['approved_shortlist_last']}")
        log(f"approved_assets_now: {int(pd.to_numeric(row['approved_assets_now'], errors='coerce'))}")
        log(f"probation_assets_now: {int(pd.to_numeric(row['probation_assets_now'], errors='coerce'))}")
        log(f"suspended_assets_now: {int(pd.to_numeric(row['suspended_assets_now'], errors='coerce'))}")

    print_compare_rows(compare)

    if not live_status.empty:
        ls = live_status.iloc[0]
        log("")
        log("=== PHASE68B LIVE STATUS ===")
        log(f"latest_available_date: {ls['latest_available_date']}")
        log(f"current_asset: {ls['current_asset']}")
        log(f"next_rebalance_date: {ls['next_rebalance_date']}")
        log(f"approved_shortlist_last_size: {ls['approved_shortlist_last_size']}")
        log(f"approved_shortlist_last: {ls['approved_shortlist_last']}")

    if not latest_universe.empty:
        log("")
        log("=== PHASE68B LATEST UNIVERSE STATES ===")
        for _, row in latest_universe.head(20).iterrows():
            log(
                f"{row['asset']} | {row['state']} | "
                f"q {pd.to_numeric(row['admission_quality_score'], errors='coerce'):.2f} | "
                f"recent {pd.to_numeric(row['admission_recent_delta_pct'], errors='coerce'):.2f} | "
                f"train {pd.to_numeric(row['admission_train_delta_pct'], errors='coerce'):.2f} | "
                f"ddW {pd.to_numeric(row['admission_dd_worsen_pct'], errors='coerce'):.2f} | "
                f"pass {int(pd.to_numeric(row['pass_streaks'], errors='coerce'))} | "
                f"fail {int(pd.to_numeric(row['fail_streaks'], errors='coerce'))}"
            )

    log("")
    log(f"[PHASE68B] Saved summary -> {summary_path}")
    log(f"[PHASE68B] Saved compare -> {compare_path}")
    log(f"[PHASE68B] Saved decisions -> {decisions_path}")
    log(f"[PHASE68B] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE68B] Saved universe states -> {universe_path}")
    log(f"[PHASE68B] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE68B] Saved live status -> {live_status_path}")
    log(f"[PHASE68B] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE68B] Saved latest universe -> {latest_universe_path}")
    log(f"[PHASE68B] Saved manifest -> {manifest_path}")


if __name__ == "__main__":
    main()