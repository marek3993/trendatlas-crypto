...
PHASE67_TOP100_ASSET_QUALITY = OUTPUTS / "phase67_top100_build_and_governance" / "phase67_top100_asset_quality.csv"
PHASE67_TOP100_DOWNLOADS = OUTPUTS / "phase67_top100_build_and_governance" / "phase67_top100_downloads.csv"

PHASE68_DIR = OUTPUTS / "phase68_dynamic_admission_removal"
...
class DynamicUniverseConfig:
    ...
    suspension_probation_threshold_pct: float
...
def detect_asset_column(df: pd.DataFrame) -> Optional[str]:
    candidates = ["asset", "symbol", "ticker", "coin", "base_symbol"]
...
def detect_file_column(df: pd.DataFrame) -> Optional[str]:
    candidates = ["file", "csv_path", "path", "local_file", "daily_file", "ohlcv_file"]
    for col in candidates:
        if col in df.columns:
            return col
    return None
...
def load_broad_universe(shortlist_fallback_path: Path) -> list[dict]:
    records: list[dict] = []

    # 1) preferred: top100 mapping
    ...
    # 3) final fallback: shortlist only
    if not records:
    ...
    if not out:
        raise ValueError("No usable broad universe assets found.")
    return out
...
def simulate_dynamic_universe_governance(
    ...
    state_weeks = {asset: 0 for asset in asset_strategies.keys()}
    suspension_until: dict[str, int] = {}
    approve_streaks: dict[str, int] = {}
    ...
    for i, decision_date in enumerate(rebalance_dates):
        ...
        for asset in assets:
            ...
            current_state = asset_state.get(asset, "WATCHLIST")
            suspended = suspension_until.get(asset, -1) >= i

            if admission["admission_eligible"]:
                approve_streaks[asset] = approve_streaks.get(asset, 0) + 1
            else:
                approve_streaks[asset] = 0

            if suspended:
                new_state = "SUSPENDED"
            else:
                probation_trigger = admission["admission_train_delta_pct"] < cfg.suspension_probation_threshold_pct

                if current_state == "SUSPENDED":
                    if approve_streaks.get(asset, 0) >= cfg.reapprove_pass_weeks:
                        new_state = "APPROVED"
                    else:
                        new_state = "WATCHLIST"
                elif admission["admission_eligible"] and approve_streaks.get(asset, 0) >= cfg.reapprove_pass_weeks:
                    new_state = "APPROVED"
                elif probation_trigger:
                    new_state = "PROBATION"
                else:
                    new_state = "WATCHLIST"
            ...
            if new_state == current_state:
                state_weeks[asset] = state_weeks.get(asset, 0) + 1
            else:
                state_weeks[asset] = 1
            ...
            universe_rows.append(
                {
                    ...
                    "asset": asset,
                    "state": new_state,
                    "state_weeks": state_weeks.get(asset, 0),
                    **admission,
                }
            )
        ...
        if leaderboard.empty:
            leaderboard = pd.DataFrame(columns=[
                "asset", "score", "passed_filters", "recent_total_delta_pct", "train_total_delta_pct"
            ])
        else:
...
def build_profiles() -> list[tuple[ChallengerOverlayConfig, DynamicUniverseConfig]]:
    ...
    common_cfg = dict(
        ...
        suspension_probation_threshold_pct=-1.5,
    ...
    return [
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            DynamicUniverseConfig(
                profile_name="phase68_dynamic_admit_strict",
                admission_train_days=180,
                admission_recent_days=42,
                admission_min_train_delta_pct=0.5,
                admission_min_recent_delta_pct=0.25,
                admission_max_dd_worsen_pct=4.5,
                admission_min_trigger_days=3,
                admission_min_history_days=300,
                admission_min_quality_score=0.75,
                **common_cfg,
            ),
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            DynamicUniverseConfig(
                profile_name="phase68_dynamic_admit_fast",
                admission_train_days=120,
                admission_recent_days=28,
                admission_min_train_delta_pct=0.0,
                admission_min_recent_delta_pct=0.0,
                admission_max_dd_worsen_pct=6.0,
                admission_min_trigger_days=2,
                admission_min_history_days=180,
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            DynamicUniverseConfig(
                profile_name="phase68_dynamic_admit_no_neo_bias",
                admission_train_days=180,
                admission_recent_days=42,
                ...
                admission_min_trigger_days=2,
                admission_min_history_days=240,
                admission_min_quality_score=0.40,
                **common_cfg,
            ),
        ),
    ]
...
def print_compare_rows(compare: pd.DataFrame) -> None:
    if compare.empty:
        log("[PHASE68] No compare rows")
        return
    log("")
    log("=== PHASE68 ALL PROFILE RESULTS ===")
    cols = [
        "model",
        "cagr_pct",
        "max_drawdown_pct",
        ...
        "selection_count",
        "switch_count",
        "approved_shortlist_avg_size",
        "approved_shortlist_last_size",
    ]
    tmp = compare.copy()
    for col in cols:
        if col not in tmp.columns:
            tmp[col] = np.nan
    for _, row in tmp[cols].iterrows():
        log(
            f"{row['model']} | "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE68 dynamic admission/removal over broad universe")
    ...
    log("[PHASE68] Start")
    log(f"[PHASE68] Core paper: {args.core_paper}")
    log(f"[PHASE68] Broad universe size: {len(broad_universe)}")
    log(f"[PHASE68] Broad universe assets: {[x['asset'] for x in broad_universe[:20]]}{' ...' if len(broad_universe) > 20 else ''}")
    ...
    for overlay_cfg, cfg in profiles:
        ...
        if not decisions_df.empty and "approved_shortlist_size" in decisions_df.columns:
        ...
        log(f"[PHASE68] done {cfg.profile_name} | loaded_assets={len(asset_strategies)} | failed_assets={len(failed_assets)}")
            ...
            [
                {
                    ...
                    "approved_shortlist_last": approved_shortlist_last,
                }
            ]
        )
    ...
    summary_path = PHASE68_DIR / "phase68_dynamic_admission_summary.csv"
    compare_path = PHASE68_DIR / "phase68_dynamic_admission_compare.csv"
    decisions_path = PHASE68_DIR / "phase68_dynamic_admission_decisions.csv"
    leaderboard_path = PHASE68_DIR / "phase68_dynamic_admission_leaderboard.csv"
    universe_path = PHASE68_DIR / "phase68_dynamic_universe_states.csv"
    asset_quality_path = PHASE68_DIR / "phase68_dynamic_admission_asset_quality.csv"
    live_status_path = PHASE68_DIR / "phase68_live_status.csv"
    latest_top10_path = PHASE68_DIR / "phase68_latest_decision_top10.csv"
    latest_universe_path = PHASE68_DIR / "phase68_latest_universe_states.csv"
    manifest_path = PHASE68_DIR / "phase68_manifest.json"
    ...
    for model, paper in papers.items():
        out_path = PHASE68_DIR / f"{model}_paper.csv"
        paper.reset_index().rename(columns={paper.index.name or "index": "date"}).to_csv(out_path, index=False)

    manifest = {
        "phase": "phase68_dynamic_admission_removal",
        "phase63_paper": str(args.phase63_paper),
        "core_paper": str(args.core_paper),
        "universe_source_priority": [
            str(PHASE67_TOP100_MAPPING),
            str(PHASE67_TOP100_DOWNLOADS),
            str(args.shortlist_file),
        ],
        "profiles": [
            {
                "overlay": asdict(overlay_cfg),
                "dynamic_universe": asdict(cfg),
            }
            for overlay_cfg, cfg in profiles
        ],
        ...
        "notes": [
            "Phase68 robí broad universe -> weekly approved shortlist -> challenger vs 66G core.",
            "Asset statusy: WATCHLIST / APPROVED / SUSPENDED.",
            "Výstup je písaný aj do CMD, aby netrebalo ručne otvárať CSV."
        ],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    log("")
    log("=== PHASE68 TOP RESULT ===")
    if best.empty:
        log("No phase68 profile processed.")
    else:
        row = best.iloc[0]
    ...
    if not latest_universe.empty:
        log("")
        log("=== PHASE68 LATEST UNIVERSE STATES ===")
        show_cols = ["asset", "state", "admission_quality_score", "admission_recent_delta_pct", "admission_train_delta_pct", "admission_dd_worsen_pct"]
        tmp = latest_universe.copy()
        for _, row in tmp[show_cols].head(20).iterrows():
            log(
    ...
    log("")
    log(f"[PHASE68] Saved summary -> {summary_path}")
    log(f"[PHASE68] Saved compare -> {compare_path}")
    log(f"[PHASE68] Saved decisions -> {decisions_path}")
    log(f"[PHASE68] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE68] Saved universe states -> {universe_path}")
    log(f"[PHASE68] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE68] Saved live status -> {live_status_path}")
    log(f"[PHASE68] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE68] Saved latest universe -> {latest_universe_path}")
    log(f"[PHASE68] Saved manifest -> {manifest_path}")
