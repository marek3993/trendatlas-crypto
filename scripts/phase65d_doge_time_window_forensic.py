...
PHASE65D_DIR = OUTPUTS / "phase65d_doge_time_window_forensic"
...
def align_doge_to_baseline(baseline: pd.DataFrame, doge_df: pd.DataFrame) -> pd.DataFrame:
    x = baseline.copy()
...
def build_variants() -> list[VariantConfig]:
    return [
        VariantConfig("phase65d_pre2025_default", 20, 20, 100, 0.12, 0.03, 150, -0.03, 30, 0.060, 3, None, "2024-12-31"),
        VariantConfig("phase65d_2023_2024_balanced", 20, 20, 100, 0.12, 0.03, 150, -0.03, 30, 0.060, 3, "2023-01-01", "2024-12-31"),
        VariantConfig("phase65d_only_2024", 20, 20, 100, 0.12, 0.03, 150, -0.03, 30, 0.060, 3, "2024-01-01", "2024-12-31"),
        VariantConfig("phase65d_pre2025_stricter", 30, 20, 100, 0.16, 0.06, 150, -0.03, 20, 0.045, 3, None, "2024-12-31"),
        VariantConfig("phase65d_2023_2024_stricter", 30, 20, 100, 0.16, 0.06, 150, -0.03, 20, 0.045, 3, "2023-01-01", "2024-12-31"),
        VariantConfig("phase65d_post2023_pre2025_soft", 20, 20, 100, 0.08, 0.00, 150, -0.03, 30, 0.060, 0, "2023-01-01", "2024-12-31"),
        VariantConfig("phase65d_pre2024_only", 20, 20, 100, 0.12, 0.03, 150, -0.03, 30, 0.060, 3, None, "2023-12-31"),
        VariantConfig("phase65d_2024_h1", 20, 20, 100, 0.12, 0.03, 150, -0.03, 30, 0.060, 3, "2024-01-01", "2024-06-30"),
        VariantConfig("phase65d_2024_h2", 20, 20, 100, 0.12, 0.03, 150, -0.03, 30, 0.060, 3, "2024-07-01", "2024-12-31"),
        VariantConfig("phase65d_pre2025_tight_window", 30, 30, 150, 0.16, 0.06, 200, 0.00, 20, 0.045, 3, "2023-10-01", "2024-12-31"),
    ]
...
def compute_signal_columns(df: pd.DataFrame, cfg: VariantConfig) -> pd.DataFrame:
    ...
    allow_mask = pd.Series(True, index=x.index)
    if cfg.allow_start is not None:
        allow_mask &= x.index >= pd.Timestamp(cfg.allow_start)
    if cfg.allow_end is not None:
        allow_mask &= x.index <= pd.Timestamp(cfg.allow_end)

    x["time_window_ok"] = allow_mask
...
def simulate_variant(baseline: pd.DataFrame, doge_daily: pd.DataFrame, cfg: VariantConfig) -> pd.DataFrame:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE65D DOGE time-window forensic")
    ...
    ensure_dir(PHASE65D_DIR)

    log("[PHASE65D] Start")
    log(f"[PHASE65D] Baseline paper: {args.baseline_paper}")
    ...
    for cfg in variants:
        try:
            ...
            row["allow_start"] = cfg.allow_start or ""
            row["allow_end"] = cfg.allow_end or ""
            ...
            row["source"] = "phase65d_doge_time_window_forensic"

            rows.append(row)
            papers_for_top[cfg.name] = sim
            log(f"[PHASE65D] done {cfg.name}")
    ...
    final_summary = pd.concat([baseline_df, cand_df], ignore_index=True)
    compare_df = pd.concat([baseline_df, cand_df.head(20)], ignore_index=True)

    summary_path = PHASE65D_DIR / "phase65d_doge_time_window_forensic_summary.csv"
    compare_path = PHASE65D_DIR / "phase65d_doge_time_window_forensic_compare.csv"
    manifest_path = PHASE65D_DIR / "phase65d_manifest.json"

    final_summary.to_csv(summary_path, index=False)
    compare_df.to_csv(compare_path, index=False)

    top_models = [CURRENT_WINNER_KEY] + cand_df["model"].head(max(args.top_n_save, 1)).astype(str).tolist()
    for model in top_models:
        paper = papers_for_top.get(model)
        if paper is None:
            continue
        out_path = PHASE65D_DIR / f"{model}_paper.csv"
        paper.reset_index().rename(columns={paper.index.name or "index": "date"}).to_csv(out_path, index=False)

    manifest = {
        "phase": "phase65d_doge_time_window_forensic",
        ...
        "notes": [
            "Toto je forensic lokalizačný test, nie finálny produkčný winner search.",
            "Cieľ: zistiť, či DOGE edge sedí hlavne v 2023–2024 a mimo 2025.",
        ],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    best = cand_df.iloc[0].to_dict() if not cand_df.empty else None

    log("")
    log("=== PHASE65D TOP RESULT ===")
    if best is None:
        log("No valid variants processed.")
    else:
        ...
        log(f"allow_start: {best.get('allow_start')}")
        log(f"allow_end: {best.get('allow_end')}")
        log("")

    log(f"[PHASE65D] Saved summary -> {summary_path}")
    log(f"[PHASE65D] Saved compare -> {compare_path}")
    log(f"[PHASE65D] Saved manifest -> {manifest_path}")
