...
def validate_production_snapshot(snapshot: dict[str, Any], *, source_path: Path) -> dict[str, Any]:
    ...
    route_contract = source_path.resolve().parents[2] / "source_of_truth/production_route_identity_contract.json"
    if route_contract.exists() or "resolved_execution_asset" in snapshot:
        from scripts.production.route_identity import validate_target_identity
        try:
            validate_target_identity(snapshot)
        except (ValueError, KeyError, TypeError) as exc:
            fail("Execution intent route contract: " + str(exc))
...
def main() -> None:
    ...
    if "resolved_execution_asset" in production_snapshot:
        from scripts.production.route_identity import FIELDS
        intent.update({key: production_snapshot[key] for key in FIELDS})
