...
def build_execution_plan(
    *,
    ...
    reasons: list[str] = []
    if "resolved_execution_asset" in production:
        from scripts.production.route_identity import FIELDS, validate_target_identity
        try:
            validate_target_identity(dict(production))
            if any(intent.get(k) != production[k] for k in FIELDS):
                raise ValueError("Normalized intent route lineage differs from Production Core")
        except (ValueError, KeyError, TypeError) as exc:
            reasons.append("route_identity_mismatch:" + str(exc))
