...
def dashboard_model_index_from_source_row(source_row: dict[str, Any]) -> float | None:
    if source_row.get("performance_contract") == "causal_execution_interval_ledger_v1":
        value = parse_float_maybe(source_row.get("performance_model_equity"))
        if value is None:
            raise ValueError("Missing causal model ledger value: performance_model_equity")
        return value
...
def dashboard_model_exposure_from_source_row(source_row: dict[str, Any]) -> float | None:
    if source_row.get("performance_contract") == "causal_execution_interval_ledger_v1":
        value = parse_float_maybe(source_row.get("performance_exposure"))
        if value is None:
            raise ValueError("Missing causal model ledger value: performance_exposure")
        return value
...
def dashboard_model_return_net_from_source_row(source_row: dict[str, Any]) -> float | None:
    if source_row.get("performance_contract") == "causal_execution_interval_ledger_v1":
        value = parse_float_maybe(source_row.get("performance_net_strategy_return"))
        if value is None:
            raise ValueError("Missing causal model ledger value: performance_net_strategy_return")
        return value
    return parse_float_maybe(
        first_present_runtime_value(
...
def dashboard_model_return_gross_from_source_row(source_row: dict[str, Any]) -> float | None:
    if source_row.get("performance_contract") == "causal_execution_interval_ledger_v1":
        value = parse_float_maybe(source_row.get("performance_gross_strategy_return"))
        if value is None:
            raise ValueError("Missing causal model ledger value: performance_gross_strategy_return")
        return value
    return parse_float_maybe(
        first_present_runtime_value(
...
def build_dashboard_public_chart_timeseries_contract(
    *,
    ...
    if dashboard_public_status.get("performance_contract") == "causal_execution_interval_ledger_v1":
        if not production_timeseries_rows or any(row.get("performance_contract") != "causal_execution_interval_ledger_v1" for row in production_timeseries_rows):
            raise ValueError("Causal dashboard cannot consume legacy same-day performance")
    real_account = (
...
def build_dashboard_public_status_contract(
    *,
    ...
    if "resolved_execution_asset" in production_snapshot_payload:
        from scripts.production.route_identity import validate_target_identity
        validate_target_identity(production_snapshot_payload)
        preferred_asset = production_snapshot_payload["resolved_execution_asset"]
        model_exposure = float(execution_intent["target_exposure"])
    decision_context = (
    ...
    if production_snapshot_payload.get("performance_contract") == "causal_execution_interval_ledger_v1":
        top_performance_metrics = dict(production_snapshot_payload["metrics"])
        main_strategy_metrics = dict(top_performance_metrics)
    ...
    return {
        ...
        "closed_day": closed_day,
        "performance_contract": production_snapshot_payload.get("performance_contract"),
    ...
    return {
        ...
        "source_metadata": {
            "strategy_freshness": {
                ...
                "real_account_performance": source_metadata(REAL_PERFORMANCE_LEDGER_PATH, "real_account_performance_ledger"),
            },
...
def materialize_production_core_dependency_artifacts() -> dict[str, Any]:
    ...
    if (ROOT / "source_of_truth/production_route_identity_contract.json").exists():
        from scripts.production.strategy_adapters.causal_route_adapter import build_route_inputs
        route_inputs = build_route_inputs(ROOT)
        report_rows.append({"artifact_key": "causal_route_dependencies", "status": "source_validated",
                            "closed_day": route_inputs["closed_day"], "legacy_same_day_rebuild": "not_required"})
    else:
        phase68g_refresh_info = refresh_phase68g_native_outputs_if_needed()
        phase68g_source_export_row = read_single_csv_row(PHASE68G_SOURCE_AUTHORITATIVE_EXPORT_PATH)
        phase68g_canonical_metrics_row = build_full_canonical_main_strategy_metrics_row(
