...
def event_from_fill(fill: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "deposits_usd": 0.0,
        "withdrawals_usd": 0.0,
        "source": "Hyperliquid userFills.closedPnl/fee",
        "raw": fill,
    }
...
def event_from_non_funding(update: dict[str, Any], account_address: str) -> dict[str, Any] | None:
    ...
    deposits = amount if kind == "deposit" or incoming_send else 0.0
    withdrawals = amount if kind == "withdraw" or outgoing_send else 0.0
    return {
...
def extract_events(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for fill in raw.get("userFills", []) if isinstance(raw.get("userFills"), list) else []:
...
def merge_valuations(existing: list[dict[str, Any]], snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    point = {
        ...
        "effective_day": effective_day(timestamp),
        "equity_usd": equity,
        "free_collateral_usd": as_float(summary.get("available_balance_usd")),
        "margin_used_usd": as_float((summary.get("margin_summary") or {}).get("totalMarginUsed")),
...
def build_ledger(snapshot: dict[str, Any], existing: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_at_utc": utc_now_iso(), "source_complete": all(key in latest_raw for key in ("userFills", "userFunding", "userNonFundingLedgerUpdates")),
        "missing_sources": [key for key in ("userFills", "userFunding", "userNonFundingLedgerUpdates") if key not in latest_raw],
        ...
        "windows": windows,
        "source_fields": {
            "equity": "clearinghouseState.marginSummary.accountValue or spotClearinghouseState stable balance", "external_cash_flows": "userNonFundingLedgerUpdates.delta.type/usdc", "fills_fees_realized": "userFills.closedPnl/fee", "funding": "userFunding.usdc", "unrealized": "clearinghouseState.assetPositions[].position.unrealizedPnl"
        },
    }
