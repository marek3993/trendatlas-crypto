...
def detect_selected_asset_column(df: pd.DataFrame) -> str | None:
    ...
    for col in df.columns:
        lc = str(col).lower()
        if any(x in lc for x in ["symbol", "asset", "leader", "pick", "holding"]):
            return col
    return None
...
def discover_base_strategy_file(key: str) -> Path:
    ...
    for path in ranked[:100]:
        ...
        try:
            ...
            ret_col = detect_return_column(df)
            eq_col = detect_equity_column(df)
            if ret_col or eq_col:
                log(f"[DISCOVER] base strategy file: {path}")
                return path
        except Exception:
            continue
...
def discover_phase_summary_file() -> Path | None:
    files = discover_candidate_files()
    ranked = []
    for path in files:
        name = path.name.lower()
        full = str(path).lower()
        score = 0
        if "phase61" in full and "compare" in full:
            score += 50
        if "summary" in name:
            score += 25
        if "final" in name:
        ...
        if path.suffix.lower() in {".csv", ".parquet", ".pq"}:
            score += 5
        ranked.append((score, -len(full), path))
...
def load_base_strategy(path: Path) -> pd.DataFrame:
    ...
    if eq_col is not None:
        out["base_equity"] = pd.to_numeric(raw[eq_col], errors="coerce").ffill().bfill()
    else:
        out["base_equity"] = (1.0 + out["base_return"]).cumprod()
...
def build_variant_grid() -> list[VariantConfig]:
                            ...
                            for vol_lb in vol_lbs:
                                for vol_cap in vol_caps:
                                    for cooldown in cooldowns:
                                        name = (
                                            f"phase62_btcov_f{fast}_s{slow}_r{ret_lb}"
    ...
    for v in [
        ...
        VariantConfig(
            name="phase62_btcov_tighter_risk",
            btc_fast_ma=20,
            btc_slow_ma=150,
            btc_ret_lb=20,
            btc_ret_min=0.06,
...
def compute_regime_columns(df: pd.DataFrame, cfg: VariantConfig) -> pd.DataFrame:
    ...
    out["btc_led_raw"] = out["btc_trend_ok"] & (~out["risk_off"])

    if cfg.cooldown_days > 0:
        raw = out["btc_led_raw"].fillna(False).astype(bool).values
        ...
        out["btc_led"] = out["btc_led_raw"].fillna(False)
...
def simulate_variant(base_input: pd.DataFrame, cfg: VariantConfig) -> pd.DataFrame:
    ...
    df["signal_regime"] = np.where(
        ...
        "CASH",
        np.where(df["btc_led"].fillna(False), "BTC", "BASE"),
    )
...
def try_load_compare_summary() -> pd.DataFrame | None:
    path = discover_phase_summary_file()
    if path is None:
        return None
    try:
        return normalize_columns(safe_read_table(path))
    except Exception:
        return None


def extract_baseline_row_from_summary(summary_df: pd.DataFrame, baseline_key: str) -> dict | None:
    if summary_df is None or summary_df.empty:
        return None

    lower_cols = {c.lower(): c for c in summary_df.columns}
    possible_cols = [lower_cols.get("model"), lower_cols.get("label")]
    possible_cols = [c for c in possible_cols if c]
    if not possible_cols:
        possible_cols = list(summary_df.columns[:2])

    mask = pd.Series(False, index=summary_df.index)
    for col in possible_cols:
        mask |= summary_df[col].astype(str).str.lower().str.contains(baseline_key.lower(), regex=False)
    ...
    return {
        "model": row.get("model", baseline_key),
        "label": row.get("label", baseline_key),
        ...
        "cash_days_pct": row.get("cash_days_pct"),
        "since2021_total_return_pct": row.get("since2021_total_return_pct"),
        "since2021_cagr_pct": row.get("since2021_cagr_pct"),
        "since2021_max_drawdown_pct": row.get("since2021_max_drawdown_pct"),
        "since2023_total_return_pct": row.get("since2023_total_return_pct"),
        "since2023_cagr_pct": row.get("since2023_cagr_pct"),
        "since2023_max_drawdown_pct": row.get("since2023_max_drawdown_pct"),
        "since2025_total_return_pct": row.get("since2025_total_return_pct"),
        "since2025_cagr_pct": row.get("since2025_cagr_pct"),
        "since2025_max_drawdown_pct": row.get("since2025_max_drawdown_pct"),
    }
...
def add_delta(df: pd.DataFrame, baseline_model: str) -> pd.DataFrame:
    ...
    for metric in ["cagr_pct", "max_drawdown_pct", "since2021_cagr_pct", "since2023_cagr_pct", "since2025_cagr_pct"]:
        if metric in out.columns:
            out[f"delta_vs_{baseline_model}_{metric}"] = pd.to_numeric(out[metric], errors="coerce") - pd.to_numeric(base.get(metric), errors="coerce")
    return out


def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE62 BTC leadership overlay")
    ...
    log("[PHASE62] Start")
    log(f"[PHASE62] Winner key: {args.winner_key}")
    log(f"[PHASE62] Baseline key: {args.baseline_key}")

    base_file = discover_base_strategy_file(args.winner_key)
    ...
    for i, cfg in enumerate(variants, start=1):
        try:
            ...
            if i % 25 == 0:
                log(f"[PHASE62] done {i}/{len(variants)}")
    ...
    compare_summary = try_load_compare_summary()
    baseline_row = extract_baseline_row_from_summary(compare_summary, args.baseline_key)

    summary["source"] = np.where(summary["model"].eq(args.winner_key), "phase62_rebuilt_base", "phase62_overlay")

    if baseline_row is not None:
        baseline_df = pd.DataFrame([baseline_row])
        baseline_df["source"] = "phase61_compare_summary"
        combined_compare = pd.concat([baseline_df, summary.head(20)], ignore_index=True, sort=False)
    else:
        combined_compare = summary.head(20).copy()

    combined_compare = add_delta(combined_compare, args.baseline_key)
    combined_compare = add_delta(combined_compare, args.winner_key)

    summary_path = PHASE62_DIR / "phase62_overlay_summary.csv"
    compare_path = PHASE62_DIR / "phase62_overlay_top_compare.csv"
    manifest_path = PHASE62_DIR / "phase62_manifest.json"
    ...
    for model in top_models:
        ...
        if paper is None:
            continue
        out_path = PHASE62_DIR / f"{model}_paper.csv"
    ...
    manifest = {
        ...
        "notes": [
            ...
            "CASH = nulový denný return",
            "Priorita signálu: risk_off -> BTC-led -> BASE",
            "Exekúcia je posunutá o 1 deň, bez lookahead bias",
        ],
    }
    ...
    log("")
    log(f"[PHASE62] Saved summary -> {summary_path}")
    log(f"[PHASE62] Saved compare -> {compare_path}")
    log(f"[PHASE62] Saved manifest -> {manifest_path}")
