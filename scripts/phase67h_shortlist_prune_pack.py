...
PHASE67H_DIR = OUTPUTS / "phase67h_shortlist_prune_pack"
    ...
    return [
        (
            ...
            WeeklyGovernanceConfig(
                profile_name="phase67h_keep_all_67f_main",
                **common_governance,
            ),
            [],
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67h_drop_neo",
                **common_governance,
            ),
            ["NEO"],
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67h_drop_bch",
                **common_governance,
            ),
            ["BCH"],
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67h_drop_xtz",
                **common_governance,
            ),
            ["XTZ"],
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67h_drop_neo_xtz",
                **common_governance,
            ),
            ["NEO", "XTZ"],
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67h_drop_bch_xtz",
                **common_governance,
            ),
            ["BCH", "XTZ"],
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE67H shortlist prune pack over 66G core")
    ...
    log("[PHASE67H] Start")
    log(f"[PHASE67H] Core paper: {args.core_paper}")
    log(f"[PHASE67H] Full shortlist assets: {[x['asset'] for x in full_shortlist]}")
    ...
    for overlay_cfg, gov_cfg, drops in profiles:
        ...
        for item in shortlist:
            ...
            try:
                ...
                failed_assets.append({"profile": gov_cfg.profile_name, "asset": asset, "reason": str(e), "dropped_assets": ",".join(drops)})
        ...
        log(f"[PHASE67H] done {gov_cfg.profile_name} | drops={drops}")
    ...
    summary_path = PHASE67H_DIR / "phase67h_shortlist_prune_summary.csv"
    compare_path = PHASE67H_DIR / "phase67h_shortlist_prune_compare.csv"
    decisions_path = PHASE67H_DIR / "phase67h_shortlist_prune_decisions.csv"
    leaderboard_path = PHASE67H_DIR / "phase67h_shortlist_prune_leaderboard.csv"
    asset_quality_path = PHASE67H_DIR / "phase67h_shortlist_prune_asset_quality.csv"
    live_status_path = PHASE67H_DIR / "phase67h_live_status.csv"
    latest_top10_path = PHASE67H_DIR / "phase67h_latest_decision_top10.csv"
    manifest_path = PHASE67H_DIR / "phase67h_manifest.json"
    ...
    manifest = {
        ...
        "notes": [
            "67H je shortlist prune pack nad 67F main governance baseline.",
            "Testuje drop NEO/BCH/XTZ a 2-asset prune varianty.",
            "Cieľ: zachovať since2025 a skúsiť stlačiť DD bez rozbitia CAGR.",
        ],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    log("")
    log("=== PHASE67H TOP RESULT ===")
    if best.empty:
        log("No phase67h profile processed.")
    else:
        row = best.iloc[0]
    ...
    log(f"[PHASE67H] Saved summary -> {summary_path}")
    log(f"[PHASE67H] Saved compare -> {compare_path}")
    log(f"[PHASE67H] Saved decisions -> {decisions_path}")
    log(f"[PHASE67H] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE67H] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE67H] Saved live status -> {live_status_path}")
    log(f"[PHASE67H] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE67H] Saved manifest -> {manifest_path}")
