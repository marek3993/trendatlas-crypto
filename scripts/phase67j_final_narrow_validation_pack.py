...
def normalize_public_asset(position: object, regime: object) -> str:
    pos = str(position).strip().upper()
    reg = str(regime).strip().upper()

    if pos and pos not in {"", "NA", "NAN", "NONE"}:
        return pos
    ...
    if reg == "BTC":
        return "BTC"
    if reg in {"BASE", "CANDIDATE"}:
        return pos if pos and pos not in {"", "NA", "NAN", "NONE"} else reg

    return "CASH"


def build_execution_state(current_asset: str, held_asset_public: str, executed_regime: str) -> str:
    cur = str(current_asset).strip().upper()
    held = str(held_asset_public).strip().upper()
    reg = str(executed_regime).strip().upper()

    if held == "CASH" or reg == "CASH":
        return "CASH"

    if cur and cur not in {"", "CORE"}:
        return "CHALLENGER_ACTIVE"

    return "CORE_ACTIVE"


def build_held_state_label(held_asset_public: str, execution_state: str) -> str:
    held = str(held_asset_public).strip().upper()
...
def simulate_weekly_challenger_governance(
    ...
    for i, decision_date in enumerate(rebalance_dates):
        ...
        for asset in assets:
            ...
            required_recent_delta = gov_cfg.promotion_margin_pct + (
                gov_cfg.bnb_shield_margin_pct if core_is_bnb else 0.0
            )
        ...
        if eval_rows:
            ...
            leaderboard = pd.DataFrame(
                columns=[
                    "asset",
                    "train_total_delta_pct",
                    "recent_total_delta_pct",
                    "train_dd_worsen_pct",
                    "downside_dd_worsen_pct",
                    "train_triggers",
                    "score",
                    "passed_filters",
                    "profile",
        ...
        best_asset = ""
        best_meta: dict = {}
        passed_df = leaderboard[leaderboard["passed_filters"] == True] if not leaderboard.empty else pd.DataFrame()
        ...
        decision_rows.append(
            {
                ...
                "required_recent_delta_pct": gov_cfg.promotion_margin_pct
                + (gov_cfg.bnb_shield_margin_pct if core_is_bnb else 0.0),
            }
        )
    ...
    governance["equity"] = (
        1.0 + pd.to_numeric(governance["strategy_return"], errors="coerce").fillna(0.0)
    ).cumprod()
    ...
    return [
        (
            ...
            WeeklyGovernanceConfig(
                profile_name="phase67j_no_neo_main",
                **common_governance,
            ),
            ["NEO"],
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67j_no_neo_drop_bch",
                **common_governance,
            ),
            ["NEO", "BCH"],
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67j_no_neo_drop_xtz",
                **common_governance,
            ),
            ["NEO", "XTZ"],
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67j_no_neo_drop_bch_xtz",
                **common_governance,
            ),
            ["NEO", "BCH", "XTZ"],
        ),
...
def print_compare_rows(compare: pd.DataFrame) -> None:
    if compare.empty:
        return
    log("")
    log("=== PHASE67J ALL PROFILE RESULTS ===")
    for _, row in compare.iterrows():
        log(
            f"{row['model']} | drops={row['dropped_assets']} | "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE67J final narrow validation pack")
    ...
    log("[PHASE67J] Start")
    log(f"[PHASE67J] Core paper: {args.core_paper}")
    log(f"[PHASE67J] Full shortlist assets: {[x['asset'] for x in full_shortlist]}")
    ...
    for overlay_cfg, gov_cfg, drops in profiles:
        ...
        row["unique_selected_assets"] = int(
            governance["chosen_asset"].astype(str).replace("", np.nan).dropna().nunique()
        )
        ...
        log(f"[PHASE67J] done {gov_cfg.profile_name} | drops={drops}")
    ...
    if best_model and best_model in papers:
        ...
        latest_executed_position = (
            str(best_paper["executed_position"].astype(str).iloc[-1]) if len(best_paper) else ""
        )
        latest_executed_regime = (
            str(best_paper["executed_regime"].astype(str).iloc[-1]) if len(best_paper) else ""
        )

        held_asset_public = normalize_public_asset(latest_executed_position, latest_executed_regime)
        execution_state = build_execution_state(current_asset, held_asset_public, latest_executed_regime)
        held_state_label = build_held_state_label(held_asset_public, execution_state)

        best_decisions = (
            decisions_out[decisions_out["profile"].astype(str) == best_model].copy()
            if not decisions_out.empty else pd.DataFrame()
        )
        next_rebalance_date = compute_next_rebalance_date(
            best_decisions,
            best_gov_cfg.rebalance_every_days if best_gov_cfg else 7,
        )
        ...
        live_status = pd.DataFrame(
            [{
                ...
                "held_state_label": held_state_label,
        ...
        best_leaderboard = (
            leaderboards_out[leaderboards_out["profile"].astype(str) == best_model].copy()
            if not leaderboards_out.empty else pd.DataFrame()
        )
    ...
    summary_path = PHASE67J_DIR / "phase67j_final_narrow_validation_summary.csv"
    compare_path = PHASE67J_DIR / "phase67j_final_narrow_validation_compare.csv"
    decisions_path = PHASE67J_DIR / "phase67j_final_narrow_validation_decisions.csv"
    leaderboard_path = PHASE67J_DIR / "phase67j_final_narrow_validation_leaderboard.csv"
    ...
    log("")
    log("=== PHASE67J TOP RESULT ===")
    if best.empty:
        log("No phase67j profile processed.")
    else:
        row = best.iloc[0]
    ...
    if not live_status.empty:
        ...
        log("")
        log("=== PHASE67J LIVE STATUS ===")
        log(f"latest_available_date: {ls['latest_available_date']}")
        log(f"current_asset: {ls['current_asset']}")
        log(f"held_asset_public: {ls['held_asset_public']}")
        log(f"held_state_label: {ls['held_state_label']}")
        log(f"execution_state: {ls['execution_state']}")
    ...
    if not best.empty:
        row = best.iloc[0]
        log("=== PHASE67J VERDICT ===")
        log(
            f"Best final narrow candidate so far: {row['model']} | "
            f"drops={row['dropped_assets']} | shortlist={row['final_shortlist']} | "
            f"CAGR={pd.to_numeric(row['cagr_pct'], errors='coerce'):.2f} | "
            f"DD={pd.to_numeric(row['max_drawdown_pct'], errors='coerce'):.2f} | "
            f"since2023={pd.to_numeric(row['since2023_cagr_pct'], errors='coerce'):.2f} | "
            f"since2025={pd.to_numeric(row['since2025_cagr_pct'], errors='coerce'):.2f}"
    ...
    log("")
    log(f"[PHASE67J] Saved summary -> {summary_path}")
    log(f"[PHASE67J] Saved compare -> {compare_path}")
    log(f"[PHASE67J] Saved decisions -> {decisions_path}")
    log(f"[PHASE67J] Saved leaderboard -> {leaderboard_path}")
