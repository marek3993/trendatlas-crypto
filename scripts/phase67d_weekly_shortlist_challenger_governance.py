...
PHASE67D_DIR = OUTPUTS / "phase67d_weekly_shortlist_challenger_governance"
...
def build_asset_overlay_strategy(core_df: pd.DataFrame, candidate_daily: pd.DataFrame, cfg: ChallengerOverlayConfig, asset: str) -> pd.DataFrame:
    ...
    return [
        (
            ...
            WeeklyGovernanceConfig(
                profile_name="phase67d_weekly_challenger_base",
        ...
        (
            ...
            WeeklyGovernanceConfig(
                profile_name="phase67d_weekly_challenger_strict",
                trailing_train_days=365,
                recent_days=60,
                rebalance_every_days=7,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE67D weekly shortlist challenger governance over 66G core")
    ...
    log("[PHASE67D] Start")
    log(f"[PHASE67D] Core paper: {args.core_paper}")
    log(f"[PHASE67D] Shortlist assets: {[x['asset'] for x in shortlist]}")
    ...
    for overlay_cfg, gov_cfg in profiles:
        ...
        log(f"[PHASE67D] done {gov_cfg.profile_name}")
    ...
    summary_path = PHASE67D_DIR / "phase67d_weekly_shortlist_challenger_summary.csv"
    compare_path = PHASE67D_DIR / "phase67d_weekly_shortlist_challenger_compare.csv"
    decisions_path = PHASE67D_DIR / "phase67d_weekly_shortlist_challenger_decisions.csv"
    leaderboard_path = PHASE67D_DIR / "phase67d_weekly_shortlist_challenger_leaderboard.csv"
    asset_quality_path = PHASE67D_DIR / "phase67d_weekly_shortlist_challenger_asset_quality.csv"
    live_status_path = PHASE67D_DIR / "phase67d_live_status.csv"
    latest_top10_path = PHASE67D_DIR / "phase67d_latest_decision_top10.csv"
    manifest_path = PHASE67D_DIR / "phase67d_manifest.json"
    ...
    for model, paper in papers.items():
        out_path = PHASE67D_DIR / f"{model}_paper.csv"
        paper.reset_index().rename(columns={paper.index.name or "index": "date"}).to_csv(out_path, index=False)

    manifest = {
        "phase": "phase67d_weekly_shortlist_challenger_governance",
        ...
        "notes": [
            "67C daily freerotate je nahradený weekly challenger governance nad 66G core.",
            "Shortlist asset sa zapne len keď dlhšie poráža 66G core.",
            "Cieľ: zachovať 2025 a pritom skúsiť zlepšiť full history / since2023.",
        ],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    log("")
    log("=== PHASE67D TOP RESULT ===")
    if best.empty:
        log("No weekly challenger profile processed.")
    else:
        row = best.iloc[0]
    ...
    log(f"[PHASE67D] Saved summary -> {summary_path}")
    log(f"[PHASE67D] Saved compare -> {compare_path}")
    log(f"[PHASE67D] Saved decisions -> {decisions_path}")
    log(f"[PHASE67D] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE67D] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE67D] Saved live status -> {live_status_path}")
    log(f"[PHASE67D] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE67D] Saved manifest -> {manifest_path}")
