...
def load_manifest() -> dict:
    path = PHASE63_DIR / "phase63_manifest.json"
    if not path.exists():
...
def load_summary() -> pd.DataFrame:
    path = PHASE63_DIR / "phase63_overlay_summary.csv"
    if not path.exists():
...
def load_compare() -> pd.DataFrame:
    path = PHASE63_DIR / "phase63_overlay_top_compare.csv"
    if not path.exists():
...
def model_paper_path(model: str) -> Path:
    return PHASE63_DIR / f"{model}_paper.csv"
...
def build_lagged_returns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    x["lag1_strategy_return"] = np.where(
        ...
        x["btc_return"],
        np.where(x["lag1_regime"].eq("CASH"), 0.0, x["base_return"]),
    )
...
def main() -> None:
    ...
    log(f"[PHASE63 FORENSIC] Models: {models}")
    ...
    forensic_path = PHASE63_DIR / "phase63_forensic_check.csv"
    forensic.to_csv(forensic_path, index=False)

    out_manifest = {
        "phase": "phase63_forensic_check",
    ...
    out_manifest_path = PHASE63_DIR / "phase63_forensic_manifest.json"
    ...
    log("")
    log("=== PHASE63 FORENSIC TOP VIEW ===")
    print(forensic[cols].to_string(index=False))

    bad = forensic[forensic["error"].astype(str).str.len() > 0].copy()
    if not bad.empty:
        log("")
        log("=== PHASE63 FORENSIC ERRORS ===")
        print(bad[["model", "error"]].to_string(index=False))

    log("")
    log(f"[PHASE63 FORENSIC] Saved -> {forensic_path}")
    log(f"[PHASE63 FORENSIC] Saved -> {out_manifest_path}")
