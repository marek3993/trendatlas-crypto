...
PHASE66K_DIR = OUTPUTS / "phase66k_core_volatility_gate_sweep"


@dataclass(frozen=True)
class VolatilityVariant:
    model: str
    candidate_vol_lb_delta: int
    candidate_vol_cap_bump: float


VARIANTS: list[VolatilityVariant] = [
    VolatilityVariant(
        model="phase66k_core_volgate_A",
        candidate_vol_lb_delta=-5,
        candidate_vol_cap_bump=-0.02,
    ),
    VolatilityVariant(
        model="phase66k_core_volgate_B",
        candidate_vol_lb_delta=0,
        candidate_vol_cap_bump=-0.02,
    ),
    VolatilityVariant(
        model="phase66k_core_volgate_C",
        candidate_vol_lb_delta=5,
        candidate_vol_cap_bump=-0.02,
    ),
    VolatilityVariant(
        model="phase66k_core_volgate_D",
        candidate_vol_lb_delta=-5,
        candidate_vol_cap_bump=-0.04,
    ),
    VolatilityVariant(
        model="phase66k_core_volgate_E",
        candidate_vol_lb_delta=0,
        candidate_vol_cap_bump=-0.04,
    ),
    VolatilityVariant(
        model="phase66k_core_volgate_F",
        candidate_vol_lb_delta=5,
        candidate_vol_cap_bump=-0.04,
    ),
]
...
def clamp_float(value: float, minimum: float = 0.0001) -> float:
...
def build_overlay_config(base_cfg: object, variant: VolatilityVariant) -> object:
    ...
    base_vol_lb = int(getattr(base_cfg, "candidate_vol_lb"))
    base_vol_cap = float(getattr(base_cfg, "candidate_vol_cap"))

    cfg.candidate_vol_lb = clamp_int(base_vol_lb + variant.candidate_vol_lb_delta, 5)
    cfg.candidate_vol_cap = clamp_float(base_vol_cap + variant.candidate_vol_cap_bump, 0.0001)

    # 66K: ostatné overlay fieldy nechávame presne na baseline 66G
    cfg.candidate_fast_ma = int(getattr(base_cfg, "candidate_fast_ma"))
    cfg.candidate_slow_ma = int(getattr(base_cfg, "candidate_slow_ma"))
    cfg.candidate_ret_lb = int(getattr(base_cfg, "candidate_ret_lb"))
    cfg.candidate_risk_ma = int(getattr(base_cfg, "candidate_risk_ma"))
...
def run_variant(
    ...
    row["candidate_vol_lb"] = int(getattr(overlay_cfg, "candidate_vol_lb"))
    row["candidate_vol_cap"] = float(getattr(overlay_cfg, "candidate_vol_cap"))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE66K core volatility gate sweep over phase66e overlay config")
    ...
    log("[PHASE66K] Start")
    log(f"[PHASE66K] Baseline paper: {args.baseline_paper}")
    log(f"[PHASE66K] Phase66G summary ref: {args.phase66g_summary}")
    log(f"[PHASE66K] Remove assets: {remove_assets}")
    log(
        "[PHASE66K] Fixed baseline non-vol fields | "
        f"candidate_fast_ma={getattr(base_overlay_cfg, 'candidate_fast_ma')} "
        f"candidate_slow_ma={getattr(base_overlay_cfg, 'candidate_slow_ma')} "
        f"candidate_ret_lb={getattr(base_overlay_cfg, 'candidate_ret_lb')} "
        f"candidate_risk_ma={getattr(base_overlay_cfg, 'candidate_risk_ma')} "
    ...
    log(f"[PHASE66K] Candidate assets cached: {len(asset_daily_cache)}")
    ...
    for variant in VARIANTS:
        log(
            f"[PHASE66K] running {variant.model} | "
            f"candidate_vol_lb_delta={variant.candidate_vol_lb_delta:+d} "
            f"candidate_vol_cap_bump={variant.candidate_vol_cap_bump:+.2f}"
        )
    ...
    summary_path = PHASE66K_DIR / "phase66k_core_volatility_summary.csv"
    compare_path = PHASE66K_DIR / "phase66k_core_volatility_compare.csv"
    live_status_path = PHASE66K_DIR / "phase66k_live_status.csv"
    decisions_path = PHASE66K_DIR / "phase66k_core_volatility_decisions.csv"
    leaderboard_path = PHASE66K_DIR / "phase66k_core_volatility_leaderboard.csv"
    latest_top10_path = PHASE66K_DIR / "phase66k_latest_decision_top10.csv"
    suspended_now_path = PHASE66K_DIR / "phase66k_suspended_assets_now.csv"
    asset_quality_path = PHASE66K_DIR / "phase66k_core_volatility_asset_quality.csv"
    asset_usage_path = PHASE66K_DIR / "phase66k_core_volatility_asset_usage.csv"
    failed_assets_path = PHASE66K_DIR / "phase66k_core_volatility_failed_assets.csv"
    baseline_copy_path = PHASE66K_DIR / f"{CURRENT_WINNER_KEY}_paper.csv"
    manifest_path = PHASE66K_DIR / "phase66k_manifest.json"
    ...
    manifest = {
        "phase": "phase66k_core_volatility_gate_sweep",
        ...
        "fixed_overlay_fields_from_base": {
            "candidate_fast_ma": int(getattr(base_overlay_cfg, "candidate_fast_ma")),
            "candidate_slow_ma": int(getattr(base_overlay_cfg, "candidate_slow_ma")),
            "candidate_ret_lb": int(getattr(base_overlay_cfg, "candidate_ret_lb")),
            "candidate_risk_ma": int(getattr(base_overlay_cfg, "candidate_risk_ma")),
        ...
        "swept_overlay_fields": {
            "candidate_vol_lb_base": int(getattr(base_overlay_cfg, "candidate_vol_lb")),
            "candidate_vol_cap_base": float(getattr(base_overlay_cfg, "candidate_vol_cap")),
        },
        ...
        "notes": [
            "Phase66K nemení GovernanceConfig ani universe/drop logic.",
            "Phase66K nemení timing, permission ani weak-base fieldy oproti baseline 66G.",
            "Sweepuje len candidate_vol_lb a candidate_vol_cap.",
    ...
    if top_row is not None:
        ...
        log(f"candidate_vol_lb: {int(pd.to_numeric(top_row['candidate_vol_lb'], errors='coerce'))}")
        log(f"candidate_vol_cap: {float(top_row['candidate_vol_cap']):.4f}")
    ...
    log(f"[PHASE66K] Saved summary -> {summary_path}")
    log(f"[PHASE66K] Saved compare -> {compare_path}")
    log(f"[PHASE66K] Saved live status -> {live_status_path}")
    log(f"[PHASE66K] Saved decisions -> {decisions_path}")
    log(f"[PHASE66K] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE66K] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE66K] Saved suspended now -> {suspended_now_path}")
    log(f"[PHASE66K] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE66K] Saved asset usage -> {asset_usage_path}")
    log(f"[PHASE66K] Saved failed assets -> {failed_assets_path}")
    log(f"[PHASE66K] Saved baseline paper -> {baseline_copy_path}")
    log(f"[PHASE66K] Saved manifest -> {manifest_path}")
