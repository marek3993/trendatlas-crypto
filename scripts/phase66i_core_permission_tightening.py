...
PHASE66I_DIR = OUTPUTS / "phase66i_core_permission_tightening"
...
VARIANTS: list[PermissionVariant] = [
    PermissionVariant(
        model="phase66i_core_permission_A",
        candidate_risk_buffer_bump=0.05,
        candidate_ret_min_bump=0.02,
    ),
    PermissionVariant(
        model="phase66i_core_permission_B",
        candidate_risk_buffer_bump=0.10,
        candidate_ret_min_bump=0.02,
    ),
    PermissionVariant(
        model="phase66i_core_permission_C",
        candidate_risk_buffer_bump=0.10,
        candidate_ret_min_bump=0.05,
    ),
    PermissionVariant(
        model="phase66i_core_permission_D",
        candidate_risk_buffer_bump=0.15,
        candidate_ret_min_bump=0.05,
    ),
    PermissionVariant(
        model="phase66i_core_permission_E",
        candidate_risk_buffer_bump=0.05,
        candidate_ret_min_bump=0.05,
    ),
    PermissionVariant(
        model="phase66i_core_permission_F",
        candidate_risk_buffer_bump=0.15,
...
def build_overlay_config(base_cfg: object, variant: PermissionVariant) -> object:
    ...
    # 66I: weak_base / cooldown nechávame presne na baseline 66G
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE66I core permission tightening sweep over phase66e overlay config")
    ...
    log("[PHASE66I] Start")
    log(f"[PHASE66I] Baseline paper: {args.baseline_paper}")
    log(f"[PHASE66I] Phase66G summary ref: {args.phase66g_summary}")
    log(f"[PHASE66I] Remove assets: {remove_assets}")
    log(
        "[PHASE66I] Fixed baseline overlay fields | "
    ...
    log(f"[PHASE66I] Candidate assets cached: {len(asset_daily_cache)}")
    ...
    for variant in VARIANTS:
        log(
            f"[PHASE66I] running {variant.model} | "
    ...
    summary_path = PHASE66I_DIR / "phase66i_core_permission_summary.csv"
    compare_path = PHASE66I_DIR / "phase66i_core_permission_compare.csv"
    live_status_path = PHASE66I_DIR / "phase66i_live_status.csv"
    decisions_path = PHASE66I_DIR / "phase66i_core_permission_decisions.csv"
    leaderboard_path = PHASE66I_DIR / "phase66i_core_permission_leaderboard.csv"
    latest_top10_path = PHASE66I_DIR / "phase66i_latest_decision_top10.csv"
    suspended_now_path = PHASE66I_DIR / "phase66i_suspended_assets_now.csv"
    asset_quality_path = PHASE66I_DIR / "phase66i_core_permission_asset_quality.csv"
    asset_usage_path = PHASE66I_DIR / "phase66i_core_permission_asset_usage.csv"
    failed_assets_path = PHASE66I_DIR / "phase66i_core_permission_failed_assets.csv"
    baseline_copy_path = PHASE66I_DIR / f"{CURRENT_WINNER_KEY}_paper.csv"
    manifest_path = PHASE66I_DIR / "phase66i_manifest.json"
    ...
    manifest = {
        "phase": "phase66i_core_permission_tightening",
        ...
        "swept_overlay_fields": {
            "candidate_risk_buffer_base": float(getattr(base_overlay_cfg, "candidate_risk_buffer")),
            "candidate_ret_min_base": float(getattr(base_overlay_cfg, "candidate_ret_min")),
        },
        ...
        "notes": [
            "Phase66I nemení GovernanceConfig ani universe/drop logic.",
            "Phase66I nemení weak_base_lb, weak_base_threshold ani cooldown_days oproti baseline 66G.",
            "Sweepuje len candidate_risk_buffer a candidate_ret_min.",
    ...
    if top_row is not None:
        ...
        log(f"candidate_risk_buffer: {float(top_row['candidate_risk_buffer']):.4f}")
        log(f"candidate_ret_min: {float(top_row['candidate_ret_min']):.4f}")
    ...
    log(f"[PHASE66I] Saved summary -> {summary_path}")
    log(f"[PHASE66I] Saved compare -> {compare_path}")
    log(f"[PHASE66I] Saved live status -> {live_status_path}")
    log(f"[PHASE66I] Saved decisions -> {decisions_path}")
    log(f"[PHASE66I] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE66I] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE66I] Saved suspended now -> {suspended_now_path}")
    log(f"[PHASE66I] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE66I] Saved asset usage -> {asset_usage_path}")
    log(f"[PHASE66I] Saved failed assets -> {failed_assets_path}")
    log(f"[PHASE66I] Saved baseline paper -> {baseline_copy_path}")
    log(f"[PHASE66I] Saved manifest -> {manifest_path}")
