...
PHASE67K_DIR = OUTPUTS / "phase67k_no_neo_2025_guardrails"
...
class WeeklyGovernanceConfig:
    ...
    # 67K hardening
    anti_flip_lookback_periods: int
    anti_flip_max_switches: int
    anti_flip_block_periods: int
    stricter_entry_train_delta_pct: float
    stricter_entry_recent_delta_pct: float
    stricter_entry_score_margin: float
    bnb_deny_recent_delta_below_pct: float
    bnb_deny_train_delta_below_pct: float
...
def simulate_weekly_challenger_governance(
    ...
    for i, decision_date in enumerate(rebalance_dates):
        ...
        recent_switches = sum(1 for x in switch_rebalance_idxs if x > i - gov_cfg.anti_flip_lookback_periods)
        anti_flip_active = recent_switches >= gov_cfg.anti_flip_max_switches or i <= anti_flip_block_until
        ...
        for asset in assets:
            ...
            required_train_delta = gov_cfg.stricter_entry_train_delta_pct
            required_recent_entry_delta = max(gov_cfg.stricter_entry_recent_delta_pct, required_recent_delta)

            promotion_ok = float(meta["recent_total_delta_pct"]) >= required_recent_delta
            strict_entry_ok = (
                float(meta["train_total_delta_pct"]) >= required_train_delta
                and float(meta["recent_total_delta_pct"]) >= required_recent_entry_delta
                and float(meta["score"]) >= gov_cfg.stricter_entry_score_margin
            )

            bnb_pattern_deny = False
            if core_is_bnb:
                if (
                    float(meta["recent_total_delta_pct"]) < gov_cfg.bnb_deny_recent_delta_below_pct
                    or float(meta["train_total_delta_pct"]) < gov_cfg.bnb_deny_train_delta_below_pct
                ):
                    bnb_pattern_deny = True

            anti_flip_deny = anti_flip_active and asset != incumbent_asset

            if suspended or reentry_blocked or (not persistence_ok) or (not promotion_ok) or (not strict_entry_ok) or bnb_pattern_deny or anti_flip_deny:
            ...
            meta["strict_entry_ok"] = strict_entry_ok
            meta["bnb_pattern_deny"] = bnb_pattern_deny
            meta["anti_flip_active"] = anti_flip_active
            meta["anti_flip_deny"] = anti_flip_deny
            meta["recent_switches_in_window"] = recent_switches
        ...
        if incumbent_asset and selected_asset != incumbent_asset:
            switch_rebalance_idxs.append(i)
            if gov_cfg.anti_flip_block_periods > 0:
                anti_flip_block_until = i + gov_cfg.anti_flip_block_periods
        ...
        decision_rows.append(
            {
                ...
                "required_recent_delta_pct": gov_cfg.promotion_margin_pct + (gov_cfg.bnb_shield_margin_pct if core_is_bnb else 0.0),
                "anti_flip_active": anti_flip_active,
...
def load_shortlist(shortlist_path: Path) -> list[dict]:
    ...
    for _, row in df.iterrows():
        ...
        if asset in NO_NEO_DROP:
            continue
        rows.append({"asset": asset, "file": row["file"]})
    return rows


def build_profiles() -> list[tuple[ChallengerOverlayConfig, WeeklyGovernanceConfig]]:
    common_overlay = dict(
        score_lb=30,
        fast_ma=20,
        slow_ma=100,
    ...
    return [
        (
            ...
            WeeklyGovernanceConfig(
                profile_name="phase67k_no_neo_control_67j",
                recent_days=42,
                ...
                anti_flip_lookback_periods=8,
                anti_flip_max_switches=99,
                anti_flip_block_periods=0,
                stricter_entry_train_delta_pct=0.0,
                stricter_entry_recent_delta_pct=0.0,
                stricter_entry_score_margin=-999.0,
                bnb_deny_recent_delta_below_pct=-999.0,
                bnb_deny_train_delta_below_pct=-999.0,
                **common_governance,
            ),
        ),
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67k_no_neo_anti_flip_guard",
                recent_days=42,
                min_recent_delta_pct=0.0,
                switch_score_margin=2.5,
                min_hold_periods=3,
                promotion_margin_pct=2.0,
                persistence_weeks=2,
                ...
                bnb_shield_margin_pct=1.0,
                anti_flip_lookback_periods=8,
                anti_flip_max_switches=2,
                anti_flip_block_periods=2,
                stricter_entry_train_delta_pct=0.0,
                stricter_entry_recent_delta_pct=0.0,
                stricter_entry_score_margin=-999.0,
                bnb_deny_recent_delta_below_pct=-999.0,
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67k_no_neo_stricter_entry",
                recent_days=56,
                min_recent_delta_pct=0.25,
                switch_score_margin=2.5,
                min_hold_periods=3,
                promotion_margin_pct=2.5,
                persistence_weeks=3,
                reentry_cooldown_days=14,
                downside_lookback_days=42,
                bnb_shield_margin_pct=1.0,
                anti_flip_lookback_periods=8,
                anti_flip_max_switches=3,
                anti_flip_block_periods=1,
                stricter_entry_train_delta_pct=1.0,
                stricter_entry_recent_delta_pct=0.5,
                stricter_entry_score_margin=2.0,
                bnb_deny_recent_delta_below_pct=-999.0,
                bnb_deny_train_delta_below_pct=-999.0,
                **common_governance,
            ),
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67k_no_neo_2025_combined_guard",
                recent_days=56,
                min_recent_delta_pct=0.25,
                ...
                persistence_weeks=3,
                reentry_cooldown_days=14,
                downside_lookback_days=63,
                bnb_shield_margin_pct=2.0,
                anti_flip_lookback_periods=8,
                anti_flip_max_switches=2,
                anti_flip_block_periods=2,
                stricter_entry_train_delta_pct=1.0,
                stricter_entry_recent_delta_pct=0.5,
                stricter_entry_score_margin=2.0,
                bnb_deny_recent_delta_below_pct=2.0,
                bnb_deny_train_delta_below_pct=1.0,
                **common_governance,
            ),
        ),
    ]
...
def print_compare_rows(compare: pd.DataFrame) -> None:
    if compare.empty:
        return
    log("")
    log("=== PHASE67K ALL PROFILE RESULTS ===")
    for _, row in compare.iterrows():
        log(
            ...
            f"uniq {int(pd.to_numeric(row['unique_selected_assets'], errors='coerce'))}"
        )


def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE67K no-NEO 2025 guardrails")
    ...
    log("[PHASE67K] Start")
    log(f"[PHASE67K] Core paper: {args.core_paper}")
    log(f"[PHASE67K] No-NEO shortlist assets: {[x['asset'] for x in shortlist]}")
    ...
    for overlay_cfg, gov_cfg in profiles:
        asset_strategies: dict[str, pd.DataFrame] = {}
        failed_assets = []
        ...
        row["final_shortlist"] = ",".join(sorted(asset_strategies.keys()))
        ...
        row["anti_flip_lookback_periods"] = gov_cfg.anti_flip_lookback_periods
        row["anti_flip_max_switches"] = gov_cfg.anti_flip_max_switches
        row["anti_flip_block_periods"] = gov_cfg.anti_flip_block_periods
        ...
        log(f"[PHASE67K] done {gov_cfg.profile_name}")
    ...
    if best_model and best_model in papers:
        ...
        live_status = pd.DataFrame(
            [{
                ...
                "final_shortlist": str(best.iloc[0]["final_shortlist"]) if not best.empty else "",
            }]
        )
    ...
    summary_path = PHASE67K_DIR / "phase67k_no_neo_2025_guardrails_summary.csv"
    compare_path = PHASE67K_DIR / "phase67k_no_neo_2025_guardrails_compare.csv"
    decisions_path = PHASE67K_DIR / "phase67k_no_neo_2025_guardrails_decisions.csv"
    leaderboard_path = PHASE67K_DIR / "phase67k_no_neo_2025_guardrails_leaderboard.csv"
    asset_quality_path = PHASE67K_DIR / "phase67k_no_neo_2025_guardrails_asset_quality.csv"
    live_status_path = PHASE67K_DIR / "phase67k_live_status.csv"
    latest_top10_path = PHASE67K_DIR / "phase67k_latest_decision_top10.csv"
    manifest_path = PHASE67K_DIR / "phase67k_manifest.json"
    ...
    log("")
    log("=== PHASE67K TOP RESULT ===")
    if best.empty:
        log("No phase67k profile processed.")
    else:
        row = best.iloc[0]
        log(f"model: {row['model']}")
        log(f"final_shortlist: {row['final_shortlist']}")
    ...
    if not live_status.empty:
        ...
        log(f"final_shortlist: {ls['final_shortlist']}")

    log("")
    log(f"[PHASE67K] Saved summary -> {summary_path}")
    log(f"[PHASE67K] Saved compare -> {compare_path}")
    log(f"[PHASE67K] Saved decisions -> {decisions_path}")
    log(f"[PHASE67K] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE67K] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE67K] Saved live status -> {live_status_path}")
    log(f"[PHASE67K] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE67K] Saved manifest -> {manifest_path}")
