...
PHASE67F_DIR = OUTPUTS / "phase67f_targeted_guardrail_backtest"
        ...
        decision_rows.append(
            {
                ...
                "required_recent_delta_pct": (
                    gov_cfg.promotion_margin_pct + (gov_cfg.bnb_shield_margin_pct if core_is_bnb else 0.0)
                ),
            }
        )
    ...
    return [
        (
            ...
            WeeklyGovernanceConfig(
                profile_name="phase67f_pm002_pw2_cd7_dl42_bs001_main",
                recent_days=42,
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67f_pm001_pw2_cd7_dl42_bs001_softer",
                recent_days=42,
                min_recent_delta_pct=0.0,
                min_hold_periods=2,
                promotion_margin_pct=1.0,
                persistence_weeks=2,
                reentry_cooldown_days=7,
                downside_lookback_days=42,
                bnb_shield_margin_pct=1.0,
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67f_pm002_pw2_cd7_dl63_bs001_harder_downside",
                recent_days=63,
                min_recent_delta_pct=0.0,
                min_hold_periods=2,
                promotion_margin_pct=2.0,
        ...
        (
            ChallengerOverlayConfig(**common_overlay),
            WeeklyGovernanceConfig(
                profile_name="phase67f_pm002_pw2_cd7_dl42_bs002_bnb_strict",
                recent_days=42,
                min_recent_delta_pct=0.0,
                min_hold_periods=2,
                promotion_margin_pct=2.0,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PHASE67F targeted guardrail backtest over 66G core")
    ...
    log("[PHASE67F] Start")
    log(f"[PHASE67F] Core paper: {args.core_paper}")
    log(f"[PHASE67F] Shortlist assets: {[x['asset'] for x in shortlist]}")
    ...
    for overlay_cfg, gov_cfg in profiles:
        ...
        for item in shortlist:
            ...
            try:
                ...
                failed_assets.append({"profile": gov_cfg.profile_name, "asset": asset, "reason": str(e)})
        ...
        log(f"[PHASE67F] done {gov_cfg.profile_name}")
    ...
    summary_path = PHASE67F_DIR / "phase67f_targeted_guardrail_summary.csv"
    compare_path = PHASE67F_DIR / "phase67f_targeted_guardrail_compare.csv"
    decisions_path = PHASE67F_DIR / "phase67f_targeted_guardrail_decisions.csv"
    leaderboard_path = PHASE67F_DIR / "phase67f_targeted_guardrail_leaderboard.csv"
    asset_quality_path = PHASE67F_DIR / "phase67f_targeted_guardrail_asset_quality.csv"
    live_status_path = PHASE67F_DIR / "phase67f_live_status.csv"
    latest_top10_path = PHASE67F_DIR / "phase67f_latest_decision_top10.csv"
    manifest_path = PHASE67F_DIR / "phase67f_manifest.json"
    ...
    manifest = {
        ...
        "notes": [
            "67F je targeted backtest pack okolo 67E winner guardrail profilu.",
            "Promotion margin, persistence, reentry cooldown, downside window a BNB shield sú explicitne implementované.",
            "Cieľ: dorovnať alebo zlepšiť 2025 bez zabitia full-history liftu 67D.",
        ],
    }
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    log("")
    log("=== PHASE67F TOP RESULT ===")
    if best.empty:
        log("No phase67f profile processed.")
    else:
        row = best.iloc[0]
        ...
        log(f"promotion_margin_pct: {row['promotion_margin_pct']:.2f}")
        log(f"persistence_weeks: {int(row['persistence_weeks'])}")
        log(f"reentry_cooldown_days: {int(row['reentry_cooldown_days'])}")
        log(f"downside_lookback_days: {int(row['downside_lookback_days'])}")
        log(f"bnb_shield_margin_pct: {row['bnb_shield_margin_pct']:.2f}")
        log("")

    if not live_status.empty:
        ls = live_status.iloc[0]
        log("=== PHASE67F LIVE STATUS ===")
    ...
    log(f"[PHASE67F] Saved summary -> {summary_path}")
    log(f"[PHASE67F] Saved compare -> {compare_path}")
    log(f"[PHASE67F] Saved decisions -> {decisions_path}")
    log(f"[PHASE67F] Saved leaderboard -> {leaderboard_path}")
    log(f"[PHASE67F] Saved asset quality -> {asset_quality_path}")
    log(f"[PHASE67F] Saved live status -> {live_status_path}")
    log(f"[PHASE67F] Saved latest top10 -> {latest_top10_path}")
    log(f"[PHASE67F] Saved manifest -> {manifest_path}")
