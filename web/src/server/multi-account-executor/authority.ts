import { validateResolvedRoute } from "./route-identity";
import "server-only";
...
export function normalizedTargetAsset(value: unknown): string | null {
  const asset = asString(value)?.toUpperCase();
  return asset && /^[A-Z0-9][A-Z0-9._:-]{0,63}$/.test(asset) ? asset : null;
}
...
export function parseAuthorizedTarget(snapshotValue: unknown, attemptValue: unknown): AuthorizedTarget {
  ...
  const asset = normalizedTargetAsset(intent.target_asset);
  ...
  if (!closedDay || !strategyVersion || !signalId || !asset || exposure === null || exposure < 0) throw new AuthorityError("authority target is invalid or ambiguous");
  ...
  const gateState = gate.status === "no_action" ? "no_action" : gate.would_place_real_order === true ? "approved" : null;
  if (!gateState) throw new AuthorityError("authority gate does not permit the target");
  return { strategyVersion, closedDay, signalId, asset, exposure, stale: false, executionGate: gateState };
}
...
export async function loadCanonicalRunTarget(
  ...
  const [productionText, intentText, gateText, accountText, modeText] = await Promise.all([
    readFile(productionPath, "utf8"),
    readFile(intentPath, "utf8"),
    readFile(gatePath, "utf8"),
    readFile(accountPath, "utf8").catch(() => null),
    readFile(path.join(repositoryRoot, "execution/config/execution_mode.json"), "utf8")
  ]);
  ...
  const routeRequired = await readFile(path.join(repositoryRoot, "source_of_truth/production_route_identity_contract.json"), "utf8").then(() => true).catch((error) => { if (error.code === "ENOENT") return false; throw error; });
  if (routeRequired || production.resolved_execution_asset !== undefined) validateResolvedRoute(production, intent);
  ...
  const asset = normalizedTargetAsset(productionIntent.target_asset);
  ...
  if (productionIntent.stale_signal !== false || asRecord(production.validation).status !== "passed" || production.strategy_status !== "ready") throw new AuthorityError("canonical target is stale or invalid");
  if (asRecord(JSON.parse(modeText)).kill_switch !== false) throw new AuthorityError("emergency execution switch is active or invalid");
  if (asset !== "CASH" && (production.trend_permission_active !== true || productionIntent.allow_live_order_candidate !== true)) throw new AuthorityError("canonical entry is not authorized by the strategy");
  ...
  if (fingerprints.production_snapshot_sha256 !== sha256(productionText) || intentFingerprints.production_snapshot_sha256 !== sha256(productionText) || fingerprints.intent_sha256 !== sha256(intentText)) throw new AuthorityError("canonical source fingerprints do not match");
  const perAccountValidation = process.env.MRV1_EXECUTION_BACKEND === "multi_account" && gate.account_validation_scope === "per_account_exchange";
  if (!perAccountValidation && (!accountText || fingerprints.account_snapshot_sha256 !== sha256(accountText))) throw new AuthorityError("canonical account context fingerprint does not match");
  // Wallet-specific gate failures belong to the affected account's preflight.
  if ((asset === "CASH" && exposure !== 0) || (asset !== "CASH" && exposure <= 0)) throw new AuthorityError("canonical target exposure is invalid");
  return { strategyVersion, closedDay, signalId, asset, exposure, stale: false, executionGate: "approved" };
}
