...
describe("production deployment assets are derived from strategy data", () => {
  it("plans every emitted target from cash, each emitted position and another supported position", () => {
    expect(verifyProductionAssetSupport(deriveProductionTargets(csv, snapshot), markets)).toEqual({ assets: ["AVAX", "BTC", "CASH"], planCount: 12 });
  });
  ...
  it("rejects deployment for an unsupported strategy target before trading", () => {
    ...
    expect(() => verifyProductionAssetSupport(targets, markets)).toThrow(/does not support UNLISTED/);
  });
  it("does not silently whitelist historical synthetic execution targets", () => {
    ...
    expect(() => verifyProductionAssetSupport(targets, markets)).toThrow(/does not support BASE/);
  });
