...
OUT_DIR = OUTPUTS / "phase48_bnb_hybrid_tune"
...
MODEL_LABELS = {
    "phase48_current_main": "Current main (No BNB)",
    "phase48_bnb_trend_loose": "BNB hybrid loose",
    "phase48_bnb_trend_base": "BNB hybrid base",
    "phase48_bnb_trend_strict": "BNB hybrid strict",
    "phase48_bnb_trend_plus_gap": "BNB hybrid trend + gap",
}

VARIANT_RULES = {
    "current_main": None,
    "bnb_trend_loose": {"lt_min": 40.0, "st_min": 30.0, "xs20_min": 10.0, "gap_min": None},
    "bnb_trend_base": {"lt_min": 45.0, "st_min": 35.0, "xs20_min": 20.0, "gap_min": None},
    "bnb_trend_strict": {"lt_min": 55.0, "st_min": 40.0, "xs20_min": 30.0, "gap_min": None},
    "bnb_trend_plus_gap": {"lt_min": 45.0, "st_min": 35.0, "xs20_min": 20.0, "gap_min": 8.0},
}
...
def bnb_allowed(top: dict, next_non_bnb: dict | None, rule: dict) -> bool:
    if not (
        top["lt_score"] >= rule["lt_min"]
        and top["st_score"] >= rule["st_min"]
        and top["xs20"] >= rule["xs20_min"]
    ):
        return False
    ...
    if next_non_bnb is None:
        return True

    return (top["score"] - next_non_bnb["score"]) >= gap_min
...
def select_daily_top1_variant(asset_tables: dict[str, pd.DataFrame], variant: str) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        if market_score >= MARKET_THRESHOLD and candidates:
            ...
            if variant == "current_main":
                non_bnb = [c for c in candidates if c["symbol"] != "BNBUSDT"]
                if non_bnb:
                    selected = non_bnb[0]["symbol"]
            else:
                top = candidates[0]
                if top["symbol"] == "BNBUSDT":
                    next_non_bnb = next((c for c in candidates if c["symbol"] != "BNBUSDT"), None)
                    if bnb_allowed(top, next_non_bnb, rule):
                        selected = "BNBUSDT"
                    else:
...
def main() -> None:
    ...
    variants = [
        ("phase48_current_main", "current_main"),
        ("phase48_bnb_trend_loose", "bnb_trend_loose"),
        ("phase48_bnb_trend_base", "bnb_trend_base"),
        ("phase48_bnb_trend_strict", "bnb_trend_strict"),
        ("phase48_bnb_trend_plus_gap", "bnb_trend_plus_gap"),
    ]
    ...
    summary_df = pd.DataFrame(summary_rows).sort_values(
        ["recent_total_return_pct", "recent_cagr_pct", "max_drawdown_pct"],
        ascending=[False, False, False],
    )
    summary_df.to_csv(OUT_DIR / "phase48_bnb_hybrid_tune_summary.csv", index=False)
    ...
    eq = eq.reset_index()
    eq.to_csv(OUT_DIR / "phase48_bnb_hybrid_tune_equity_curves.csv", index=False)

    print("\n=== PHASE48 BNB HYBRID TUNE SUMMARY ===\n")
    print(summary_df.to_string(index=False))
    print(f"\nSaved summary: {OUT_DIR / 'phase48_bnb_hybrid_tune_summary.csv'}")
    print(f"Saved equity curves: {OUT_DIR / 'phase48_bnb_hybrid_tune_equity_curves.csv'}")
