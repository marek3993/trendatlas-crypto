...
OUT_DIR = OUTPUTS / "phase59_bnb_selective_restore"
...
MODEL_LABELS = {
    "phase59_phase42_core": "Phase42 core",
    "phase59_phase49_strict": "Phase49 strict",
    "phase59_restore_trx_only": "Restore BNB vs TRX only",
    "phase59_restore_trx_sol": "Restore BNB vs TRX/SOL",
}
...
def bnb_strict_allowed(bnb_candidate: dict) -> bool:
    return (
        bnb_candidate["lt_score"] >= BNB_STRICT_RULE["lt_min"]
        and bnb_candidate["st_score"] >= BNB_STRICT_RULE["st_min"]
        and bnb_candidate["xs20"] >= BNB_STRICT_RULE["xs20_min"]
    )
...
def select_daily_top1_variant(asset_tables: dict[str, pd.DataFrame], model_key: str) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        if market_score >= MARKET_THRESHOLD and candidates:
            ...
            if model_key == "phase59_phase42_core":
                selected = top["symbol"]

            elif model_key == "phase59_phase49_strict":
                if top["symbol"] == "BNBUSDT":
                    if bnb_strict_allowed(top):
                        selected = "BNBUSDT"
                    else:
            ...
            elif model_key == "phase59_restore_trx_only":
                if top["symbol"] == "BNBUSDT":
                    if bnb_strict_allowed(top):
                        selected = "BNBUSDT"
                    else:
                        next_non_bnb = next((c for c in candidates if c["symbol"] != "BNBUSDT"), None)
                        if next_non_bnb is not None and next_non_bnb["symbol"] == "TRXUSDT":
                            selected = "BNBUSDT"
                        else:
                            selected = next_non_bnb["symbol"] if next_non_bnb is not None else "CASH"
                else:
            ...
            elif model_key == "phase59_restore_trx_sol":
                if top["symbol"] == "BNBUSDT":
                    if bnb_strict_allowed(top):
                        selected = "BNBUSDT"
                    else:
                        next_non_bnb = next((c for c in candidates if c["symbol"] != "BNBUSDT"), None)
                        if next_non_bnb is not None and next_non_bnb["symbol"] in {"TRXUSDT", "SOLUSDT"}:
                            selected = "BNBUSDT"
                        else:
                            selected = next_non_bnb["symbol"] if next_non_bnb is not None else "CASH"
                else:
...
def main() -> None:
    ...
    variants = [
        "phase59_phase42_core",
        "phase59_phase49_strict",
        "phase59_restore_trx_only",
        "phase59_restore_trx_sol",
    ]
    ...
    summary_df.to_csv(OUT_DIR / "phase59_bnb_selective_restore_summary.csv", index=False)
    ...
    eq = eq.reset_index()
    eq.to_csv(OUT_DIR / "phase59_bnb_selective_restore_equity_curves.csv", index=False)

    print("\n=== PHASE59 BNB SELECTIVE RESTORE ===\n")
    print(summary_df.to_string(index=False))
    print(f"\nSaved summary: {OUT_DIR / 'phase59_bnb_selective_restore_summary.csv'}")
    print(f"Saved equity curves: {OUT_DIR / 'phase59_bnb_selective_restore_equity_curves.csv'}")
